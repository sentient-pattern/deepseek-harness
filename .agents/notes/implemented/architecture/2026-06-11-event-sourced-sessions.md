...
- Persistence stays a plugin concern; the in-memory store ships in fw-session.
...
- Derivation cost grows with log length — compaction (fw-compaction) is the intended mitigation, not log mutation.
