...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `fw-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `fw-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `fw-timeout` (`idleWatchdog`) |
| Clear the timer | `fw-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `fw-timeout` (`timeoutOf`) |
