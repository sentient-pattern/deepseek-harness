...
工具是 Client UI 的一级展示概念，由 `@forgeweaver/fw-client-ui-tool` 统一拥有 root/subcall 编排、按 wire 工具名称的原子 renderer 分发、Generic fallback、card model 和 details output。业务插件只注册自己的原子工具 renderer，不修改 conversation 或会话。
