...
harness 在 Windows 上没有持久 shell。持久 `bash` 栈按构造就是 POSIX-only：`@forgeweaver/fw-subprocess-local` 在终端分配时直接抛错（`createProcessInspector()` 拒绝 win32），`@forgeweaver/fw-terminal-bash` 是 bash 形态（`/bin/bash` 默认值、`PS1`/`PROMPT_COMMAND` 环境标记），`@forgeweaver/fw-tool-bash-persistent` 用 bash 语法包装命令，pty 测试全部在 win32 上 skip。一次性 `pwsh` 工具（`@forgeweaver/fw-tool-pwsh` + `@forgeweaver/fw-pwsh-local`）已经能在 Windows 运行，但每次调用都是全新的 `pwsh -Command` 进程：cwd、`$env:` 变量、函数和交互式子进程都随调用结束，其 README 把 "No persistent shell or PTY" 记为 deferred work。
...
### `@forgeweaver/fw-subprocess-local` 的 Windows 基座
