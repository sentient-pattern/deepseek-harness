...
仓库还曾在两种含义下使用 `SDK`。受支持的 Python 和 TypeScript 客户端使用 JSON-RPC SDK 协议。项目整体是 ForgeWeaver，而不是 SDK 项目。已移除的 SDK 项目工具链使宽泛的含义失去依据，但文案和名称仍保留了部分旧用法。
...
`SDK` 表示受支持的 Python 和 TypeScript SDK 所使用、基于 JSON-RPC 的客户端／服务器协议。仓库保留 `@forgeweaver/fw-sdk-client`、`@forgeweaver/fw-sdk-protocol` 和协议身份 `forgeweaver-harness-sdk-runtime`；JSON-RPC 服务器属于同一系列。ForgeWeaver 本身不是 SDK，已移除的项目生成器、启动器、辅助工具和启动器遥测包继续保持不存在。
...
不得为了避免重复而删除有意保留的供应商限定词。`fw-subagent-fw-sdk` 表示 ForgeWeaver SDK 提供方，可避免与其他 SDK 混淆。其私有类改名为 `SdkSubagentProvider`，因为类名还需要说明它提供什么。
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@forgeweaver/fw-jsonrpc` | `@forgeweaver/fw-sdk-jsonrpc-server` | 它是 SDK 协议的服务器端。单独使用 `jsonrpc` 只说明编码；`sdk-jsonrpc-server` 则同时说明所属系列、机制和职责。 |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | 该类是 JSON-RPC 服务器的一种实现，并不代表所有可能的 SDK 服务器。 |

保留 `@forgeweaver/fw-sdk-client`、`@forgeweaver/fw-sdk-protocol` 和 `forgeweaver-harness-sdk-runtime`。排除 `@forgeweaver/create-sdk`、`@forgeweaver/fw-scripts`、`@forgeweaver/fw-helper` 和 `@forgeweaver/fw-telemetry`；单独的移除决策负责删除这些包及其支撑依赖图。
...
| `@forgeweaver/fw-bash`, `ctx.bash` | `@forgeweaver/fw-shell`, `ctx.shell` | PowerShell 已经实现该 seam。此项能力是 shell 执行，而不是 Bash。 |
...
| `@forgeweaver/fw-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@forgeweaver/fw-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | Bash 和 PowerShell 工具共享该环境注册表。 |
...
| `@forgeweaver/fw-pty`, `ctx.pty`, `PtyService` | `@forgeweaver/fw-terminal`, `ctx.terminals`, `TerminalSessionService` | 调用方管理多个具名终端会话，而不是通过该服务分配原始 PTY。 |
| 公开的高层 `Pty*` 会话和后端名称 | `Terminal*` 名称 | 公开抽象是终端会话。保留底层 `SubprocessTerminal*` 名称，因为它们已经说明底层机制。 |
| `@forgeweaver/fw-pty-local`, `LocalPtyBackend` | `@forgeweaver/fw-terminal-bash`, `BashTerminalBackend` | 该提供方依赖 Bash 提示符和 shell 行为。`local` 隐藏了实际方言。 |
| `@forgeweaver/fw-tool-pty` | `@forgeweaver/fw-tool-terminal` | 面向模型的工具已使用 `terminal_*`；包应采用相同的产品名词。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@forgeweaver/fw-lsp-local` | `@forgeweaver/fw-lsp-stdio` | 该提供方通过可替换的文件系统和子进程服务，以 stdio 传输 LSP。它不一定在本地运行。 |
| `packages/tasks/` | `packages/jobs/` | 该系列负责脱离前台运行的工具作业。`jobs` 简短，并可避免与用户任务或 todo 概念冲突。 |
| `@forgeweaver/fw-tasks`, `ctx.tasks`, `TaskService` | `@forgeweaver/fw-jobs`, `ctx.jobs`, `JobRegistry` | 该服务注册、拥有、观察、等待并取消多个后台作业。它是注册表，而不是通用任务服务。 |
...
| `@forgeweaver/fw-tasks-local`, `LocalTaskService` | `@forgeweaver/fw-jobs-local`, `LocalJobRegistry` | 这是作业注册表的进程内提供方。此处的 `local` 有明确含义，因为作业和回调都存在于同一进程。 |
| `@forgeweaver/fw-tool-tasks` | `@forgeweaver/fw-tool-jobs` | 消费方控制作业注册表，应使用相同的领域名词。 |
...
| `@forgeweaver/fw-client-ui-task`、`client/ui-task/` | `@forgeweaver/fw-client-ui-jobs`、`client/ui-jobs/` | 该客户端包呈现后台作业集合，而不是一项用户任务。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@forgeweaver/fw-client-ui-slash`, `ui-slash/` | `@forgeweaver/fw-client-ui-input-trigger`, `ui-input-trigger/` | 客户端处理 `/`、`@`、键盘仲裁、候选菜单和程序化启动，并非只处理斜杠命令。 |
...
| `@forgeweaver/fw-agent-tool-mode`，插件 `tool-mode` | `@forgeweaver/fw-agent-tool-presentation`，插件 `tool-presentation` | 该插件改变工具向模型呈现的方式，而不改变执行行为。保留局部 `Config.mode` 和 `ToolPresentationMode`。 |
| `packages/interaction/permission/` | `packages/interaction/permission-presets/` | 该包拥有沙箱与审批设置的具名组合，而不负责执行权限。 |
| `@forgeweaver/fw-permission`, `ctx.permission`, `PermissionService` | `@forgeweaver/fw-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | 该服务选择并持久化预设。沙箱和审批服务负责执行结果。 |
| `@forgeweaver/fw-client-ui-permission` | `@forgeweaver/fw-client-ui-permission-presets` | UI 编辑和选择权限预设。 |
| `docs/subsystems/permission.md` | `docs/subsystems/permission-presets.md` | 该页面记录预设选择，而不是权限执行。 |
| `@forgeweaver/fw-user-interaction`, `user-interaction/` | `@forgeweaver/fw-user-questions`, `user-questions/` | 该 seam 仅支持批量问题和答案。审批、命令和目录选择属于其他交互 seam。 |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | 这些名称说明唯一受支持的交互形式。保留 `AskUserQuestion*`、`ask_user_question` 工具和 `@forgeweaver/fw-tool-ask-user`。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `packages/typert/type-meta/`, `@forgeweaver/fw-type-meta` | `typert/protocol/`, `@forgeweaver/fw-typert-protocol` | 该包拥有 Typert Remote 协议、装饰器、绑定、编解码器、查找逻辑和上下文约定。它不是通用类型元数据。 |
...
保留 `@forgeweaver/fw-tools` 和 `ctx.tools`。保留 `@forgeweaver/fw-api-gateway`、其 `gateway/` 目录、`ctx.typertGateway` 以及 `TypertGatewayService`；该服务是真正的 API 网关。其内部的 `TypeRT*` 标识符仍应遵循 `Typert*` 拼写规则。

### 工作区指令、遥测、身份和启动环境

| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` 拥有多个工作区，但 Client `ctx.workspaces` 已经使用不兼容的类型。即使二者运行时上下文独立，两份声明仍会在编译时合并进同一个 Cordis `Context` 接口。职责后缀明确指出 host 服务，并避免该冲突。保留 `@forgeweaver/fw-workspace`、`WorkspaceRegistry`、`Workspace` 和 `workspace.*` 协议名称。 |
| `@forgeweaver/fw-workspace-context`, `context/workspace-context/` | `@forgeweaver/fw-agent-instructions`, `context/agent-instructions/` | 该包为 agent（智能体）加载分层的 `AGENTS.md` 和 `CLAUDE.md` 文件。它并非通用工作区上下文。 |
...
| `TelemetryOtel`、`TelemetryMode`，插件 `telemetry-otel` | `OpenTelemetrySessionBackend`、`SessionTelemetryMode`，插件 `session-telemetry-otel` | 提供方名称同时说明 OpenTelemetry 机制和会话作用域。保留包名 `fw-session-telemetry` 和 `fw-session-telemetry-otel`。 |
| `docs/subsystems/telemetry.md` | `docs/subsystems/session-telemetry.md` | 该页面记录会话遥测，而不是仓库级可观测性。 |
| `session/user-id/`, `@forgeweaver/fw-user-id` | `identity/anonymous-user-id/`, `@forgeweaver/fw-anonymous-user-id` | 该值是遥测、反馈和 ForgeWeaver 请求共用的随机关联 id。它既不属于 Session 领域，也不是经过身份验证的用户身份。 |
...
| `util/environment/`, `@forgeweaver/fw-environment` | `util/launch-environment/`, `@forgeweaver/fw-launch-environment` | 该包在启动时捕获一份不可变的分层快照。它不是通用环境 API。 |
| 公开的 `Environment*`、`createEnvironmentSnapshot`、`environmentOf`、`FW_ENVIRONMENT_KEY` | `LaunchEnvironment*`、`createLaunchEnvironmentSnapshot`、`launchEnvironmentOf`、`FW_LAUNCH_ENVIRONMENT_KEY` | 这些名称说明快照的生命周期和用途。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@forgeweaver/fw-tool-schedule`、`schedule/tool-schedule/`、插件 `tool-schedule` | `@forgeweaver/fw-schedule`、`schedule/schedule/`、插件 `schedule` | 该包拥有持久 Schedule 领域、持久化屏障、管理工具、定时器、后续轮次和运行时生命周期。`tool-` 只描述其中一部分。 |
| `ScheduleOwner` | `ScheduleRuntime` | 该逐 agent 对象运行实时定时器、持久化投影、分派、空闲等待和资源释放。`Owner` 没有说明这一执行职责。耦合的私有 `owner*` 名称也改用 `runtime*`。 |
| `WorkflowService`, `ctx.workflows` | `WorkflowEngine`, `ctx.workflowEngine` | 一个引擎负责解析并执行工作流程序。复数键错误地暗示这是注册表。保留 `@forgeweaver/fw-workflow` 以及工作流事件和工具。 |
| `@forgeweaver/fw-workflow-workerthread`, `WorkerWorkflowEngine` | `@forgeweaver/fw-workflow-worker-thread`, `WorkerThreadWorkflowEngine` | `worker thread` 是准确的 Node 机制，仓库拼写要求使用完整单词。 |
| `@forgeweaver/fw-goal-session`, `goal/goal-session/` | `@forgeweaver/fw-goal-round-driver`, `goal/goal-round-driver/` | 该插件驱动同一会话内的 Goal Rounds。它既不存储目标，也不定义会话。保留 `GoalService`、目标来源、事件和约定。 |
| `packages/compact/` | `packages/compaction/` | 该组是以名词命名的领域系列。`compact` 仍作为面向用户的命令动词。 |
| `@forgeweaver/fw-compact`, `ctx.compact`, `CompactService` | `@forgeweaver/fw-compaction`, `ctx.compaction`, `CompactionEngine` | 该对象运行压缩（compaction）算法和生命周期。它是引擎，而不是通用服务。 |
| `compact/*` 事件和公开领域前缀 | `compaction/*` | 事件和领域类型使用名词形式。保留动词形式的操作，例如 `compactNow`、`compactRegion` 和 `compactIfNeeded`。 |
| `@forgeweaver/fw-compact-basic`、`BasicCompactService`、公开的 `BasicCompact*` | `@forgeweaver/fw-compaction-basic`、`BasicCompactionEngine`、对应的 `BasicCompaction*` | `basic` 朴素但准确。`compaction-llm` 没有增加信息，因为当前实现系列已使用 LLM。 |
| `@forgeweaver/fw-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@forgeweaver/fw-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | 该插件是剪除工具结果的执行主体。名词 `pruner` 说明了这一职责。 |
...
| `@forgeweaver/fw-settings-local`, `SettingsLocal` | `@forgeweaver/fw-settings-file`, `FileSettingsProvider` | 该实现通过文件系统 seam 以文件为后端。`file` 说明机制，`local` 则不能。 |
...
| `@forgeweaver/fw-session-export`, `session-export/`, Loader id `session-export`, `ctx.sessionExport` | `@forgeweaver/fw-session-log-export`, `session-log-export/`, Loader id `session-log-download`, `ctx.sessionLogDownload` | npm 包名使用 Session 日志导出语义，因为 npm 禁止包名包含 `download`。Loader id 与浏览器 API 保留 `download`，因为它们描述浏览器副作用。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | 该服务器拥有 HTTP 路由和 WebSocket 升级路由。`Web` 可以同时涵盖两者；此处的 `Http` 作用域过窄。保留 `packages/host/webserver`、`@forgeweaver/fw-host-webserver`、`WebRoute` 和 `WebUpgradeRoute`。 |
...
| `@forgeweaver/fw-session-title-first-message-llm`，触发周期 `first-message` | `@forgeweaver/fw-session-title-first-prompt-llm`，触发周期 `first-prompt` | 触发条件是第一条用户提示词，而不是会话日志中的任意消息。 |
| `@forgeweaver/fw-session-title-all-messages-llm`，触发周期 `all-user-messages` | `@forgeweaver/fw-session-title-all-prompts-llm`，触发周期 `all-prompts` | 后端根据用户提示词刷新。`all messages` 会错误地包含助手消息和工具事件。 |
| `@forgeweaver/fw-code-runtime-worker`, `WorkerCodeRuntime` | `@forgeweaver/fw-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | 该实现使用 Node 工作线程。单独的 `worker` 作用域过宽。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@forgeweaver/fw-fs-policy` | `@forgeweaver/fw-fs-observation-policy` | 该包定义哪些文件系统观察可以授权后续操作。它不是完整的文件系统策略或沙箱策略。 |
| `FsPolicyExec` | `FsObservationActor` | 该值表示策略所关联的观察与操作的执行主体。它本身不执行策略。 |
| `SkillService` | `SkillRegistry` | 该服务注册提供方，并从其目录解析 skill（技能）。 |
| `@forgeweaver/fw-skill-local`、`LocalSkillProvider`，提供方 id `local` | `@forgeweaver/fw-skill-filesystem`、`FileSystemSkillProvider`，提供方 id `filesystem` | 该提供方通过可位于本地或远端的 `ctx.fs` 发现 skill 文件。其机制是文件系统访问，而不是本地性。 |
| `SubagentService` | `SubagentRuntime` | 该服务选择提供方，并拥有实时 spawn、恢复、跟进、取消和结算行为。 |
| `@forgeweaver/fw-subagent-spawn`, `SpawnProvider` | `@forgeweaver/fw-subagent-spawn-in-process`, `SpawnInProcessProvider` | 该提供方在当前进程内启动子 agent。配置的提供方 id 仍为 `spawn`。 |
| `@forgeweaver/fw-subagent-fork`, `ForkProvider` | `@forgeweaver/fw-subagent-fork-in-process`, `ForkInProcessProvider` | 该提供方在当前进程内 fork 一个 agent。配置的提供方 id 仍为 `fork`。 |
| `@forgeweaver/fw-subagent-inprocess`, `subagent-inprocess/` | `@forgeweaver/fw-subagent-in-process-driver`, `subagent-in-process-driver/` | 该包包含通用的进程内驱动逻辑，而不是第三个提供方。 |
| 私有的 `SdkProvider`，位于 `fw-subagent-fw-sdk` 中 | `SdkSubagentProvider` | 重复的包限定词是有意保留的，类名还必须说明它通过 SDK 提供 subagent。 |
| `WebService`, `WebServiceConfig` | `WebRuntime`, `WebRuntimeConfig` | 该对象选择提供方并运行实时搜索和抓取操作。保留包、键、提供方包和模型工具。 |
| `@forgeweaver/fw-web-fetch-local`、`LocalFetchProvider`、`LocalFetchLimits`，提供方 id `local-http` | `@forgeweaver/fw-web-fetch-http`、`HttpFetchProvider`、`HttpFetchLimits`，提供方 id `http` | 该提供方执行直接 HTTP 抓取。`local` 只说明代码恰好在哪里运行，并未说明它提供哪种机制。 |

保留 `@forgeweaver/fw-subagent-fw-sdk`、其提供方 id `fw-sdk`、外部 ACP（Agent Client Protocol）、Codex 和 Claude Code 提供方系列、subagent 工具包名、主文件系统包和后端、文件系统工具和事件，以及 skill 徽章和工具包。

### 钩子、防护、Plan Mode、扩展与诊断

| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@forgeweaver/fw-hooks-claude`、`ClaudeHookConfig`、`parseClaudeConfig`，方言 `claude` | `@forgeweaver/fw-hooks-claude-code`、`ClaudeCodeHookConfig`、`parseClaudeCodeConfig`，方言 `claude-code` | 该钩子桥接面向 Claude Code，而非所有 Anthropic 或 Claude 产品。 |
| `@forgeweaver/fw-repeat-tool-guard`，插件／来源 `repeat-tool-guard` | `@forgeweaver/fw-repeat-tool-reminder`，插件／来源 `repeat-tool-reminder` | 该插件向模型添加提醒，并不阻止工具调用，也不执行防护决策。 |
| `@forgeweaver/fw-timeout-policy` | `@forgeweaver/fw-tool-call-timeout-policy` | 完整的 `tool-call` 限定词说明该策略限制的对象，而不会把插件称为面向模型的工具。保留其 `guard/timeout-policy/` 目录和插件 id `timeout-policy`；`packages/*/tool-*` 目录约定仍只适用于注册工具的包。 |
...
| `InvariantService` | `InvariantRegistry` | 该对象拥有已注册的不变量检查。保留 `@forgeweaver/fw-invariants` 和 `ctx.invariants`。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `util/paths/`, `@forgeweaver/fw-paths` | `util/home-paths/`, `@forgeweaver/fw-home-paths` | 这些辅助函数解析 Harness 主目录下的路径，并非通用路径库。已准确说明返回路径的函数名保持不变。 |
| `util/retention/`, `@forgeweaver/fw-retention` | `util/output-retention/`, `@forgeweaver/fw-output-retention` | 该策略保留命令和工具输出，而不是通用数据保留框架。 |
| `E2BSandboxService` | `E2BRuntime` | 该类创建、复用和释放文件系统与子进程适配器所使用的 E2B 执行环境。它比单个沙箱句柄的职责更广，又比通用所有者更具体。保留 `@forgeweaver/fw-e2b`、`ctx.e2b` 和 `e2b/` 组。 |
| `@forgeweaver/fw-frontend-static` | `@forgeweaver/fw-host-frontend-static` | 该包是提供前端资源的 Host 插件。此前缀可将它与前端应用代码区分开。 |
...
| `@forgeweaver/fw-jsonrpc-demo` | `@forgeweaver/fw-sdk-jsonrpc-demo` | 该示例演示通过 JSON-RPC 使用运行时 SDK，属于 SDK 的唯一含义。 |
| `@forgeweaver/fw-frontend` | `@forgeweaver/fw-web-frontend` | 该应用是 Web 前端。保留其物理目录 `apps/web/`。 |

保留 atomic-write、brand、native-command、timeout 实用工具、目录选择器、`fw-base`、`fw-web-app`、应用启动、CLI（命令行界面）名称，以及 `headless` 包、组合包和示例身份。`headless` 是预期的产品本质，未来也可以支持不止一次性执行。
...
| `@forgeweaver/fw-client-ui-model` | `@forgeweaver/fw-client-ui-model-selection` | 该包控制会话的模型选择。单数 `model` 名称作用域过宽。 |
...
| `@forgeweaver/fw-client-ui-models` | `@forgeweaver/fw-client-ui-settings-models` | 该包拥有 Models 设置面板。保留 `ModelsSettingsStore`；它保存一个具有数据操作和订阅能力的设置视图模型，确实是存储。 |
| `@forgeweaver/fw-client-ui-plugin-config`、`client/ui-plugin-config/` | `@forgeweaver/fw-client-ui-settings-plugins`、`client/ui-settings-plugins/` | 该包拥有 Plugins 设置分区，而不是通用的插件配置系统。目标名称归入 `ui-settings-*` 系列，并采用该分区的复数产品名。 |
...
| `@forgeweaver/fw-client-ui-plugins`、`client/ui-plugins/`、Loader id `ui-plugins`、`client-ui-plugins-invariant` | `@forgeweaver/fw-client-ui-settings-plugin-inventory`、`client/ui-settings-plugin-inventory/`、Loader id `ui-settings-plugin-inventory`、`client-ui-settings-plugin-inventory-invariant` | 这个后来加入的包拥有 Plugins 设置分区中的只读 Plugin Inventory tab。`ui-plugins` 作用域过宽，也无法将该清单与可编辑插件设置区分开。 |
...
| `@forgeweaver/fw-client-ui-feedback`、`client/ui-feedback/`、Loader id `ui-feedback`、`client-ui-feedback-invariant` | `@forgeweaver/fw-client-ui-message-feedback`、`client/ui-message-feedback/`、Loader id `ui-message-feedback`、`client-ui-message-feedback-invariant` | 这个包通过 `messageFeedback` Remote 展示 assistant 消息的评分和说明。旧名称看起来还涵盖 command feedback 和以后可能出现的其他反馈界面，但实际并非如此。 |
...
| `@forgeweaver/fw-client-ui-question` | `@forgeweaver/fw-client-ui-user-questions` | UI 呈现用户问题 seam，而不是任意问题领域。 |
| `@forgeweaver/fw-client-ui-command`, `ui-command/` | `@forgeweaver/fw-client-ui-commands`, `ui-commands/` | 该包呈现并运行一组命令。 |
| `@forgeweaver/fw-client-ui-directory-picker`、`client/ui-directory-picker/`、Loader id `ui-directory-picker`、`client-ui-directory-picker-invariant` | `@forgeweaver/fw-client-ui-directory-picker-browse`、`client/ui-directory-picker-browse/`、Loader id `ui-directory-picker-browse`、`client-ui-directory-picker-browse-invariant` | 客户端包现已拆成 `browse` 和 `native` 两种目录选择器呈现。未加限定词的包实际只是 browse 实现，并非两者的共同定义。目标名称与 Host 后端系列一致，不改变边界。 |
...
- 保留 `@forgeweaver/fw-api-gateway`、`ctx.typertGateway` 和 `TypertGatewayService`。
- 保留会话投影名称。投影并不只是归约函数。
- 保留 `@forgeweaver/fw-session-stats`、`sessionStats` 和 `SessionStatsProjection`。这些名称准确表示全会话统计数据及承载它们的持续维护读模型。
...
- 保留 `fw-subagent-fw-sdk` 和提供方 id `fw-sdk`；重复的限定词可避免歧义。
...
- 保留 `@forgeweaver/fw-client-ui-directory-picker-native`；其后缀说明它是在重命名后的 `-browse` 变体旁使用原生选择器的呈现。保留 `SURFACE_PACKAGES`；在目录选择器自动选择器中，它是客户端呈现端面的包映射，并与 `BACKEND_PACKAGES` 对照。
- 保留 `@forgeweaver/fw-host-plugin-inventory`、`ctx.pluginInventory`、`pluginInventory/list` Remote 以及 `PluginInventory*` 载荷类型。它们准确命名由 Host 拥有的只读清单；只有适配器类和作用域过宽的客户端呈现名称需要修改。
...
- 保留 `@forgeweaver/fw-message-feedback` 能力、`messageFeedback` Remote、assistant-action entry id `feedback`、hook key `feedback` 和 locale namespace `feedback`。它们所在的接口已经把作用域限定为消息反馈或本地 assistant-message slot。只修改作用域过宽的 Client 包名和导出的 UI 名称。
...
**将 `fw-compact-basic` 重命名为 `fw-compaction-llm`。**不予采纳。`LLM` 没有在当前后端系列中增加区别。`basic` 意图更克制，也不会声称存在一个实际并不存在的算法。
...
- 配对的包创建指南包含职责词约定，`packages/AGENTS.md` 链接到该约定，术语表记录选定用词和 `Typert` 拼写，根项目文案将产品称为 ForgeWeaver，而不是 ForgeWeaver SDK。
