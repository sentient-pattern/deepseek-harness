...
- The served rendering of the manifest global changed from `window.__FW_BOOT__ =` to `globalThis["__FW_BOOT__"] =`; no committed snapshot expectation carries that text, so none needed re-recording.
