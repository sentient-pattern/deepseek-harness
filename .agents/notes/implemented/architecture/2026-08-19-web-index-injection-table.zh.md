...
- `__FW_BOOT__` 的 served 渲染文本从 `window.__FW_BOOT__ =` 变为 `globalThis["__FW_BOOT__"] =`；已核实无已提交快照期望含此文本，无需重录。
