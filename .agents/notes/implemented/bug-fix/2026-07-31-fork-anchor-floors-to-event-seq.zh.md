...
`SessionRuntime.fork` 在发起 RPC 前对 `atSeq` 向下取整。分数 seq 这个约定属于 `fw-client-runtime`，实时投影和回放投影都由它生成，因此也由同一个包在跨出 wire 边界时把它换回真实事件 seq，而不是要求每个 UI 调用方各自记得转换。整数锚点不受影响。
