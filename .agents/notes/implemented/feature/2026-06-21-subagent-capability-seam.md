...
> The full seam is shipped: the `fw-subagent` interface and `fw-tool-subagent` consumer; the two in-process backends (`fw-subagent-spawn-in-process`, `fw-subagent-fork-in-process`); the nested-agent snapshot infrastructure ([per-session snapshot replay](../testing/2026-06-22-subagent-snapshot-replay.md)); and the out-of-process ACP, Codex, and Claude Code backends ([ACP Agent Note](2026-06-22-acp-subagent-backend.md), [product-provider Agent Note](2026-08-04-claude-code-and-codex-subagent-backends.md)).
...
| Package | Role |
|---|---|
| `@forgeweaver/fw-subagent` | interface: `SubagentRuntime` (`ctx.subagents`), `SubagentProvider`, `SubagentRun`, the request/result/capability vocabulary, the `subagent/*` events |
| `@forgeweaver/fw-subagent-spawn-in-process` | implementation: a fresh in-process child via `ctx.agents.create` |
| `@forgeweaver/fw-subagent-fork-in-process` | implementation: an in-process child seeded with a snapshot of the parent's log |
| `@forgeweaver/fw-subagent-acp` | implementation: an ACP client driving a configured child process |
| `@forgeweaver/fw-subagent-codex` | implementation: a one-shot official Codex app-server process |
| `@forgeweaver/fw-subagent-claude-code` | implementation: a one-shot official Claude Code process through the Agent SDK |
| `@forgeweaver/fw-tool-subagent` | consumer: the model-facing `subagent` tool over `ctx.subagents` |
...
`fw-tool-subagent` passes its execution signal to `start()`, awaits the child result, and disposes the run before reporting. Non-completed outcomes become error results rather than successful partial output; they present the optional safe diagnostic owned by the [non-interactive permissions decision](2026-08-15-product-subagent-noninteractive-permissions.md) separately from partial assistant text. Independent result and disposal rejections remain independently observable.

### Provider selection is config, not model-facing

`fw-tool-subagent` binds to exactly one provider name (`Config.provider`); the model sees only `{ description, prompt }`. To expose more than one transport, load the tool plugin more than once, each bound to a different provider and a distinct `toolName` (the tool registry rejects a duplicate name). The *service* holds the multi-provider registry; the *tool* picks one — the schema carries no provider/type parameter.
