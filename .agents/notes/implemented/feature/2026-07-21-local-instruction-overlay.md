...
The fixed user-global `$FW_HOME/AGENTS.md` has no local overlay and stays base-only.
...
**Extend the overlay to the user-global scope.** Deferred: `$FW_HOME` is a single fixed `AGENTS.md` with no committed base to supplement, so it stays base-only until a concrete need appears.
