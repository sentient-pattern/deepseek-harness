...
`@forgeweaver/fw-tmux-context` 是位于 `packages/context/tmux-context/` 的可选启用型函数插件，与其他既不定义工具也不定义服务的有界请求上下文增强并列。已交付的 TUI 会挂载它，因为终端复用器上下文是该界面特有的；`fw-agent-spine-demo` 与 Web／无头界面保持沉默。
