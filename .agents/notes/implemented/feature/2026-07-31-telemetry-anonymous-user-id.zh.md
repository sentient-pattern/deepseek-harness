# Agent Note: 遥测匿名用户 id（$FW_HOME/.anonymous-user-id）与 OTel Resource 的 user.id
...
`getOrCreateAnonymousUserId()` 返回 `$FW_HOME/.anonymous-user-id`（`resolveDshHome` 解析，`$FW_HOME` > `~/.fw`）中的裸 UUID 行，首用生成随机 UUID v4 并落盘；后端构造时把它作为 Resource 的 `user.id`（OTel semconv 标准用户属性）随每批导出携带一次。原始实现位于 `session-telemetry-otel`，因为当时不存在第二个真实消费方。`/feedback` 后来成为该消费方，因此[共享 id 决策](../architecture/2026-08-07-shared-feedback-telemetry-user-id.zh.md)将所有权移交给 `@forgeweaver/fw-anonymous-user-id`，但不改变本 Note 记录的存储、匿名、并发与丢失语义。[直连 ForgeWeaver 请求身份](2026-08-11-forgeweaver-request-user-id-header.zh.md)是同一 id 的第三个消费方。
...
| 落点 | `@forgeweaver/fw-anonymous-user-id`，由 OTel 后端、`/feedback` 与直连 ForgeWeaver 请求共享 | 消费方共用同一存储契约，且不依赖导出后端 |
| 单独开关 | 无 | 任一消费方都可创建该身份；`FW_TELEMETRY_DISABLED` 会停止遥测上报，但不会禁用反馈确认或 ForgeWeaver 请求头 |
...
| 挂进 `@forgeweaver/fw-home-paths` | paths 是纯路径计算零 IO；带持久化的身份能力会污染包边界 |

## 后果

- 一个 `$FW_HOME` 在 OTel 回流中是一个稳定用户；不同 home 在构造上就是不同用户，无跨 home 关联机制。
- OTel 回流、`/feedback` 与直连 ForgeWeaver 请求共享 `.anonymous-user-id`。
