# Agent Note: Default session-telemetry mount (OTel reporting) in the fw web composition
...
| Sharing mode | `FW_TELEMETRY_MODE`, default `DISABLED`; explicit `FULL` or `FEEDBACK_ONLY` opts in | A fresh profile makes no telemetry network request, while internal deployments retain both upload policies |
| Endpoint | `FW_TELEMETRY_OTLP_URL`, default `https://harness-telemetry.forgeweaversvc.com/v1/logs` | Internal collector; the env override serves local/dev runs |
| Hard opt-out | any non-empty `FW_TELEMETRY_DISABLED` (including `0`/`false`) disables the row | The launcher patch takes effect before load-time transport validation and overrides every configured mode |
...
| CI isolation | top-level `env: FW_TELEMETRY_DISABLED: '1'` in GitHub workflows | Defense in depth keeps test sessions local even when a job explicitly selects an uploading mode |
...
- A developer running `fw web` without telemetry configuration makes no telemetry network request. An internal deployment sets `FW_TELEMETRY_MODE` and may point `FW_TELEMETRY_OTLP_URL` at another collector.
