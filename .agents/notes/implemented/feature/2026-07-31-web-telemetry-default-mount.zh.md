# Agent Note: fw web 组合默认挂载会话遥测（OTel 上报）
...
| CI 隔离 | GitHub 工作流顶层 `env: FW_TELEMETRY_DISABLED: '1'` | 即使 CI 任务显式选择上传模式，纵深防御也会让测试会话留在本地 |
...
- 开发者运行没有遥测配置的 `fw web` 时，不会发出遥测网络请求。内部部署需设置 `FW_TELEMETRY_MODE`，并可让 `FW_TELEMETRY_OTLP_URL` 指向其他 collector。
