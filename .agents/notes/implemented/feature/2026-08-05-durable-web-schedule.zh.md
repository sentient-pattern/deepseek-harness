...
[`examples/web-schedule`](../../../../examples/web-schedule/README.zh.md) overlay 显式加载 `@forgeweaver/fw-time-context` 与 `@forgeweaver/fw-schedule`；默认 Web 配置树保持不变。Schedule 只观察插件加载后发布的根 Agent，并在该 Agent scope 中安装三个工具和一个可丢弃 owner。cold history 读取、已发布的根、child Agent 与其他 host 都不会激活它。
