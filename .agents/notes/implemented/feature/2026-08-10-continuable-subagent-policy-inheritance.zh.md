...
- `fw-subagent` 新增针对 `fw-sandbox-policy` 与 `fw-user-approval` 的可选 peer 类型（即一次性驱动器所用的 `ctx.get` 模式）；`fw-subagent-in-process-driver` 完全移除自己的策略服务 peer 与类型导入，委托给共享辅助函数。
...
- 进程外提供方（`acp`、`fw-sdk`、`claude-code`、`codex`）不支持可继续子 agent（没有 `prepareContinuable`），其一次性子 agent 保留自身的部署策略（`inheritsParentContext = false`）；跨进程策略传播仍不在范围内。
