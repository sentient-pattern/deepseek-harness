...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `fw-tool-bash` (`bash`), `fw-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `fw-tool-subagent` (`subagent`). Example-only tools are excluded.
