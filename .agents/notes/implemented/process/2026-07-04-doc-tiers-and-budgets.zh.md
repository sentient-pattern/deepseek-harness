...
- **单一产品入门路径。**根 README 负责推荐的包运行路径、从源码运行的备选路径和简要的 `fw plugin --profile` 用法。已发布的用户指南从运行中的 Web UI 内部任务开始，再链接到其他界面的独立教程或插件开发与进阶配置的参考文档归属处，而不会重复介绍 Web 启动步骤。
...
- **精简的工作流 skill（技能），约定归文档。**[.agents/skills/fw-doc-standards](../../../skills/fw-doc-standards/SKILL.md) 承载文档放置、审计和门禁失败处理工作流，并以文档标准为真源，与 [fw-translate-docs](../../../skills/fw-translate-docs/SKILL.md) 和 i18n 约定之间的分工相同。
