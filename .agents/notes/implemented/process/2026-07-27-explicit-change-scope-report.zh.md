...
[pre-push 工作流](../../../skills/fw-pre-push-checks/SKILL.md)需要取得相对于实际基准的 diff，但按 `origin/<current-branch>` 构造引用存在两类问题：对于第一次推送前跟踪 `origin/master`、尚无同名远端分支的新 worktree 分支，该引用无法解析；对于 PR（Pull Request）以另一功能分支为基准的堆叠分支，该引用会错误描述基准。[代码评审](../../../skills/fw-code-review/SKILL.md)与[文档审计](../../../skills/fw-doc-standards/SKILL.md)工作流同样需要判断当前基准。
