...
[三条发布序列](2026-08-10-npm-release-sequences.zh.md)交付时带的是 `publishConfig.access: restricted`,因此发到 `@forgeweaver` scope 的每个包只在组织内可见。五次排练发布都是这样跑的:`fw@0.0.1-rc.5`、vendor 的 `*-rc.4`、`landlock-run@0.0.1`。

真正卡住公开消费者的是**受限的依赖**。每个 harness 包都把 vendored 框架声明成 `peerDependency`,`fw-sandbox-local` 把 Landlock 入口声明成 `dependency`。一个公开包若要求一个受限包,组织外的人根本装不上;所以这两条序列必须先公开,fw 族才可能公开 —— 而在 fw 族仍受限期间,它们也正是外部消费者唯一需要解析到的两条。
...
| fw | `packages/*/*` + `apps/*`(221 个成员) | `restricted` |

`check-workspace-constraints.ts` 按各自序列的级别校验每个 manifest,这是阻止 scope 漂移的那道闸:新增的 `vendor/*` 包留在 `restricted`、或某个 fw 成员被改成 `public`,都会让 workspace 约束失败。
...
**一次性把整个 scope 改成 public。** 暂不采用:那会让下一次 fw 发布因为一次 manifest 改动而顺带变成公开,而不是出自一个刻意的发布决定。先公开这两条依赖序列,是能让每一步的已发布包都保持可安装的顺序,也是将来决定公开 fw 时的前置条件。
...
- **`@forgeweaver/fw` 仍然装不了(组织外)。** 它的 manifest 保持 `restricted`;变化的是它已发布的依赖不再受限,所以将来公开它是一个版本决定,而不再是依赖问题。
