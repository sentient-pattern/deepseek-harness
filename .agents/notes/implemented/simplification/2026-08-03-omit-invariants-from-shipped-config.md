# Agent Note: Omit runtime invariants from shipped fw config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@forgeweaver/fw-*/invariant` entry.
...
- Ordinary `fw` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
