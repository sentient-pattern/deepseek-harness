# Agent Note: 从交付的 fw 配置中省略运行时不变式
...
`@forgeweaver/fw-invariants` 与各包拥有的 `./invariant` 伴随插件是可选的开发诊断。交付的 TUI 挂载了该服务和四个有状态伴随插件，而交付的 Web 配置树省略了这些条目，导致两个产品 surface 的诊断成本和失败行为不同。即使始终启用的产品边界仍负责会话验证与不可变历史，关系断言失败也可能终止普通的 TUI 运行。

## 决策

`apps/cli/config/` 下交付的 `fw` 配置树既不挂载 `@forgeweaver/fw-invariants`，也不挂载任何包拥有的 `./invariant` 伴随插件。因此，CLI 包不再直接依赖不变式服务。
...
构建后 CLI 的配置转储测试会检查两个交付的 surface，并拒绝服务条目或任何 `@forgeweaver/fw-*/invariant` 条目。
...
- 普通的 `fw` TUI 与 Web 运行不安装不变式监听器或 trace 状态，也不会因 `InvariantError` 失败。
