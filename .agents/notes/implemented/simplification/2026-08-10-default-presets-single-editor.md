...
The `standard`, `code`, and `cordis` preset configurations mount `fw-tool-fs` and `fw-tool-fs-search`, but do not mount `fw-tool-str-replace-editor`. Code Mode therefore omits `str_replace_editor` from both its registry and generated SDK. The `minimal` preset continues to mount `fw-tool-str-replace-editor`, and deployments or user-authored presets may still mount the plugin explicitly.
