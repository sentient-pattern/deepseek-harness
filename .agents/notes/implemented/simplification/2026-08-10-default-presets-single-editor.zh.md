...
`standard`、`code` 和 `cordis` preset 配置挂载 `fw-tool-fs` 与 `fw-tool-fs-search`，但不挂载 `fw-tool-str-replace-editor`。因此 Code Mode 的注册表和生成的 SDK 均不包含 `str_replace_editor`。`minimal` preset 继续挂载 `fw-tool-str-replace-editor`，部署配置或用户自定义 preset 仍可显式挂载该插件。
