...
- **六个 merge-extensible map**（约 370 行核心类型）：`ContentBlockMap`、`MessageSourceMap`、`FinishReasonMap`（位于 `fw-llm`）；`TurnTriggerMap`、`TurnEndReasonMap`、`SessionEventMap`（位于 `fw-session`）。
- **约 10 处 `declare module` 声明增补位置**，分布在 `fw-agent`、`fw-agent-loop`、`fw-shell`、`fw-llm`、`fw-session`、`fw-session-persistence`、`fw-system-prompt`、`fw-tools` 各包中——每处都将从声明合并改为运行时 `register()` 调用。
- **事件生产者**——agent loop（智能体循环）中 16 处 `session.append(...)` 调用——形状不变，但现在在边界处被校验。
- **约 7 个 switch 消费方**，对这些联合类型进行分支：`deriveMessages` 与包自有的不变式 companion（`fw-session`）、`BlockAssembler`（`fw-llm`）、两个 LLM（大语言模型）适配器（`fw-llm-forgeweaver`、`fw-llm-pi-ai`）以及工具 schema 层（`fw-tools`）。`assertNever` 对封闭联合类型的穷举 vs 对可扩展联合类型的 fall-through 约定（一条已记录的 lint 规则）需要重新考量——运行时变体在静态层面不可穷举。
- **`defineTool` 的 `InferArgs` DSL**（`fw-tools`），它从编译期 schema 规范派生出零类型转换的 `execute` 参数类型——这是当前方案的标杆用例。
