...
| --- | --- | --- | --- |
| `@forgeweaver/fw-storage` | `packages/storage/storage/` | `ctx.storage` (the hub) | ✓ |
| `@forgeweaver/fw-storage-json` | `packages/storage/storage-json/` | registers backend `json` | ✓ |
| `@forgeweaver/fw-storage-sqlite` | `packages/storage/storage-sqlite/` | registers backend `sqlite` | ✓ |
| `@forgeweaver/fw-storage-domain` | `packages/storage/storage-domain/` | mounts `ctx.storage.domain` | ✓ |
| `@forgeweaver/fw-workspace` | `packages/workspace/workspace/` | `ctx.workspaceRegistry` | ✓ |
...
(workspace lives in its own group rather than `packages/host/`: the host group's naming rule requires the `fw-host-*` prefix while this package is named `fw-workspace`; and the workspace entity is a domain concept, not bound to the host assembly tier. Unrelated to the existing `agent-instructions` package — that is an AGENTS.md instruction loader.)

Dependency direction: `fw-workspace` → `fw-domain` → `fw-storage` ← the two backends. `fw-workspace` additionally depends on the read-only face of `ctx.sessionPersistence` (attach's cwd check reads the session header; when the service is absent, attach rejects outright — no verification, no bookkeeping). The `ctx.sessions` running-check for session deletion moves into future work together with the cascade.

### `fw-storage`: the storage hub

A pure registration hub, no IO of its own, no Config. The `Storage` service mounts at `ctx.storage` with two faces: `backend` (a `BackendRegistry`: `register(name, backend)` returns the disposer, duplicate names throw; `get(name)` throws `backend-not-found` for unknown names) and data-form mounting (`mount(form, facility)` over the merge-extensible `StorageForms` map, into which `fw-domain` merges the `domain` key; unmounted access throws `form-not-mounted`). The signature text lives in `packages/storage/storage/src/index.ts` and `src/registry.ts`.

**Multiple backends stay mounted side by side**; which backend serves a domain is `fw-domain`'s configuration (below), never a global either-or. Disposer semantics = remove the name from the table; closing the backend itself belongs to the backend package's effect closure, unregister first then close.
...
### `fw-domain`: the domain data form
...
| --- | --- | --- |
| JSONL: temp write + fsync + link/unlink atomic publish, 0o700/0o600 permissions, Windows variant (win32.ts) | pure medium | copied by `fw-storage-json` this phase (whole-file atomic rewrite is the same protocol); becomes the shared implementation at migration |
...
| SQLite: openDatabase (mkdir/exclusive create/PRAGMA sequence/user_version check) | pure medium | copied by `fw-storage-sqlite` this phase — the two openDatabase copies are already near line-identical and this group is the third user; copy now, extract at migration |
