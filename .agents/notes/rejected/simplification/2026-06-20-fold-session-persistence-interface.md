# Agent Note: Fold the persistence interface into fw-session

Status: rejected — the separate persistence Service Definition package is the intended modular role split for the durable-persistence capability seam. Folding it into `fw-session` would reduce package count at the cost of a cleaner backend boundary.
...
- `@forgeweaver/fw-session-persistence` is removed as a package.
- `fw-session` exports the persistence service type, coordinator, and contract helpers.
- JSONL and SQLite backend packages depend on `fw-session` directly.
