...
Compaction is split between `@forgeweaver/fw-compaction`, which owns an abstract two-method service and shared types, and `@forgeweaver/fw-compaction-basic`, which owns the only complete provider. Shipped configurations load only the basic package, and no production package independently consumes the Service Definition package except that provider.
...
Move the basic implementation into `@forgeweaver/fw-compaction` and remove `@forgeweaver/fw-compaction-basic`. Keep `ctx.compaction`, `CompactionResult`, the shared transcript and tool-pairing helpers, the existing configuration, and the concrete compaction algorithm in one package.
...
- `@forgeweaver/fw-compaction-basic` and its workspace/package metadata are removed.
- `@forgeweaver/fw-compaction` owns the current configuration, plugin class, algorithm, types, events, and shared helpers.
