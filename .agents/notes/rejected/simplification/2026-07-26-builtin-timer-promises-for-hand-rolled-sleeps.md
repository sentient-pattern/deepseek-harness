...
Three packages hand-roll promise-wrapped timers that the `node:timers/promises` builtin already provides, while other packages (`fw-llm-mock-server` `pause()`, `fw-lsp-stdio`, `fw-acp-snapshot`) already use the builtin — so the hand-rolled copies are also a consistency gap:
