/**
 * `fw plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `fw.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `fw.bundle` joins the layer stack; a
 ...
 * gained its `fw.bundle` declaration in a newer version.
 * @module @forgeweaver/fw/plugin
 */
 ...
 * @returns true when the package manifest declares `fw.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.fw?.bundle?.patch !== undefined
}

/**
 * Reconcile `fw.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `fw.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.fw?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no fw.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (fw-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.fw = { ...after.fw, profile: { ...after.fw?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `fw` was invoked from.
...
/**
 * Run one `fw plugin` invocation: init if needed, forward to pnpm, reconcile.
