...
export default async function open(url) {
  ...
  console.log(`fw browser-open: ${JSON.stringify({
    url,
    status: response.status,
    bootManifest: html.includes('__FW_BOOT__'),
    apiKeyPresent: process.env.FORGEWEAVER_API_KEY !== undefined,
    fwHomePresent: process.env.FW_HOME !== undefined,
  })}`)
  ...
  const launcher = spawn(process.execPath, [
    '--eval', handoffProbe,
    '--', join(process.cwd(), `.fw-browser-open-${process.ppid}`), String(process.pid),
  ], { stdio: 'ignore' })
  launcher.unref()
  return launcher
}
