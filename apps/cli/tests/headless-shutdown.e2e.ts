...
import { LOADER_SMOKE_TEST_TIMEOUT_MS, resolveExampleLaunch } from '@forgeweaver/fw-loader-smoke'

const fwBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"fw-test: never-dispose ready", b"fw-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'fw-headless-shutdown-'))
  try {
    const home = join(cwd, '.fw')
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'fw-profile-headless',
      private: true,
      dependencies: {},
      fw: { profile: { bundles: ['@forgeweaver/fw-base', '@forgeweaver/fw-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        FORGEWEAVER_API_KEY: 'keyless-shutdown-no-call',
        FW_TELEMETRY_DISABLED: '1',
        FW_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`fw headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`fw headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('fw: observing at ')
    expect(output).toContain('fw-test: never-dispose ready')
    expect(output).toContain('fw-test: never-dispose started')
