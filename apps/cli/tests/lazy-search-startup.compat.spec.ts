 ...
 * web profile (fw-base + fw-web-app bundle patches, auto-initialized).
...
const requireBuiltArtifacts = process.env.FW_REQUIRE_BUILT_CLI_SMOKE === '1'
...
function runBuiltWeb(cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveRun, rejectRun) => {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      FORGEWEAVER_API_KEY: 'fw-cli-smoke-dummy-key',
      FW_HOME: join(cwd, '.fw'),
    }
    delete env.FORGEWEAVER_BASE_URL
    ...
    child.stdout.on('data', (chunk: string) => {
      stdout += chunk
      if (!settled && /fw web: http:\/\/127\.0\.0\.1:\d+/u.test(stdout)) {
        settled = true
        child.kill('SIGTERM')
      }
    })
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search off by default', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'fw-cli-lazy-search-'))
    try {
      const result = await runBuiltWeb(cwd)
      expect(result.stdout).toMatch(/fw web: http:\/\/127\.0\.0\.1:\d+/u)
