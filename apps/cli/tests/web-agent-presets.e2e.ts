...
import { boot, healProfilesModuleFallback, loadOverlayPatches, loadProfile } from '@forgeweaver/fw-app-boot'
...
import { resolveSessionPreset, SETTINGS_NAMESPACE } from '@forgeweaver/fw-agent-presets'
import { applyChildComposition, childSessionMeta } from '@forgeweaver/fw-subagent'
import { CallId } from '@forgeweaver/fw-llm'
import type {} from '@forgeweaver/fw-compaction-basic'
...
import type {} from '@forgeweaver/fw-token-meter'

const CONFIG_DIR = fileURLToPath(new URL('../config/', import.meta.url))
const REPO_ROOT = fileURLToPath(new URL('../../..', import.meta.url))
/** The shipped Web surface: the fw-base and fw-web-app bundle patches over an empty preset root. */
...
async function bootWeb(
  ...
  const overrides: PatchOptions[] = [
    // The settings row defaults to `$FW_HOME/settings.yaml`. Left alone it
    ...
    // storage-json's root is anchored to the real $FW_HOME. Unpinned, this
    // file writes the developer's own `~/.fw/storages/` — and then reads it
    ...
    // developer's own `~/.fw/.preset` cannot change this test's outcome.
  ...
  // Product Bundles are installed into the Profile, not the fw app. Model
  ...
  let bundlePatches: PatchOptions[] = [
    ...loadOverlayPatches('fw-test', BASE_PATCH),
    ...loadOverlayPatches('fw-test', WEB_PATCH),
  ]
  if (profileBundles !== undefined) {
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      private: true,
      dependencies: Object.fromEntries(profileBundles.map(name => [name, 'workspace:*'])),
      fw: { profile: { bundles: profileBundles } },
    }, null, 2) + '\n')
    const profile = loadProfile('fw-test', 'spec', INSTALL_ANCHOR, home, { userLayer: false })
    bundlePatches = profile.layers.flatMap(layer => layer.patches)
  }
  const rootConfig = join(profileDir, 'cordis.yml')
  await writeFile(rootConfig, '[]\n')
  return await boot('fw-test', rootConfig, [...bundlePatches, ...overrides], (bootCtx) => {
...
let ctx: Context
beforeAll(async () => {
  const settingsFile = join(await mkdtemp(join(tmpdir(), 'fw-web-presets-')), 'settings.yaml')
...
describe('the shipped Web composition', () => {
  ...
  it('merges the global skill layer into a preset agent\'s catalog, keeping local discovery preset-side', async () => {
    const proj = await mkdtemp(join(tmpdir(), 'fw-preset-skill-proj-'))
    await mkdir(join(proj, '.fw', 'skills', 'project-proof'), { recursive: true })
    await writeFile(join(proj, '.fw', 'skills', 'project-proof', 'SKILL.md'), [
      '---',
      'name: project-proof',
    ...
    try {
      ...
      expect((await ctx.skills.list({ cwd: proj })).map(skill => skill.name)).toEqual(['fw-badge'])
      ...
      expect(scoped).toContain('fw-badge')
      ...
      expect(JSON.stringify(loaded.content)).toContain('powered by fw')
    } finally {
      await handle.dispose()
    }
  })
  ...
  it('shows a minimal agent the global layer but no loader tool', async () => {
    ...
    try {
      ...
      expect((await ctx.skills.list({ scope: handle.agent })).map(skill => skill.name)).toContain('fw-badge')
...
describe('product Bundle and user-preset intersection', () => {
  ...
  async function bootProducts(installed: readonly Product[]): Promise<Context> {
    const root = await mkdtemp(join(tmpdir(), 'fw-product-presets-'))
...
describe('a launcher that configures no writable root', () => {
  ...
  // roster derives `<fwHome>/.agent-presets` itself. `$FW_HOME` is pointed
  ...
  beforeAll(async () => {
    const home = await mkdtemp(join(tmpdir(), 'fw-preset-derived-'))
    previousHome = process.env.FW_HOME
    process.env.FW_HOME = home
    ...
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'fw-preset-derived-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
  ...
  afterAll(async () => {
    if (previousHome === undefined) delete process.env.FW_HOME
    else process.env.FW_HOME = previousHome
...
describe('authoring a preset on the shipped composition', () => {
  ...
  beforeAll(async () => {
    userRoot = join(await mkdtemp(join(tmpdir(), 'fw-preset-authoring-')), 'profiles')
    const settingsFile = join(await mkdtemp(join(tmpdir(), 'fw-preset-authoring-settings-')), 'settings.yaml')
    await writeFile(settingsFile, '{}\n')
