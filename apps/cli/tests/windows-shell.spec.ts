 ...
 * the REAL shipped bundle layers (fw-base + fw-web-app resolved from the
...
import { entryListSchema } from '@forgeweaver/cordis-plugin-include'
import { evaluate } from '@forgeweaver/cordis-plugin-loader'
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@forgeweaver/fw-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL fw-base/fw-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'fw-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@forgeweaver/fw-base', '@forgeweaver/fw-web-app'])
    const profile = loadProfile('fw', 'web', anchor, home)
    ...
    for (const name of ['@forgeweaver/fw-pwsh-sandbox', '@forgeweaver/fw-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'fw-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@forgeweaver/fw-base'])
    const profile = loadProfile('fw', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
