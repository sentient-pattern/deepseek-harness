...
the deliberate composition divergences from `fw web` — are documented in
...
## Do not import `@forgeweaver/fw-client-*` here
...
`@forgeweaver/fw-goal/remote`, so the Host build phase ended up waiting on an
...
it imports `AppWebEntry` from `@forgeweaver/fw-client-web` and the boot-manifest
type from `@forgeweaver/fw-client-modules/client`: booting the real shell is what
...
`@forgeweaver/fw-client-runtime/client` because `client/runtime` is reachable
