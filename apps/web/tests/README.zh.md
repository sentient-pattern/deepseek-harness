...
的运行机制——模式、fixture、golden，以及与 `fw web` 之间刻意保留的组合差异——记录在
...
`@forgeweaver/fw-client-web` import `AppWebEntry`、从
`@forgeweaver/fw-client-modules/client` import boot manifest 类型：启动真实 shell 正是该
harness 的用途，且这两个包本来就在 Host 图中。另外，chat 场景从
`@forgeweaver/fw-client-runtime/client` import `conversationContextKey`，因为
