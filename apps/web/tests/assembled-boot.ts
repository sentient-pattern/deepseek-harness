...
import { bootInjections, orderByModuleGraph } from '@forgeweaver/fw-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@forgeweaver/fw-client-modules/client'
import { AppWebEntry } from '@forgeweaver/fw-client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@forgeweaver/fw-app-boot')).href) as unknown as BootComposition
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares fw.client without a ./client export`)
  }
  return resolve(dirname(packagePath), relative)
}

/** Derive the assembled browser graph from the same bundle patches and package declarations as `fw web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.fw?.client
...
interface FixtureWindow extends Window {
  __FW_BOOT__?: { rev: string; entries: WebBootEntry[] }
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  win.__FW_BOOT__ = { rev: 'fx', entries: PLUGINS.map(({ bundlePath: _bundlePath, ...plugin }) => plugin) }
  const [facadeRow] = bootInjections(win.__FW_BOOT__)
  ...
  for (const id of ['@forgeweaver/fw-client-modules', '@forgeweaver/fw-client-runtime']) {
 ...
 * the snapshot gate's `FW_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.FW_SNAPSHOT === 'record' || process.env.FW_SNAPSHOT === 'refresh'
