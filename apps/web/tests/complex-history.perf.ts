...
import type { ReplayEntry, ReplayOverrideDoc } from '@forgeweaver/fw-llm-replay'
...
function appendRequestHeader(session: Session, turn: number, step: number): void {
  session.append('request/header', {
    header: {
      config: { provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash' },
...
async function startMutationProbe(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    Reflect.set(globalThis, '__fwPerfMutationProbe', probe)
  })
}

async function stopMutationProbe(page: Page): Promise<MutationProbeResult> {
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__fwPerfMutationProbe') as
    ...
    Reflect.deleteProperty(globalThis, '__fwPerfMutationProbe')
...
async function startUserRenderProbe(
  ...
  await page.evaluate((expectedMarker) => {
    ...
    Reflect.set(globalThis, '__fwPerfUserRenderProbe', probe)
  }, marker)
  await send.evaluate((button) => {
    button.addEventListener('click', (event) => {
      const probe = Reflect.get(globalThis, '__fwPerfUserRenderProbe') as
...
async function stopUserRenderProbe(
  ...
  return page.evaluate(() => {
    const probe = Reflect.get(globalThis, '__fwPerfUserRenderProbe') as
      ...
      | undefined
    Reflect.deleteProperty(globalThis, '__fwPerfUserRenderProbe')
    if (
...
async function launchPerformanceWorld(
  ...
  try {
    if (options.replay === undefined) {
      ...
      replayDir = await mkdtemp(join(tmpdir(), 'fw-web-perf-replay-'))
