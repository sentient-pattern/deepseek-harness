/** Published fw web + pnpm dev:web → browser HMR, with no page reload. */
...
import type { SubprocessHandle, SubprocessSpawnSpec } from '@forgeweaver/fw-subprocess'
...
it('hot-reloads a real client-plugin source edit without refreshing the page', async () => {
  const world = await mkdtemp(join(tmpdir(), 'fw-web-hmr-world-'))
  const sourcePath = join(REPO_ROOT, 'packages/client/ui-conversation/src/client/locales.ts')
  const binPath = join(REPO_ROOT, 'apps/cli/lib/bin.js')
  if (!existsSync(binPath)) throw new Error('HMR browser test needs the built fw bin; run pnpm run build first')
  ...
  try {
    ...
    host = subprocessCtx.subprocess.spawn(spawnSpec(
      ...
      world,
      {
        FORGEWEAVER_API_KEY: 'keyless-hmr-no-call',
        FW_HOME: join(world, '.fw'),
      },
    ))
    const baseUrl = await waitForOutput(host, /fw web: (http:\/\/[^\s]+)/, 'built fw web')
    ...
    const pageIdentity = await page.evaluate(() => {
      ...
      Object.defineProperty(window, '__fwHmrPageIdentity', { value: identity })
      return identity
    })

    await writeFile(sourcePath, updatedSource)
    await page.getByText(newText, { exact: true }).waitFor({ timeout: 30_000 })
    expect(await page.evaluate(() => (window as Window & { __fwHmrPageIdentity?: string }).__fwHmrPageIdentity))
      .toBe(pageIdentity)
