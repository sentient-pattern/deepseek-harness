...
// composition and wire. The model adapter is fw-llm-replay with override
...
import type { RetryPolicyConfig } from '@forgeweaver/fw-llm'
import { deriveReplayScript, parseSessionLog } from '@forgeweaver/fw-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'fw-web-e2e-sidecar-'))
