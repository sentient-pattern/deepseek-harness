...
function completedTailFixture(raw: string): string {
  ...
  const tail = [
    ...
    at({ type: 'assistant/message', data: { turn: 2, step: 1, content: [{ type: 'text', text: 'DONE' }], provenance: { provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash' } }, sourceEventSeqs: [], surfaceOp: 'append' }),
