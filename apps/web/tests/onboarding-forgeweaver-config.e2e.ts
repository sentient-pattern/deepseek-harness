// Keyless browser e2e: the shipped ForgeWeaver adapter stays mounted while its
...
const SNAPSHOT_DIR = fileURLToPath(new URL('./snapshots/onboarding-forgeweaver-config', import.meta.url))
...
describe.skipIf(MODE === 'record')('web e2e: first-run ForgeWeaver credential setup', () => {
  ...
  it('stores a key write-only and observes configured state without restarting', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-forgeweaver-config'))
    ...
    const secret = `fw_onboarding_${randomBytes(12).toString('hex')}`
    ...
    expect(stored.includes(`FORGEWEAVER_API_KEY: ${secret}`)).toBe(true)
    ...
    const deepSeekRow = settings.getByText('ForgeWeaver', { exact: true }).first()
  ...
  it('configures arbitrary ForgeWeaver models and prompts after the selected model is removed', async () => {
    onTestFailed(() => saveFailureShot(page, 'web-e2e-onboarding-forgeweaver-models'))
    ...
    await settings.waitFor({ timeout: 10_000 })
    await settings.getByRole('button', { name: '模型' }).click()
    const deepSeek = settings.getByText('ForgeWeaver', { exact: true }).first()
    ...
    expect(document).toContain('id: forgeweaver-v4-pro')
    expect(document).toContain('id: forgeweaver-v4-flash-vision-exp')
    ...
    expect(document).not.toMatch(/^\s*- id: forgeweaver-v4-flash$/m)
    ...
    expect(await page.getByText('forgeweaver-v4-flash', { exact: true }).count()).toBe(0)
    await page.getByRole('menuitemradio', { name: 'ForgeWeaver-V4-Flash-Vision-Exp' }).waitFor({ timeout: 10_000 })
