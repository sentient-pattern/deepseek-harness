...
// in the temp workspace) all run; the model adapter is fw-llm-replay (keyless)
...
// Record: FW_SNAPSHOT=record rewrites session.jsonl, then a keyless
// FW_SNAPSHOT=refresh regenerates ui.expected.md.
...
describe('web e2e: fresh round trip through the real assembly', () => {
  ...
  it.skipIf(MODE === 'record')('matches the conversation aria golden with stable anchors', async () => {
    ...
    await page.getByRole('button', {
      name: 'Select model, current ForgeWeaver-V4-Flash',
