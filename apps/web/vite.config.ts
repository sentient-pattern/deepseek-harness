...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__FW_BOOT__. '
  + 'From a repository checkout, run `pnpm fw web`; an installed package uses `fw web`. '
  + 'For client-plugin HMR, run `pnpm fw web` together with `pnpm run dev:web`.'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.FW_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'fw-client-document-title',
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'fw-reject-standalone-web-serve',
    config(_config, env) {
