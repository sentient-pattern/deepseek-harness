...
如何接入一个新的模型提供方。参考实现：`packages/llm/llm-forgeweaver`（直接 HTTP，SSE（Server-Sent Events）由 `eventsource-parser` 分帧）与 `packages/llm/llm-pi-ai`（封装 LLM 库）。请先阅读 `packages/llm/llm/src/types.ts` 中的 `StreamChunk` 文档——它记录了两个适配器都经过验证的协议约定。
...
让协议格式（wire format）类型、请求序列化、传输解析、分片转换和适配器类分别承担独立职责；[`llm-forgeweaver`](../../packages/llm/llm-forgeweaver/README.zh.md) 是参考布局。
