...
- Optional: a ForgeWeaver API key for the Web, headless, and ACP automation demos and real-API e2e tests.
...
The install also configures worktree-local Lefthook hooks and the `fw-translation-pairing` Git merge driver through `scripts/install-lefthook.mjs`. The [worktree-local hooks Agent Note](../.agents/notes/implemented/process/2026-07-27-worktree-local-lefthook.md) owns the hook-path safety contract; the [automatic pairing merges Agent Note](../.agents/notes/implemented/process/2026-08-08-automatic-translation-pairing-merges.md) owns the merge driver.
...
The real ForgeWeaver adapter and key-backed agent demos read credentials from the environment or from a gitignored `.env` at the repo root:
...
`FORGEWEAVER_BASE_URL` is optional and defaults to the public API. Never commit real credentials. The real-API e2e suites self-skip when `FORGEWEAVER_API_KEY` is not set.
...
The one-shot Headless coding agent needs `FORGEWEAVER_API_KEY` in the environment or repo-root `.env`:
...
The ACP automation server exposes fresh agent sessions over JSON-RPC stdio and also needs `FORGEWEAVER_API_KEY`:
