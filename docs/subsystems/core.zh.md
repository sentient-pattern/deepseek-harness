...
`Agent` 是每个插件（UI、钩子、orchestrator）面向编程的 surface；`ctx.agents.get(id)` 返回它，[发起者作用域](#initiating-agent)携带它。具体实现为 fw-agent-loop 包内部细节；循环外没有任何组件依赖它。统一的 `send` 方法直接暴露 target 与 wakeup 路由；`followup`、`steer` 与 `inject` 是固定预设的别名方法。
...
| Map | 包 | 派生 | 目录 |
|---|---|---|---|
| `ContentBlockMap` | fw-llm | `ContentBlock` | [llm-streaming.md](llm-streaming.zh.md#content-blocks-and-messages) |
| `MessageSourceMap` | fw-llm | `MessageSource` | [llm-streaming.md](llm-streaming.zh.md#content-blocks-and-messages) |
| `FinishReasonMap` | fw-llm | `FinishReason` | [llm-streaming.md](llm-streaming.zh.md#the-model-request-and-result) |
| `TurnTriggerMap` | fw-session | `TurnTrigger` | [session.md](session.zh.md) |
| `TurnEndReasonMap` | fw-session | `TurnEndReason` | [session.md](session.zh.md) |
| `SessionEventMap` | fw-session | `SessionEvent` | [session.md](session.zh.md) |
...
`Branded<B>` 原语位于独立的纯类型包 [fw-brand](../../packages/util/brand) 中（没有运行时代码，也不依赖 harness 包），因此任何包都能品牌化其拥有的 id，而无需依赖无关的能力包。
...
两个核心 ID 是 `CallId`（关联工具调用及其结果；fw-llm）和 `SessionId`（活跃 agent 与持久会话共享的标识；fw-session）。能力包也会品牌化各自的 id，例如 [jobs.md](jobs.zh.md) 中的 `JobId`。
