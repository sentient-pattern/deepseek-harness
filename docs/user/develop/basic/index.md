...
Run `pwd` from the repository root, then create `scratch-plugin/cordis.yml` as a Web overlay that inserts the local plugin. Replace `/absolute/path/to/forgeweaver-harness` below with the printed path:

```yaml
- insert:
    - id: hello
      name: '/absolute/path/to/forgeweaver-harness/scratch-plugin/src/my-plugin.ts'
```
...
```sh
pnpm fw web --patch ./scratch-plugin/cordis.yml
```
