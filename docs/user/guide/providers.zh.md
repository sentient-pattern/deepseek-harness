...
打开**设置 → 模型**。ForgeWeaver 卡片提供一个 API 密钥字段；输入密钥并保存。

![模型页：ForgeWeaver 卡片，以及添加提供方与添加自定义提供方两个入口](providers-models-page.zh.png)

密钥是只写的。保存后，页面只会收到脱敏描述符，永远不会收到明文密钥。密钥存储在 `$FW_HOME/.credentials.yaml` 中，settings 只保留它的凭据引用。
...
因此自定义提供方下的视觉模型需要加一行。表单没有对应字段；请在 `$FW_HOME/settings.yaml` 中给该模型加上 `input`：
...
其中两样占了绝大多数。声明了推理能力的模型，其系统提示词会以 `role: "developer"` 发出，很多网关直接拒绝；输出上限则写作 `max_completion_tokens`，只认 `max_tokens` 的服务端会拒绝。表单里没有这两个字段；请在 `$FW_HOME/settings.yaml` 的路由上更正：
...
全部开关、各自接受的取值，以及接受它们的协议，都列在[生成的 `fw-llm-pi-ai` 配置参考](../../config-catalog.zh.md#forgeweaverfw-llm-pi-ai)的 `PiAiCompatProfile` 之下——该参考派生自源码，因此不会落后于适配器实际接受的内容。
...
- **图片在发送前被拒绝**：该模型未声明图片模态。请给自定义提供方的模型加上 `input: [text, image]`；ForgeWeaver 自身的 chat-completions 路由是纯文本的，且无法通过配置改变。
...
自动生成的[插件配置目录](../../config-catalog.zh.md)列出每个插件的所有受支持字段与默认值；[`fw-llm-pi-ai`](../../config-catalog.zh.md#forgeweaverfw-llm-pi-ai) 就是本页所配置的那个提供方段落。[`fw-llm-pi-ai`](../../../packages/llm/llm-pi-ai/README.zh.md) 和 [`fw-llm-forgeweaver`](../../../packages/llm/llm-forgeweaver/README.zh.md) 参考文档负责直接 `settings.yaml` 配置、目录解析、推理控制、凭据与适配器错误。
