...
- A ForgeWeaver-compatible API endpoint and credential
...
Set the credential in the environment. Set `FORGEWEAVER_BASE_URL` as well when the model is served by an OpenAI-compatible proxy rather than the default ForgeWeaver endpoint.

```sh
export FORGEWEAVER_API_KEY=sk-your-key-here
# export FORGEWEAVER_BASE_URL=http://127.0.0.1:8000/v1
# export FW_MODEL=forgeweaver-v4-flash
# export FW_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Session persistence | Uncompressed JSONL under `FW_SESSION_ROOT` |
