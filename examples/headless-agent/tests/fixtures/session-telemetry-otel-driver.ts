...
import { boot, resolveConfigPath } from '@forgeweaver/fw-app-boot'
import { recordFeedback } from '@forgeweaver/fw-command-feedback'
import { runFixtureTurn } from '@forgeweaver/fw-loader-smoke'
...
process.env.FW_TELEMETRY_E2E_URL = `http://127.0.0.1:${address.port}/v1/logs`
...
try {
  ...
  const mode = process.env.FW_TELEMETRY_E2E_MODE ?? 'FULL'
  if (mode !== 'FULL') {
