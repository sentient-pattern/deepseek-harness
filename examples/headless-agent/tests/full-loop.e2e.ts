...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)('full loop: real model + real bash tool', () => {
  it('runs a bash command on request and reports its output', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'fw-full-loop-e2e-'))
    ctx = await codingHarness(workdir, { persona: SYSTEM_PROMPT })
    const agent = ctx.agentLoop.create(SessionId('e2e-loop'), { provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash' })
