...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)('todo_write: real model records a plan', () => {
  it('appends a todo/write event with the model-produced task list', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'fw-todo-write-e2e-'))
    ctx = await codingHarness(workdir, { persona: TODO_SYSTEM_PROMPT })
    const agent = ctx.agentLoop.create(SessionId('e2e-todo'), { provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash' })
