 ...
 * the REAL `fw-jsonrpc-agent` runtime (per `FW_EXAMPLE_MODE`) through the
 * REAL `@forgeweaver/fw-sdk-client`, drives one turn over stdio JSON-RPC,
 * and pins the SDK `RunResult`, the complete notification stream, and the
 * persisted session logs. Replay serves recorded model
 * responses via `llm-replay` (`cordis.snapshot.yml`); `FW_SNAPSHOT=record`
 * re-records against the live API; `FW_SNAPSHOT=refresh` replays committed
...
import { ForgeWeaverHarness, type HarnessNotification, type RunResult } from '@forgeweaver/fw-sdk-client'
...
const mode = process.env.FW_SNAPSHOT ?? 'replay'
...
const SCENARIOS: SdkScenario[] = [
  ...
  {
    name: 'bash-tool',
    prompt: 'Run this exact command with your bash tool, then reply with its stdout only: echo fw-sdk-proof-7391',
  ...
  },
  {
    ...
    environment: { FW_SYSTEM_PROMPT: MINIMAL_SYSTEM_PROMPT },
...
async function runScenario(scenario: SdkScenario): Promise<{
  ...
  const env: Record<string, string> = {
    ...
    FW_CORDIS_CONFIG: recording
      ? scenario.configs?.live ?? liveConfig
      : scenario.configs?.replay ?? replayConfig,
    FW_SESSION_ROOT: sessionsRoot,
    FW_CWD: cwd,
    FW_SNAPSHOT: mode,
    NODE_OPTIONS: [process.env.NODE_OPTIONS, '--disable-warning=ExperimentalWarning'].filter(Boolean).join(' '),
    ...parentFixture === undefined ? {} : {
      FW_SNAPSHOT_FILE: parentFixture,
      ...childFixtures.length > 0 ? { FW_SNAPSHOT_CHILD_FILES: childFixtures.join(delimiter) } : {},
    },
