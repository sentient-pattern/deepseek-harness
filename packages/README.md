...
npm scope: `@forgeweaver/fw-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@forgeweaver/fw-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`bundle/`](bundle/README.md) | Installable `fw --profile` patch layers | Product — stable API |
