...
npm scope 为 `@forgeweaver/fw-*`；Cordis `Service` 子类和函数插件通过 `ctx.effect()`、`ctx.on()` 或 `ctx.waterfall()` 注册。规则见[包](AGENTS.md)与[根规则](../AGENTS.md#conventions)。

## 层级结构

包按组置于 `packages/<group>/<pkg>/`；包名仍为 `@forgeweaver/fw-<pkg>`。**组 README 负责包／ctx 键映射。**
...
| [`bundle/`](bundle/README.zh.md) | 可安装的 `fw --profile` 补丁层 | 产品：稳定 API |
...
**扩展插件依赖 Service Definition，绝不依赖具体提供方。** `fw-agent-loop` 可替换；UI、钩子和工具插件使用 `fw-agent`。包括 `fw-agent-spine-demo` 在内的组合包可以依赖主干插件。能力会将需要独立演进的 Service Definition／Service Provider／Consumer 角色分离；详见[能力 seam](../.agents/notes/implemented/architecture/2026-06-13-capability-seams.zh.md)。
