...
通过 JSON-RPC stdio 提供的仅面向自动化的 [ACP（Agent Client Protocol）](https://agentclientprotocol.com) 服务器。程序化客户端可以创建新 harness agent（智能体）、发送文本／图片提示词、收集已提交的 assistant 文本／图片、按策略响应一次性权限请求并取消工作。仓库中的主要客户端是 [`fw-subagent-acp`](../../subagent/subagent-acp/README.zh.md)。
...
`pnpm --dir /path/to/forgeweaver-harness run demo:acp` 启动仓库的自动化服务器组合。父 harness 可以通过 [`@forgeweaver/fw-subagent-acp`](../../subagent/subagent-acp/README.zh.md) spawn 它；其他 ACP 客户端只需上述核心方法。
