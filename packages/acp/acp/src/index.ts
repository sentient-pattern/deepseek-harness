...
import { createUserMessage, errorChain } from '@forgeweaver/fw-llm'
import {
  AgentSideConnection,
  ndJsonStream,
  PROTOCOL_VERSION,
...
export function apply(ctx: Context, config: AcpConfig): void {
  ...
  // fw-subagent-acp. The bridge offers one-shot choices only and never infers a
    ...
    return {
      async initialize(_params: InitializeRequest): Promise<InitializeResponse> {
        ...
        return {
          ...
          agentInfo: { name: 'forgeweaver-harness-acp', version: '0.0.1' },
          agentCapabilities: {
