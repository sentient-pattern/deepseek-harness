/** Local durable attachment backend rooted below `FW_HOME`. @module @forgeweaver/fw-attachment-local */
...
export class LocalAttachmentStore extends AttachmentStore {
  ...
  constructor(ctx: Context, config: Config) {
    super(ctx)
    this.root = resolve(join(resolveDshHome(config.fwHome), 'attachments', 'v1'))
