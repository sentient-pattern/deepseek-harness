...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const fwHome = await mkdtemp(join(tmpdir(), 'fw-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { fwHome })
      ...
      await rm(fwHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const fwHome = await mkdtemp(join(tmpdir(), 'fw-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { fwHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const fwHome = await mkdtemp(join(tmpdir(), 'fw-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { fwHome })
  ...
  it('prepares every batch member before any write', async () => {
    const fwHome = await mkdtemp(join(tmpdir(), 'fw-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { fwHome, normalizedImageMaxBytes: 1 })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const fwHome = await mkdtemp(join(tmpdir(), 'fw-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { fwHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { fwHome, maxImageBytes: 1 })
