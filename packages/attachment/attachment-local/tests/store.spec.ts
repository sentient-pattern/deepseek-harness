...
import type { ImageAttachmentLimits } from '@forgeweaver/fw-attachment'
...
async function root(): Promise<string> {
  const value = await mkdtemp(join(tmpdir(), 'fw-attachment-'))
  roots.push(value)
...
describe('local attachment store', () => {
  it.skipIf(process.platform === 'win32')('syncs every object ancestor up to the durable boundary before returning', async () => {
    ...
    // Each process first proves FW_HOME durable all the way to the filesystem
