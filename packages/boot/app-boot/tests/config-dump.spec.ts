...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'fw-config-dump-'))
...
function writeBase(dir: string): string {
  ...
  writeFileSync(base, [
    ...
    '  config:',
    '    value: base',
    '    key: !!js process.env.FW_DUMP_SPEC',
    '- id: untouched',
    '  name: ./noop.mjs',
    '',
  ].join('\n'))
...
describe('renderConfigDump', () => {
  it('composes overlay layers in order, prints !!js verbatim, and labels each section with its source and patches', () => {
    ...
    expect(parsed).toEqual([
      {
        id: 'shared',
        name: './noop.mjs',
        config: { value: 'surface', key: { __jsExpr: 'process.env.FW_DUMP_SPEC' } },
      },
    ...
    expect(dump).toContain('!!js process.env.FW_DUMP_SPEC')
