/**
 * Profile machinery of `fw-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'fw-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { fw: { bundle: { patch: './cordis.patch.yml' } } },
    }))
    if (spec.patch !== undefined) writeFileSync(join(dir, 'cordis.patch.yml'), spec.patch)
  }
  writeFileSync(join(appDir, 'package.json'), JSON.stringify({ name: 'fw-app', dependencies: appDeps }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@forgeweaver/fw-base'])
    ...
    expect(readProfileManifest('t', dir).fw?.profile?.bundles).toEqual(['@forgeweaver/fw-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', fw: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).fw?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each fw.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or fw section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @forgeweaver/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@forgeweaver/fw-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).fw?.profile?.bundles)
      .toEqual([...PROFILE_TEMPLATES.web ?? []])
  })

  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@forgeweaver/fw-base': { patch: '[]\n' },
      '@forgeweaver/fw-web-app': { patch: '[]\n' },
      '@forgeweaver/fw-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@forgeweaver/fw-base', '@forgeweaver/fw-web-app', '@forgeweaver/fw-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).fw?.profile?.bundles)
      .toEqual(['@forgeweaver/fw-base', '@forgeweaver/fw-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@forgeweaver/fw-base', '@forgeweaver/fw-web-app', '@forgeweaver/fw-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).fw?.profile?.bundles).toEqual([
      '@forgeweaver/fw-base', '@forgeweaver/fw-web-app', '@forgeweaver/fw-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no fw.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no fw.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'fw-app']) {
  ...
  it('throws when a fallback entry is a real directory', () => {
    const anchor = stageInstallation({})
    const home = tmp()
    mkdirSync(join(home, 'profiles', 'node_modules', 'fw-app'), { recursive: true })
  ...
  it('replaces a wrong symlink', () => {
    ...
    symlinkSync(tmp(), join(fallback, 'fw-app'), 'junction')
    healProfilesModuleFallback(anchor, home)
    expect(readlinkSync(join(fallback, 'fw-app'))).toContain('app')
  })
  ...
  it('tolerates losing the concurrent-heal race to an identical link and rejects a different one', () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    expect(lstatSync(join(fallback, 'fw-app')).isSymbolicLink()).toBe(true)
  })
})
