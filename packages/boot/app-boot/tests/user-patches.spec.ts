/**
 * User patch-layer behavior of `fw-app-boot`: the optional patch-list loader
...
import Hmr from '@forgeweaver/cordis-plugin-hmr'
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'fw-user-patches-'))
...
describe('loadOptionalPatches', () => {
  ...
  it('parses a patch list and preserves !!js expressions as loader expression nodes', () => {
    ...
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), [
      ...
      '  config:',
      '    model: !!js process.env.FW_SPEC_MODEL',
      '- insert:',
      '    - id: llm',
    ...
    expect(patches?.[0]).toMatchObject({
      id: 'agent-loop',
      config: { model: { __jsExpr: 'process.env.FW_SPEC_MODEL' } },
    })
...
describe('boot with user patches', () => {
  it('applies id-targeted overrides, inserts, and interpolates !!js from the environment', async () => {
    ...
    writeFileSync(join(userDir, PROFILE_PATCH_FILENAME), [
      ...
      '  name: ./noop.mjs',
      '  config:',
      '    value: !!js process.env.FW_APP_BOOT_USER_SPEC',
      '- insert:',
      '    - id: user-extra',
      '      name: ./noop.mjs',
      '',
    ].join('\n'))
    process.env['FW_APP_BOOT_USER_SPEC'] = 'user-value'
    ...
    try {
      ...
      await ctx.fiber.dispose()
      delete process.env['FW_APP_BOOT_USER_SPEC']
    }
  })
