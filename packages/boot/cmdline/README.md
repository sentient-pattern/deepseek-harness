# `@forgeweaver/fw-cmdline`

English | [中文](README.zh.md)

The command line a fw launcher hands to the app it boots. The launcher parses only its own flags (`--profile`, `--patch`, the config dumps) and hands **everything after them** to the tree verbatim, so an app owns its flag family, its `--help` text, and its parse errors instead of the launcher knowing them.

## The launcher values

A launcher calls `provideCmdline(ctx, host)` before any tree entry mounts, which provides:

- `ctx.cmdlineArgs` — the invocation's inner arguments. `get()` is the whole interface, and it returns a snapshot: `fw --profile tui --resume abc` yields `['--resume', 'abc']`.
...
```yaml
- id: web-startup
  name: '@forgeweaver/fw-web-app/startup'
```
