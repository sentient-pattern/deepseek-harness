/**
 * The bundle's substance is its patch file: the `fw.bundle.patch` manifest
...
describe('fw-base bundle', () => {
  it('declares a parseable patch list through the fw.bundle.patch manifest field', () => {
    ...
    const parsed = yaml.load(
      readFileSync(resolve(root, manifest.fw!.bundle!.patch!), 'utf8'),
    ...
    expect(rows.find(row => row.id === 'session-telemetry-otel')?.config?.['mode']).toEqual({
      __jsExpr: "process.env.FW_TELEMETRY_MODE || 'DISABLED'",
    })
    expect(rows.filter(row => row.id === 'subagent-codex')).toHaveLength(0)
    expect(rows.filter(row => row.id === 'subagent-claude-code')).toHaveLength(0)
    expect(manifest.dependencies).not.toHaveProperty('@forgeweaver/fw-subagent-codex')
    expect(manifest.dependencies).not.toHaveProperty('@forgeweaver/fw-subagent-claude-code')
  })
