/**
 * @forgeweaver/fw-headless — one-shot direct Agent driver. The bundle patch
 * rides over fw-base without Host, HTTP, or browser plugins; this runner
 ...
 *
 * @module @forgeweaver/fw-headless
 */
...
import type { ModelSelectionRef } from '@forgeweaver/fw-agent'
import type {} from '@forgeweaver/fw-agent-default-model'
...
import type {} from '@forgeweaver/fw-cmdline'
...
function fail(io: HeadlessIo, error: unknown): void {
  io.stderr.write(`fw: ${error instanceof Error ? error.message : String(error)}\n`)
  io.exit(1)
}
...
async function run(ctx: Context, task: string, io: HeadlessIo): Promise<void> {
  ...
  if (outcome.reason?.kind === 'error') {
    io.stderr.write(`fw: ${outcome.reason.error.code}: ${outcome.reason.error.message}\n`)
  }
