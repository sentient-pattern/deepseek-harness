/**
 * @forgeweaver/fw-web-app — the browser-surface bundle's runtime glue plugin
 * plus the bundle patch (`cordis.patch.yml`, declared by the `fw.bundle.patch`
...
import { addHarnessSourceSection } from '@forgeweaver/fw-app-boot'
import * as FrontendStatic from '@forgeweaver/fw-host-frontend-static'
...
/** This fw installation's root, from either this package's source or built entry. */
...
const FW_WEB_URL = 'FW_WEB_URL' as const
...
/** Model-visible orientation and acceptance boundary for sessions created through `fw web`. */
function webSurfacePrompt(webUrl: string): string {
  const updateContract = 'The client-plugin HMR receiver is active, but client-plugin changes reload without a refresh only while '
    + '`pnpm run dev:web` is also running from this same checkout to rebuild their bundles; verify that watcher before promising automatic updates. '
    + 'Every other change — the apps/web shell and plain packages — requires rebuilding the affected Web artifacts and verifying this existing URL after a page refresh. '
  return `You are interacting with the user through the ForgeWeaver Web GUI at ${webUrl}. `
    + 'When the user refers to "this page", "this GUI", or "this app" without naming another target, they mean this GUI. '
    + 'The browser provides no implicit DOM, route, or screenshot context. '
    + updateContract
    + 'Starting another server does not update this GUI. '
    + 'The apps/web Vite entry builds the shell but is not a standalone application because only fw web injects window.__FW_BOOT__. '
...
function resolveDistIndex(): string {
  ...
  try {
    return require.resolve('@forgeweaver/fw-web-frontend/dist/index.html')
  } catch {
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (config.surfaceContext) {
    ...
    ctx.inject(['shellEnv'], (runtimeCtx) => {
      runtimeCtx.shellEnv.register({
        name: 'web-runtime',
        variables: {
          [FW_WEB_URL]: { description: 'Canonical local URL of the ForgeWeaver Web GUI serving this session.' },
        },
        resolve: () => ({ [FW_WEB_URL]: localWebUrl(runtimeCtx) }),
      })
    })
  }
  if (config.printUrl || handoffBrowser) {
    ...
    const announceReady = (): void => {
      ...
      if (config.printUrl) {
        console.log(`fw web: ${webUrl}${lanCandidate === undefined ? '' : ` (LAN: http://${lanCandidate}:${String(port)})`}`)
      }
      if (handoffBrowser) {
        console.log('fw web: opening the default browser; pass --no-open to disable')
