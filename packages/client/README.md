...
The browser side of the fw web GUI: shell boot, browser-host communication, shared UI services, and feature plugins. Authoring rules live in [AGENTS.md](AGENTS.md); the host half is [`host/`](../host/README.md). All except `test-runtime` are **product** packages named `@forgeweaver/fw-client-<name>`.
...
| [`ui-settings-models/`](ui-settings-models/README.md) | Provides model-provider configuration and ForgeWeaver onboarding. |
