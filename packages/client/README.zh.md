...
fw web GUI 的浏览器侧：shell 启动、浏览器与宿主通信、共享 UI 服务和功能插件。编写规则见 [AGENTS.md](AGENTS.md)；宿主半侧是 [`host/`](../host/README.zh.md)。除 `test-runtime` 外，均为名为 `@forgeweaver/fw-client-<name>` 的**产品**包。
...
| [`ui-settings-models/`](ui-settings-models/README.zh.md) | 提供模型提供方配置与 ForgeWeaver 配置引导。 |
