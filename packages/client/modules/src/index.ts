/**
 * Node half of the client module system (`fw.client` dual-face package): scans
 * the host Loader's entries for packages declaring `fw.client`, composes the
 * `window.__FW_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @forgeweaver/fw-client-modules
 */
...
import type {} from '@forgeweaver/cordis-plugin-loader'
...
/** package.json `fw.client` declaration fields, validated one by one after reading the file. */
...
/** Resolved package metadata for one `fw.client` package (cached per name, never expires). */
...
/** Narrow an unknown parsed JSON value to the `fw.client` declaration, throwing on malformed fields. */
function parseDshClient(pkgName: string, value: unknown): DshClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object fw.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} fw.client.platform must be a string`)
  }
  const inject = optionalStringArray(pkgName, 'fw.client.inject', decl.inject)
  const external = optionalStringArray(pkgName, 'fw.client.external', decl.external)
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} fw.client.immediately must be a boolean`)
  }
  return {
    ...
    for (const name of entry.external ?? []) {
      ...
      if (dependency === entry) {
        throw new Error(
          ...
          + '— a row must not declare its own package in fw.client.external',
        )
      }
...
const CLIENT_MODULES_ID = '@forgeweaver/fw-client-modules'

/** Dynamic package whose ordinary client bundle must be registered before plugin boot starts. */
const CLIENT_RUNTIME_ID = '@forgeweaver/fw-client-runtime'
...
window.__ModuleLoader__={
  ...
  return [
    ...
    ...preload,
    { kind: 'global', name: '__FW_BOOT__', value: graph },
  ]
}

/**
 * The web plugin table service: incremental `fw.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `fw.client` declaration) are
   ...
   * @returns the graph served as `window.__FW_BOOT__`.
   */
  graph(): WebBootGraph {
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const decl = parseDshClient(
      pkgName,
      fw !== null && typeof fw === 'object' ? (fw as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares fw.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
