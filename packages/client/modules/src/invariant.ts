/**
 * Package-owned invariant companion for `@forgeweaver/fw-client-modules`.
 * @module @forgeweaver/fw-client-modules/invariant
 */
...
const PACKAGE_NAME = '@forgeweaver/fw-client-modules'
  ...
  ctx.on('internal/plugin', () => {
    ...
    for (const row of host.graph().entries) {
      if (host.clientPath(row.id) === undefined) {
        fail(`web plugin graph row "${row.id}" advertises ${row.url} but resolves no client bundle path — the served __FW_BOOT__ would 404 on fetch`)
      }
    }
  }, { global: true })
}
