...
import { renderIndexInjections, type WebServer, type WebRoute } from '@forgeweaver/fw-host-webserver'
...
const RUNTIME_ID = '@forgeweaver/fw-client-runtime'
...
function writePackage(
  packageName: string,
  metadata: Record<string, unknown> = { fw: { client: { platform: 'web' } } },
): string {
  root ??= realpathSync(mkdtempSync(join(tmpdir(), 'fw-client-modules-')))
...
function writeBuiltPackage(packageName: string, client: Record<string, unknown>): void {
  const clientPath = writePackage(packageName, { fw: { client: { platform: 'web', ...client } } })
...
describe('HTML bootstrap facade', () => {
  it('precedes blocking preloads and the boot graph, then becomes the live registration target', async () => {
    ...
    const graphAt = html.indexOf('globalThis["__FW_BOOT__"] = ')
...
describe('shared module declarations', () => {
  ...
  it('rejects a non-array external', () => {
    ...
    expect(() => construct([packageName]))
      .toThrow(`client-modules: ${packageName} fw.client.external must be a string array`)
  })
})
...
describe('module graph order', () => {
  ...
  it('leaves a request no row answers to the static assembly channel', () => {
    expect(ids(orderByModuleGraph([
      entry('consumer', { external: ['@forgeweaver/cordis'] }),
      entry('other'),
