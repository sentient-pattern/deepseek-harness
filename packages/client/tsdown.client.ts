 ...
 */
const CSS_VIRTUAL_PREFIX = '\0fw-css:'
const GLOBAL_CSS_VIRTUAL_PREFIX = '\0fw-global-css:'
const INLINE_CSS_VIRTUAL_PREFIX = '\0fw-inline-css:'
 ...
 * Everything else under @forgeweaver/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@forgeweaver\/fw-(host-apiproxy|file-reference|session|llm|tools|brand)(\/|$)/
...
const GENERATED_REMOTE = /^@forgeweaver\/fw-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.FW_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.FW_BUILD_FACE) === 'host'
...
function buildFace(value: unknown): BuildFace {
  ...
  throw new Error(`tsdown: --env.FW_BUILD_FACE must be host or client, received ${String(value)}`)
}
...
interface WorkspaceManifest {
  ...
  readonly fw?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `fw.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `fw.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'fw.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `fw.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).fw?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @forgeweaver value import is a build error — a
      ...
      name: 'fw-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@forgeweaver/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s fw.client.external, an inline-safe wire layer, or a generated /remote contribution — `
...
const STATIC_LINKED_PLUGIN = 'fw-static-linked-external'
