...
import { TestSessions, TestWorkspaces } from '@forgeweaver/fw-client-test-runtime'
import type { Stabilizer } from '@forgeweaver/fw-client-test-runtime'
import { apply as nodeApply } from '@forgeweaver/fw-client-ui-renderer'
...
describe('UI renderer plugin', () => {
  ...
  it('hydrates the boot page before switching to the assembled application', async () => {
    ...
    slots.register({ name: 'root' }, () => <div data-testid="root-probe" />)
    const el = container()
    el.innerHTML = '<div class="boot" data-fw-boot=""><div><div class="spinner" data-fw-boot-spinner="" style="--fw-boot-arc: 180deg"></div><div>Loading plugins…</div></div></div>'
