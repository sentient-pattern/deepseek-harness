/**
 * Curated editor for the direct ForgeWeaver adapter's advisory model catalog.
...
export type ForgeWeaverModelDraft = Record<string, unknown>
 ...
 * (rejected by {@link validateForgeWeaverModels} before any write).
 */
...
export interface ForgeWeaverModelsValidationFailure {
...
export function modelDrafts(value: unknown): ForgeWeaverModelDraft[] {
  if (!Array.isArray(value)) return []
  return value.map(entry =>
    typeof entry === 'object' && entry !== null && !Array.isArray(entry)
      ? entry as ForgeWeaverModelDraft
      : {})
}
 ...
 */
export function validateForgeWeaverModels(value: unknown): ForgeWeaverModelsValidationFailure | undefined {
...
/** Props of {@link ForgeWeaverModelsEditor}. */
export interface ForgeWeaverModelsEditorProps {
  /** Effective rows: inherited until the parent materializes an override. */
  models: readonly ForgeWeaverModelDraft[]
  ...
  onChange: (models: ForgeWeaverModelDraft[]) => void
  /** Remove the user-owned array and return to inheritance. */
  onReset: () => void
}

/**
 * Render the direct ForgeWeaver adapter's model catalog: id and display name on
 * each row, capacities behind the row's own disclosure.
 * @param props - effective rows plus the array-level override actions.
 * @returns the catalog editor.
 */
export function ForgeWeaverModelsEditor(props: ForgeWeaverModelsEditorProps): ReactNode {
  ...
  const capacityText = (model: ForgeWeaverModelDraft, index: number, field: CapacityField): string => {
