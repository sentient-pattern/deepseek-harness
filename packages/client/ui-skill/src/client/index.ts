 ...
 * determinism
 * lives host-side — the pre-step boundary (`fw-tool-skill`) recognizes a
...
import type { ConnectionHandle, SessionId, SkillEntry } from '@forgeweaver/fw-api-remotes/client'
import type { ClientContext, ISessions } from '@forgeweaver/fw-client-runtime/client'
import type { InputTriggerServiceContract, InputTriggerSource } from '@forgeweaver/fw-client-ui-input-trigger/client'
// Type-only: pulls the locale plugin's Context merge (ctx.locale).
import type {} from '@forgeweaver/fw-client-locale/client'
...
export function apply(ctx: ClientContext): void {
  ...
  const source: InputTriggerSource = {
    ...
    onPick({ candidate }) {
      ...
      // pre-step boundary (fw-tool-skill) recognizes the leading /name and
