...
describe('WorkspaceBrowser.module.css list', () => {
  ...
  it('counts the themed scrollbar inside the shell trailing inset', () => {
    expect(root?.get('--fw-session-list-edge-inset')).toBe('var(--fw-sidebar-inline-padding)')
    expect(root?.get('--fw-session-list-scrollbar-width')).toBe('8px')
    expect(root?.get('--fw-session-list-scrollbar-offset')).toBe('2px')
    expect(root?.get('padding-right')).toBe('var(--fw-session-list-edge-inset)')
    expect(listArea?.get('margin-left')).toBe('-4px')
    expect(listArea?.get('padding-left')).toBe('4px')
    expect(listArea?.get('margin-right')).toBe('calc(-1 * var(--fw-session-list-edge-inset))')
    expect(declarations('.fade')?.get('right')).toBe('var(--fw-session-list-edge-inset)')
    expect(list?.get('margin-right')).toBe('var(--fw-session-list-scrollbar-offset)')
    expect(list?.get('margin-left')).toBe('-4px')
    expect(list?.get('padding-left')).toBe('4px')
    expect(list?.get('padding-right')).toBe([
      'calc(',
      'var(--fw-session-list-edge-inset)',
      '- var(--fw-session-list-scrollbar-width)',
      '- var(--fw-session-list-scrollbar-offset)',
      ')',
    ].join(' '))
