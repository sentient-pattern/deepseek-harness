 ...
 * @module @forgeweaver/fw-client-web/src/boot
 */
...
} from '@forgeweaver/fw-client-modules/client'
import type {} from '@forgeweaver/fw-client-ui-renderer/client'
...
export class AppWebEntry {
  ...
  async run(): Promise<void> {
    try {
      ...
      // is `ClientTransportHooks`, owned by @forgeweaver/fw-client-connection;
      // this structural slice reads one optional member without adding a
      // package edge.
      const transport = (globalThis as {
        __FW_TRANSPORT__?: { loadBundle?: ClientModuleCreateOptions['loadBundle'] }
      }).__FW_TRANSPORT__
      this.modules = moduleLoader.create({
        boot: win.__FW_BOOT__,
  ...
  private async prefetchImmediateTier(): Promise<void> {
    ...
    const transport = (globalThis as {
      __FW_TRANSPORT__?: { loadBundle?: unknown }
    }).__FW_TRANSPORT__
