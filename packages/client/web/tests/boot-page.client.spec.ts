...
describe('BootPage', () => {
  it('draws the loading skeleton before any plugin state arrives', () => {
    const { el } = mount()
    expect(el.firstElementChild?.getAttribute('data-fw-boot')).toBe('')
  ...
  it('keeps loading while entries are active or loading', () => {
    const { el, page } = mount()
    page.setTotal(2)
    const spinner = el.querySelector<HTMLElement>('[data-fw-boot-spinner]')
    expect(spinner?.style.getPropertyValue('--fw-boot-arc')).toBe('72deg')
    page.setState('a', 'active')
    expect(spinner?.style.getPropertyValue('--fw-boot-arc')).toBe('180deg')
    page.setState('b', 'loading')
    expect(el.querySelector('[data-fw-boot-spinner]')).toBe(spinner)
    page.setState('b', 'active')
    expect(spinner?.style.getPropertyValue('--fw-boot-arc')).toBe('288deg')
    expect(el.textContent).toContain('Loading plugins…')
  ...
  it('lists failed entries', () => {
    const { el, page } = mount()
    page.setState('@forgeweaver/fw-client-ui-layout', 'failed')
    page.setState('ok', 'active')
    page.setState('@forgeweaver/fw-client-ui-tool', 'failed')
    expect(el.textContent).toContain('@forgeweaver/fw-client-ui-layout')
    expect(el.textContent).toContain('@forgeweaver/fw-client-ui-tool')
