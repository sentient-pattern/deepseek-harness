 ...
 * @module @forgeweaver/fw-code-runtime
 */
 ...
 * `__fw_main__`/`__builtins__`/`__name__` (the Python backend's bootstrap
...
export const RESERVED_BINDING_GLOBALS: ReadonlySet<string> = new Set([
  'console',
  '__fw_main__', '__builtins__', '__name__', '__debug__',
])
...
export abstract class CodeRuntime extends Service {
   ...
   * `fw-tools` presents; only `'typescript'` has a published backend.
   */
