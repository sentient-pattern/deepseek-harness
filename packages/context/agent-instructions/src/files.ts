 ...
 *
 * @module @forgeweaver/fw-agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@forgeweaver/fw-fs'
import { assertNever } from '@forgeweaver/fw-llm'
import { fwHomeDisplay } from '@forgeweaver/fw-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.fwHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.fwHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    absolutePath,
    displayPath: directory === USER_GLOBAL_DIRECTORY ? userGlobalDisplayPath(resolved.fwHome) : relativeDisplay(projectRoot, absolutePath),
    target,
...
function userGlobalDisplayPath(fwHome: string): string {
  return `${fwHomeDisplay(fwHome)}/AGENTS.md`
}
