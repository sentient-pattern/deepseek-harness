...
import LlmRuntime, { createUserMessage, CallId, LlmError, MessageSource, ProviderRequestId, StreamChunk  } from '@forgeweaver/fw-llm'
import SessionStore, { Session, SessionEvent, SessionId, TurnEndReason, type UserMessage } from '@forgeweaver/fw-session'
import SystemPrompt from '@forgeweaver/fw-system-prompt'
import ToolRuntime, { defineContentToolFixture, type PostToolDecision } from '@forgeweaver/fw-tools'
...
describe('turn and step boundary recovery', () => {
  ...
  it('a step/end validation failure surfaces the resulting open-step invariant', async () => {
    ...
    expect(errors.map(error => error.message)).toEqual([
      ...
      'invariant violated by "@forgeweaver/fw-session": turn/end 1 while step 1 is still open',
    ])
    expect(boundaryCounts(agent)).toMatchObject({
      turnStart: 1,
      turnEnd: 0,
