...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionPreparation } from '@forgeweaver/fw-session'
...
async function persistentHarness(adapter: MockAdapter): Promise<{ ctx: Context; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'fw-resume-'))
  dirs.push(root)
