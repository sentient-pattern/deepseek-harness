 ...
 *
 * @module @forgeweaver/fw-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@forgeweaver/cordis'
...
import type { TypertContext, TypertLookup } from '@forgeweaver/fw-typert-protocol'
...
declare module '@forgeweaver/cordis' {
  interface Context {
     ...
     * `fw-scope` tag while retaining it, so this field is DX context rather
...
export interface CreateAgentOptions {
   ...
   * {@link CreateSessionOptions.meta} in fw-session (the internal-only
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `fw-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `fw-agent-loop` package.
 */
 ...
 * (`@forgeweaver/fw-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@forgeweaver/fw-agent#Agent',
