 ...
 *
 * @module @forgeweaver/fw-system-prompt
 */

import { Context, Service } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeTarget } from '@forgeweaver/fw-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@forgeweaver/fw-scope'
import type { ContextSnapshotSection, ToolSchema } from '@forgeweaver/fw-llm'
...
declare module '@forgeweaver/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@forgeweaver/fw-scope`): scoped listeners
  ...
  constructor(ctx: Context, config: Config) {
    ...
    if (config.includeHarnessIdentity ?? true) {
      this.section({
        ...
        order: -100,
        text: 'You are an AI agent powered by ForgeWeaver.',
      })
    }
    this.section({
