...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@forgeweaver/fw-system-prompt'
...
const IDENTITY = 'You are an AI agent powered by ForgeWeaver.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are ForgeWeaver.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are ForgeWeaver.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
  ...
  describe('prompt variables', () => {
    ...
    it('interpolates {{name}} references in section text at render — the persona included', async () => {
      ...
      ctx.systemPrompt.variable('model', () => 'forgeweaver-v4')
      ctx.systemPrompt.variable('cwd', () => '/work')

      expect(renderPrompt(await ctx.systemPrompt.assemble())).toBe(`${IDENTITY}\n\nYou run on forgeweaver-v4 in /work.`)
    })
