 ...
 * @module @forgeweaver/fw-tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@forgeweaver/fw-llm'
import type { ContentBlock } from '@forgeweaver/fw-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@forgeweaver/fw-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`fw-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
