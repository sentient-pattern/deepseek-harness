 ...
 * execution pipeline.
 * @module @forgeweaver/fw-tools
 */

import { Context, Service } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@forgeweaver/fw-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@forgeweaver/fw-scope'
import type { CallId, ContentBlock, ToolSchema } from '@forgeweaver/fw-llm'
import { assertNever, deepFreeze, HarnessError } from '@forgeweaver/fw-llm'
import type { Agent } from '@forgeweaver/fw-agent'
import { snapshotJsonValue } from '@forgeweaver/fw-session'
import type { JsonValue, UserMessage } from '@forgeweaver/fw-session'
import type { ToolProviderResult } from '@forgeweaver/fw-system-prompt'
import type { CodeRuntime } from '@forgeweaver/fw-code-runtime'
 ...
 * instead of deriving them: the seam's `fw-code-runtime` README pair, its
...
export type { JsonValue } from '@forgeweaver/fw-session'
...
// lives in its own UI-facing module; re-export it so `@forgeweaver/fw-tools`
...
declare module '@forgeweaver/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@forgeweaver/fw-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@forgeweaver/fw-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@forgeweaver/fw-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@forgeweaver/fw-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@forgeweaver/fw-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `fw-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`fw-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`fw-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @forgeweaver/fw-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`fw-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `fw-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('fw.tool.execution') as ToolExecutionToken
}
