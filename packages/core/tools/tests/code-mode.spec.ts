...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@forgeweaver/fw-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@forgeweaver/fw-tools'
import type { Agent } from '@forgeweaver/fw-agent'
import { Session, SessionId } from '@forgeweaver/fw-session'
import type { JsonValue, SessionEventMap } from '@forgeweaver/fw-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@forgeweaver/fw-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `fw-agent-tool-presentation` produces.
