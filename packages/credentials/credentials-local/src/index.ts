/**
 * File-backed credentials provider over `$FW_HOME/.credentials.yaml`, layered
 ...
 * > $FW_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `FORGEWEAVER_API_KEY=… fw`, a CI
 ...
 * @module @forgeweaver/fw-credentials-local
 */
...
import { CredentialProvider, credentialRef, parseCredentialKey } from '@forgeweaver/fw-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
...
import type { LaunchEnvironmentEntry } from '@forgeweaver/fw-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveDshHome(config.fwHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `fw-atomic-write`, this is a
...
/** File-backed credentials provider (`$FW_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start fw from instead',
      )
    }
  }
