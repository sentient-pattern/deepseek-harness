...
vi.mock('@forgeweaver/fw-atomic-write', async (importOriginal) => {
  const actual = await importOriginal<typeof import('@forgeweaver/fw-atomic-write')>()
...
async function setGate(next: Promise<void>): Promise<void> {
  const mocked = await import('@forgeweaver/fw-atomic-write') as unknown as { __setGate: (next: Promise<void>) => void }
  mocked.__setGate(next)
}

const KEY = credentialRef('FW_CRED_DRAIN_A')
const OTHER = credentialRef('FW_CRED_DRAIN_B')
...
describe('write-drain teardown', () => {
  it('lets the in-flight write land and fails the queued one after disposal', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'fw-credentials-drain-'))
  ...
  it('fails a queued record write after disposal on the same terms', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'fw-credentials-drain-record-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const ctx = new Context()
    const fiber = ctx.plugin(LocalCredentialProvider, { path: join(dir, '.credentials.yaml'), watch: false })
    await fiber
