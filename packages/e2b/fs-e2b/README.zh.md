...
[`@forgeweaver/fw-fs`](../../fs/fs/README.zh.md) 提供方约定的 E2B 实现。它没有配置：先加载 [`@forgeweaver/fw-e2b`](../e2b/README.zh.md)，再用本服务取代 `fw-fs-local`。该提供方使用所有者的远程 cwd 和 SDK 句柄，因此文件工具观察到的环境与 E2B 后端 Bash 进程相同。
...
通过 [`fw-tool-fs`](../../fs/tool-fs/README.zh.md) 间接影响模型；该工具会渲染远程 UTF-8 内容、目录结果、变更确认和提供方错误，而 E2B 身份及传输保持内部实现。
