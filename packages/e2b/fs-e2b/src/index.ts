 ...
 * @module @forgeweaver/fw-fs-e2b
 */

import { createHash, randomUUID } from 'node:crypto'
import { Buffer } from 'node:buffer'
import { posix } from 'node:path'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@forgeweaver/fw-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type { EntryInfo, Sandbox } from '@forgeweaver/fw-e2b'

const VERSION_METADATA_KEY = 'fw-version'
...
export class E2BFileSystem extends FileSystem {
  ...
  private async writeAtomic(
    ...
    const stagingDirectory = posix.join(posix.dirname(targetPath), `.fw-${randomUUID()}.tmp`)
