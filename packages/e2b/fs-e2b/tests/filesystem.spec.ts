...
import { FsTargetKey, FsVersion } from '@forgeweaver/fw-fs'
import E2BFileSystem from '@forgeweaver/fw-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.fw-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['fw-version']).toBeDefined()
