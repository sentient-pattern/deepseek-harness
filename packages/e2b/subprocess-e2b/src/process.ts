...
import type { CommandHandle, CommandResult, Sandbox } from '@forgeweaver/fw-e2b'
import type {
  SubprocessCollect,
  SubprocessHandle,
  SubprocessOutcome,
...
function commandText(spec: SubprocessSpawnSpec, paths: RemotePaths): string {
  const encoder = `"$fw_e2b_env_bin" -i "$fw_e2b_node" -e ${quoteE2BShellArg(OUTPUT_ENCODER_SOURCE)}`
  const stdoutRedirect = hasSpill(spec.stdio.stdout)
    ? `> >("$fw_e2b_tee" --output-error=warn-nopipe >("$fw_e2b_head" -c ${spec.stdio.stdout.spill.maxBytes} > ${quoteE2BShellArg(paths.stdout)}) | ${encoder} 2>/dev/null)`
    : `> >(${encoder} 2>/dev/null)`
  const stderrRedirect = hasSpill(spec.stdio.stderr)
    ? `2> >("$fw_e2b_tee" --output-error=warn-nopipe >("$fw_e2b_head" -c ${spec.stdio.stderr.spill.maxBytes} > ${quoteE2BShellArg(paths.stderr)}) | ${encoder} >&2 2>/dev/null)`
  ...
  const inner = [
    ...
    'fw_e2b_rm=$7',
    'shift 7',
    'fw_e2b_pgid="$("$fw_e2b_ps" -o pgid= -p "$$" | "$fw_e2b_tr" -d " ")"',
    `printf '%s\\n' "$fw_e2b_pgid" > ${quoteE2BShellArg(paths.pid)}`,
    `mapfile -d '' -t fw_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    `"$fw_e2b_rm" -f -- ${quoteE2BShellArg(paths.environment)}`,
    `"$fw_e2b_env_bin" -i -- "\${fw_e2b_env[@]}" "$@" ${stdoutRedirect} ${stderrRedirect}`.trimEnd(),
    'fw_e2b_status=$?',
    `printf '%s\\n' "$fw_e2b_status" > ${quoteE2BShellArg(paths.status)}`,
    'wait',
    'exit "$fw_e2b_status"',
  ].join('\n')
  ...
  const bootstrap = [
    `mapfile -d '' -t fw_e2b_env < ${quoteE2BShellArg(paths.environment)}`,
    'fw_e2b_env_bin="$(command -v env)"',
    'fw_e2b_setsid="$(command -v setsid)"',
    'fw_e2b_bash="$(command -v bash)"',
    'fw_e2b_node="$(command -v node)"',
    'fw_e2b_ps="$(command -v ps)"',
    'fw_e2b_tr="$(command -v tr)"',
    'fw_e2b_tee="$(command -v tee)"',
    'fw_e2b_head="$(command -v head)"',
    'fw_e2b_rm="$(command -v rm)"',
    'for fw_e2b_tool in "$fw_e2b_env_bin" "$fw_e2b_setsid" "$fw_e2b_bash" "$fw_e2b_node" "$fw_e2b_ps" "$fw_e2b_tr" "$fw_e2b_tee" "$fw_e2b_head" "$fw_e2b_rm"; do',
    '  [[ "$fw_e2b_tool" == /* && -x "$fw_e2b_tool" ]] || exit 125',
    'done',
    `exec "$fw_e2b_env_bin" -i -- "\${fw_e2b_env[@]}" "$fw_e2b_setsid" --wait -- "$fw_e2b_bash" -c ${quoteE2BShellArg(inner)} fw-e2b "$fw_e2b_env_bin" "$fw_e2b_node" "$fw_e2b_ps" "$fw_e2b_tr" "$fw_e2b_tee" "$fw_e2b_head" "$fw_e2b_rm" ${argv}`,
  ].join('\n')
  return bootstrap
}
