...
import type { SubprocessTerminalSpawnSpec } from '@forgeweaver/fw-subprocess'
...
class FakeTerminalSandbox {
  ...
  ambient = 'KEEP=visible\0UNICODE=你好\0NPM_TOKEN=secret\0FW_STALE=old\0BROKEN\0=bad\0'
  sessionId = '123\n'
  foreground = '456\n'
  groups = [123]
  zombieGroups: number[] = []
...
function spec(overrides: Partial<SubprocessTerminalSpawnSpec> = {}): SubprocessTerminalSpawnSpec {
  return {
    ...
    cols: 80,
    graceMs: 5,
    env: { TERM: 'dumb', FW_SESSION_ID: 'owner', TOKEN_EXPLICIT: 'kept' },
    ...overrides,
  }
}
...
describe('E2B terminal allocation', () => {
  it('hides bootstrap-shell bytes and preserves requested-shell bytes across the output boundary', async () => {
    ...
    expect(fake.writes.get('/runtime/terminal-one/environment')).not.toContain('FW_STALE')
    ...
    expect(marker).toMatch(/^fw-e2b-bootstrap:/)
    expect(fake.inputs[0]?.data.toString()).not.toContain(marker)
    const runner = fake.writes.get('/runtime/terminal-one/runner.bash') ?? ''
    expect(runner).toContain('if (( ${#fw_argv[@]} == 0 )); then')
    expect(runner).toContain('printf \'%s\' "$fw_output_marker"')
    expect(runner).toContain('exec env -i -- "${fw_env[@]}" "${fw_argv[@]}"')
  ...
  it('inherits only safe ambient values and limits the allocation signal to setup', async () => {
    ...
    expect(environment).not.toContain('FW_STALE')
...
describe('E2B subprocess terminal service', () => {
  ...
  it('resolves remote executables', async () => {
    ...
    expect(commandOptions?.envs?.HOME).toMatch(/^\/\.fw-e2b-control-/)
