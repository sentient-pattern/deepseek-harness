 ...
 * `fw-acp-demo [--config path]`, defaulting to `./cordis.yml`. Shared env
 * loading, Loader guards, snapshot config selection, and settled-tree boot live
 * in fw-app-boot. Replay skips `.env` and selects sibling
 ...
 * @module @forgeweaver/fw-acp-demo/bin
 */

import { parseArgs } from 'node:util'
import { boot, installFailLoud, loadEnv, resolveConfigPath } from '@forgeweaver/fw-app-boot'

const NAME = 'fw-acp-demo'

/* v8 ignore start -- thin self-executing composition over the unit-tested
   fw-app-boot helpers; exercised end-to-end by the snapshot suite and the
   built-bin smoke */
installFailLoud(NAME)
const snapshotMode = process.env['FW_SNAPSHOT']
