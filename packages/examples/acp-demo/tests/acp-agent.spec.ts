...
import { agentEvents } from '@forgeweaver/fw-agent'
import { TOOL_ORDER_REST } from '@forgeweaver/fw-system-prompt'
import type { Message } from '@forgeweaver/fw-llm'
import { SessionId } from '@forgeweaver/fw-session'
import * as acpAgent from '../src/index.ts'

/**
 * In-process unit coverage for the @forgeweaver/fw-acp-demo composition:
...
async function mount(config: acpAgent.Config, withBash = false): Promise<Context> {
  ...
  config.persistenceRoot ??= await mkdtemp(join(tmpdir(), 'fw-acp-demo-persistence-'))
...
async function isolatedSkillsConfig(catalogDescriptionMaxLength?: number): Promise<NonNullable<acpAgent.Config['skills']>> {
  const home = await mkdtemp(join(tmpdir(), 'fw-acp-demo-skills-'))
  return {
    filesystem: { fwHome: join(home, '.fw'), agentsHome: join(home, '.agents') },
...
async function withIsolatedSkillHomes<T>(run: () => Promise<T>): Promise<T> {
  ...
  const home = await mkdtemp(join(tmpdir(), 'fw-acp-demo-default-skills-'))
  process.env.FW_HOME = join(home, '.fw')
  process.env.FW_AGENTS_HOME = join(home, '.agents')
  try {
    return await run()
  } finally {
...
describe('fw-acp-demo composition', () => {
  it('brings up the spine + persistence + the ACP bridge', async () => {
    const ctx = await mount({
      provider: 'mock',
      model: 'mock',
      persona: 'hi',
      persistenceRoot: await mkdtemp(join(tmpdir(), 'fw-acp-demo-test-')),
  ...
  it('forwards explicit project-instruction controls to the bundled spine', async () => {
    const ctx = await mount({
      ...
      model: 'mock',
      persona: 'hi',
      persistenceRoot: await mkdtemp(join(tmpdir(), 'fw-acp-demo-workspace-context-')),
      workspaceContext: false,
    })
    expect(ctx.get('agents')).toBeDefined()
    expect(ctx.get('agentLoop')).toBeDefined()
  ...
  it('forwards skill config and fwHome into agent-spine-demo', async () => {
    const skills = await isolatedSkillsConfig(6)
    const ctx = await mount({ provider: 'mock', model: 'mock', persona: 'hi', fwHome: skills.filesystem!.fwHome!, skills, workspaceContext: false })
  ...
  it('forwards maxParallelToolCalls to the bundled agent loop', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: await mkdtemp(join(tmpdir(), 'fw-acp-demo-test-parallel-')),
      skills: await isolatedSkillsConfig(),
      workspaceContext: false,
    })
  ...
  it('forwards toolOrder through agent-spine-demo to the system-prompt assembly', async () => {
    const ctx = await mount({
      ...
      persistenceRoot: await mkdtemp(join(tmpdir(), 'fw-acp-demo-test-tool-order-')),
      workspaceContext: false,
    })
