...
describe.skipIf(!existsSync(acpBin))('fw-acp-demo BUILT bin (node lib/bin.js, no tsx)', () => {
  it('boots the published bin, completes a turn, and writes default Zstandard persistence', async () => {
    ...
    child = spawn(process.execPath, [acpBin, '--config', './cordis.yml'], {
      ...
      env: {
        ...process.env,
        FW_HOME: join(consumer, '.fw'),
        FW_AGENTS_HOME: join(consumer, '.agents'),
      },
