...
- **the LLM adapter** — the bundle ships the abstract `llm` service; the leaf registers a concrete adapter on `ctx.llm` (`llm-forgeweaver`, `llm-pi-ai`, `llm-replay`).
...
Indirectly, through `fw-system-prompt`, `fw-tool-skill`, `fw-tool-bash`, `fw-tools`, and `fw-llm-retry`, plus `fw-tool-goal` and goal-round prompts when `goals` is enabled. The bundle adds no model-bound wrapper content of its own.
