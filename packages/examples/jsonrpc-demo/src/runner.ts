 ...
 *
 * @module @forgeweaver/fw-sdk-jsonrpc-demo/runner
 */
...
export async function runJsonrpcAgent(bareModuleBaseUrl?: string): Promise<void> {
  ...
  const fromEnv = process.env['FW_CORDIS_CONFIG']
  ...
  if (configPath === undefined || !existsSync(configPath)) {
    process.stderr.write(
      `usage: ${NAME} <path/to/cordis.yml> (or set FW_CORDIS_CONFIG=<path>, which wins); the config is required — there is no built-in fallback\n`,
    )
    process.exit(1)
  }
