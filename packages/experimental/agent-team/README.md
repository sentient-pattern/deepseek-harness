...
```yaml
- id: agent-team
  name: '@forgeweaver/fw-experimental-agent-team'
  config:
    maxMembers: 8
    maxTasks: 256
...
Each peer delivery adds the sender prefix plus message content to the target history. Task and roster mutations add no model tokens; their model-facing representation belongs to `@forgeweaver/fw-experimental-tool-agent-team` results.
