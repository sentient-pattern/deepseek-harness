...
```yaml
- id: tool-agent-team
  name: '@forgeweaver/fw-experimental-tool-agent-team'
  config:
    freshProvider: spawn
    forkProvider: fork
```
...
The generated [tool catalog](../../../docs/tool-catalog.md#forgeweaverfw-experimental-tool-agent-team) owns exact schemas. The adapter supplies teammate creation; quiet and waking peer delivery; roster listing, waiting, and Lead-only interruption; and task create/list/get/compare-and-set update operations.
