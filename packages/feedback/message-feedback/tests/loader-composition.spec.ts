...
import * as StorageDomain from '@forgeweaver/fw-storage-domain'
import * as StorageJson from '@forgeweaver/fw-storage-json'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@forgeweaver/fw-session-persistence-jsonl', JsonlSessionPersistence],
    ['@forgeweaver/fw-storage', Storage],
    ['@forgeweaver/fw-storage-json', StorageJson],
    ['@forgeweaver/fw-storage-domain', StorageDomain],
    ['@forgeweaver/fw-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'fw-message-feedback-loader-'))
    ...
    await writeFile(configPath, [
      ...
      "- name: '@forgeweaver/fw-storage'",
      "- name: '@forgeweaver/fw-storage-json'",
      '  config:',
      `    root: ${JSON.stringify(join(root, 'storage'))}`,
      "- name: '@forgeweaver/fw-storage-domain'",
      '  config:',
      '    backend: json',
      "- name: '@forgeweaver/fw-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
