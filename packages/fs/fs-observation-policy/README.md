...
// Load it alongside a ctx.fs provider (e.g. @forgeweaver/fw-fs-local) and the
// @forgeweaver/fw-tool-fs tools; the tools dispatch the fs/* events this plugin
...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@forgeweaver/fw-tool-fs` | model-facing schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@forgeweaver/fw-fs-observation-policy` (this) | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@forgeweaver/fw-fs` | `ctx.fs`: text IO + atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@forgeweaver/fw-fs-local` | local implementation of `ctx.fs` |

## How the gate participates

Three `fs/*` events (declared by `@forgeweaver/fw-fs`, dispatched by `@forgeweaver/fw-tool-fs`):
...
Because the plugin influences the world only through events, removing it does not break `@forgeweaver/fw-tool-fs` at a service-injection boundary: the tool falls through to the bare `ctx.fs` provider (unconditional write/edit, no observed-state). Loading it back layers the policy on. That graceful add/remove is the whole point of the event gate over a mandatory method service.
