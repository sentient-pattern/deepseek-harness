...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@forgeweaver/fw-tool-fs` | model-facing `read`/`write`/`edit` schemas + read windowing + text rendering; reads/writes/edits via `ctx.fs`, dispatches the `fs/*` events |
| policy | `@forgeweaver/fw-fs-observation-policy` | observed-state + read-before-edit + version-guarded write/edit, contributed through the `fs/*` event gate (no service) |
| provider contract | `@forgeweaver/fw-fs` (this) | `ctx.fs`: execution-world paths, text IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
| provider | `@forgeweaver/fw-fs-local` | the host-filesystem implementation |
...
Indirectly, through `fw-tool-fs`, which renders provider text and errors as bounded, retained filesystem tool results.
