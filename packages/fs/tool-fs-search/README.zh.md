...
| `timeoutMs` | `30000` | 附加到两个工具定义上的协作式工具调用预算，由 `@forgeweaver/fw-tool-call-timeout-policy` 通过 `exec.signal` 强制执行；subprocess seam 的终止升级提供硬终止。 |
...
glob 描述声明了配置的超过上限排序方式。生成的 [`glob` 和 `grep` schema](../../../docs/tool-catalog.zh.md#forgeweaverfw-tool-fs-search) 使用 `sampleOverCapGlobResults: true`；工具无条件注册。
