 ...
 * model-facing render consumes ({@link module:@forgeweaver/fw-tool-fs-search/search-core}
 ...
 * final output budget (`fw-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @forgeweaver/fw-tool-fs-search/presentation
 */
 ...
 * ({@link module:@forgeweaver/fw-tool-fs-search/grep} `formatGrepMatches`), so
