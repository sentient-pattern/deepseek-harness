 ...
 *
 * @module @forgeweaver/fw-tool-fs-search/search-core
 */
...
import { ItemRetainer, TextRetainer } from '@forgeweaver/fw-output-retention'
import type { RetainedItems } from '@forgeweaver/fw-output-retention'
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@forgeweaver/fw-subprocess'
import type { SaveTextSpill, SpillRef } from '@forgeweaver/fw-spill'
 ...
 * `@forgeweaver/fw-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`fw-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@forgeweaver/fw-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@forgeweaver/fw-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@forgeweaver/fw-tool-fs-search/presentation} `grepSearchMeta`)
