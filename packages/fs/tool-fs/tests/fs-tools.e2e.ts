...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)('fs tools with-key smoke', () => {
  it('creates, reads, then edits a file — verified on disk', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'fw-fs-e2e-'))
    ...
    const agent = ctx.agentLoop.create(SessionId('fs-e2e'), { provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash' })
  ...
  it('resolves a relative path against the per-session cwd (factory meta.cwd)', async () => {
    ...
    const configDir = await mkdtemp(join(tmpdir(), 'fw-fs-e2e-cfg-'))
    workdir = configDir
    const sessionDir = await mkdtemp(join(tmpdir(), 'fw-fs-e2e-session-'))
    try {
