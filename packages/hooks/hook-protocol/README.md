# @forgeweaver/fw-hook-protocol

English | [中文](README.zh.md)

The **shared core** of the Claude Code / Codex hook wire protocol. NOT a cordis plugin — it registers nothing and injects nothing. It is a **library** of dialect-neutral primitives the two bridge plugins (`@forgeweaver/fw-hooks-claude-code`, `@forgeweaver/fw-hooks-codex`) import so neither re-implements the identical halves of the protocol.
...
| Concern | Here (`fw-hook-protocol`) | The bridge (`fw-hooks-claude-code` / `-codex`) |
|---|---|---|
...
Indirectly, through `fw-hooks-claude-code` and `fw-hooks-codex`, which can turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
