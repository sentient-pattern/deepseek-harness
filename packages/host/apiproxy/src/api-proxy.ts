...
import { installModelSelection } from '@forgeweaver/fw-agent'
import type { Agent, ModelSelection, ModelSelectionRef, AgentOptions, AgentStatus } from '@forgeweaver/fw-agent'
...
import { createUserMessage, freezeMessage, ReasoningEffortId } from '@forgeweaver/fw-llm'
import { errorChain } from '@forgeweaver/fw-llm'
import type { ContentBlock, MessageSource } from '@forgeweaver/fw-llm'
import { isAppendSurfaceEvent, isJsonValue } from '@forgeweaver/fw-session'
import type { JsonValue, Session, SessionEvent, SessionEventMap, SessionHeader, SessionId, UserMessage } from '@forgeweaver/fw-session'
import type { SessionPersistence } from '@forgeweaver/fw-session-persistence'
import { SessionQueryError, type SessionSearchCursor } from '@forgeweaver/fw-session-query'
import { SubagentError } from '@forgeweaver/fw-subagent'
import type { SubagentListEntry as CatalogSubagentListEntry } from '@forgeweaver/fw-subagent'
import { isUserInvocable } from '@forgeweaver/fw-skill'
import type { Workspace, WorkspaceRecord } from '@forgeweaver/fw-workspace'
import {
  workspaceDomainState, workspaceRecord, WorkspaceId as brandWorkspaceId,
  WorkspaceMoveInvalidError, WorkspaceOrderInvalidError, WorkspaceUnknownSessionError,
} from '@forgeweaver/fw-workspace'
...
import type { PresetBearingSession } from '@forgeweaver/fw-agent-presets'
...
import type { SessionRawArtifact } from '@forgeweaver/fw-session-persistence'
import {
...
import type {} from '@forgeweaver/fw-jobs'
import type { JobSnapshot } from '@forgeweaver/fw-jobs'
// Type-only: resolves `ctx.get('sessionProjectionCache')` (the cold listing column).
import type {} from '@forgeweaver/fw-session-projection-cache'
// GoalError narrows domain rejections to their stable codes at the wire boundary.
import { GoalError } from '@forgeweaver/fw-goal'
import type { GoalRef as CoreGoalRef } from '@forgeweaver/fw-goal'
// Type-only edges: resolve the command-change stream and `ctx.get('skills')`.
import type {} from '@forgeweaver/fw-commands'
...
import type {} from '@forgeweaver/fw-cordis-host-runner/types'
import type {} from '@forgeweaver/fw-skill'
// The settings/credentials seams: brand guards run at this wire boundary; the
// service reads stay optional (`ctx.get`) so a composition without either
// provider still serves every other domain.
import { SettingsConflictError, settingsNamespace } from '@forgeweaver/fw-settings'
import type { SettingsDescriptor, SettingsNamespace, SettingsPathOp } from '@forgeweaver/fw-settings'
import { credentialRef } from '@forgeweaver/fw-credentials'
// Value edge: the rename impl narrows the title service's validation failure; the import also resolves `ctx.get('sessionTitle')`.
import { SessionTitleInvalidError } from '@forgeweaver/fw-session-title'
import type { CallId } from '@forgeweaver/fw-llm/brand'
import type { ScopeKey } from '@forgeweaver/fw-scope'
import type { ApprovalOutcome, ApprovalRequestId } from '@forgeweaver/fw-user-approval'
// Side-effect type import: resolves the `approval/request` waterfall and
// `ctx.get('approval')` without a value dependency on the seam (optional composition).
import type {} from '@forgeweaver/fw-user-approval'
...
} from '@forgeweaver/fw-user-questions'
import { UserQuestionError } from '@forgeweaver/fw-user-questions'
import { DirectoryPickerError } from '@forgeweaver/fw-host-directory-picker'
import {
...
} from '@forgeweaver/fw-api-remotes'
...
function projectionsUnavailableError(): RpcError {
  return {
    code: 'internal',
    message: 'subagent catalog is unavailable: this deployment does not mount the sessionProjections registry (load @forgeweaver/fw-session-projection)',
    details: {},
  }
}
...
export function createApiProxy(ctx: Context, defaults: ApiProxyDefaults): ApiProxy {
  ...
  // attachment Service Definition: fw-llm depends on fw-attachment, so the
  ...
  function goalServiceFor(agent: Agent): NonNullable<ReturnType<typeof ctx.get<'goals'>>> | { error: RpcError } {
    ...
    if (goals === undefined) {
      return { error: { code: 'internal', message: 'goal service is absent: neither this session\'s agent preset nor the host composition mounts @forgeweaver/fw-goal', details: {} } }
    }
    return goals
  }
  ...
  function settingsAbsent(): RpcError {
    return { code: 'internal', message: 'settings service is absent: this deployment does not mount a settings provider (e.g. @forgeweaver/fw-settings-file) in its composition', details: {} }
  }
  ...
  function credentialsAbsent(): RpcError {
    return { code: 'internal', message: 'credentials service is absent: this deployment does not mount a credential provider (e.g. @forgeweaver/fw-credentials-local) in its composition', details: {} }
  }
  ...
  return {
    sessions: {
      ...
      async search(request, signal) {
        ...
        if (sessionQuery === undefined) {
          return err(request, {
            code: 'internal',
            message: 'session search is unavailable: this deployment does not mount @forgeweaver/fw-session-query',
            details: {},
          })
        }
        try {
    ...
    agentPresets: {
      ...
      // Authoring is privileged (see PRIVILEGED_METHODS in fw-client-connection):
    ...
    skills: {
      ...
      async list(request) {
        ...
        // composition mounts fw-skill, not an empty catalog. `ctx.get` also
        ...
        if (skillRegistry === undefined) {
          return err(request, { code: 'internal', message: 'skill registry is absent: neither this session\'s agent preset nor the host composition mounts @forgeweaver/fw-skill', details: {} })
        }
