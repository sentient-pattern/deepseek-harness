...
import type { AttachmentStore, ImageAttachmentRef } from '@forgeweaver/fw-attachment'
import type { SessionLineageNode, SessionQueryEngine } from '@forgeweaver/fw-session-query'
import type { SessionId, SessionStore } from '@forgeweaver/fw-session'
import type { SessionPersistence, SessionRawArtifact } from '@forgeweaver/fw-session-persistence'
...
export function sessionLogZipFilename(sessionId: string): string {
  return `fw-session-${safeSessionIdSegment(sessionId)}.zip`
}
