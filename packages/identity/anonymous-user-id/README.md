# @forgeweaver/fw-anonymous-user-id

English | [中文](README.zh.md)

Shared anonymous identity for session telemetry, direct feedback acknowledgement, and ForgeWeaver provider requests. `getOrCreateAnonymousUserId()` returns a random UUID v4 scoped to one harness home, persisted as the bare line `$FW_HOME/.anonymous-user-id` (`~/.fw/.anonymous-user-id` when `FW_HOME` is unset). The OpenTelemetry backend reports it as Resource `user.id`; `/feedback` includes the same value in its acknowledgement; and `fw-llm-forgeweaver` sends it as `x-forgeweaver-harness-user-id`, allowing the receiving systems to correlate records without independently generated identities.
...
This package is a shared library, not a Cordis plugin. Consumers import `getOrCreateAnonymousUserId()` directly. Its invariant companion is intentionally empty because the package owns no event stream or public mutable relation that can be checked without creating the identity as a side effect. `FW_TELEMETRY_DISABLED` stops telemetry export only; it does not suppress direct feedback acknowledgement or the ForgeWeaver provider header.

## Model Experience

None, as the identifier reaches ForgeWeaver only as model-hidden HTTP transport metadata and never enters the request body, prompt, or model-visible content.
...
- **No cross-home identity** — different `$FW_HOME` values cannot be correlated.
- **Configured ForgeWeaver gateways receive the id** — `fw-llm-forgeweaver` sends the stable header to its resolved `baseURL`, including deployment overrides, independently of telemetry sharing mode.
