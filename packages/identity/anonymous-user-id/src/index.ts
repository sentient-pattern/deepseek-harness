 ...
 * harness home resolved by {@link resolveDshHome} (`$FW_HOME` > `~/.fw`),
 ...
 * machine: every process sharing one `$FW_HOME` reports the same id, and
 ...
 *
 * @module @forgeweaver/fw-anonymous-user-id
 */
...
export interface AnonymousUserIdOptions {
  /** Environment consulted for `FW_HOME`; defaults to `process.env`. */
