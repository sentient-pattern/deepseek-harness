...
这些包通过现有的 agent 和会话约定集成，而不改变循环。交互式应用提供具体的命令、审批和提问适配器；自动化使用 [`acp/`](../acp/README.zh.md)，可运行的演示组合包位于 [`examples/`](../examples/README.zh.md)。产品 [`fw`](../../apps/cli/README.zh.md) CLI（命令行界面）直接组合这些包。
