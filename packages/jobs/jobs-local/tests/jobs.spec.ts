...
import { bindScopeParent, createScope, scopeOf } from '@forgeweaver/fw-scope'
import type { ScopeKey } from '@forgeweaver/fw-scope'
import { JobId } from '@forgeweaver/fw-jobs'
import type { JobHooks, JobKind, JobOutcome, JobSnapshot, JobStart } from '@forgeweaver/fw-jobs'
...
describe('LocalJobRegistry.start', () => {
  ...
  it('refuses to register while no job controller serves the owner', async () => {
    ...
    expect(() => ctx.jobs.start(producer().spec))
      .toThrow('background jobs unavailable: no job controller serves this agent (load @forgeweaver/fw-tool-jobs in its composition)')
  })
