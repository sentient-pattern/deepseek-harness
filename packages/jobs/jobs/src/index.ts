 ...
 * process-local registry lives in `@forgeweaver/fw-jobs-local`.
 * @module @forgeweaver/fw-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@forgeweaver/fw-jobs is the abstract job registry seam; load an implementation such as @forgeweaver/fw-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
