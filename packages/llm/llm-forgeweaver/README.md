...
ForgeWeaver chat-completions adapter for the harness LLM seam: direct `fetch` + SSE (framed by `eventsource-parser`) translating the official wire format (source of truth: the API docs — guides/thinking_mode, guides/tool_calls, api/create-chat-completion) into the `StreamChunk` protocol.

A second, library-backed implementation of the same seam exists in `@forgeweaver/fw-llm-pi-ai`. This package owns the `forgeweaver-official` provider route — deliberately distinct from pi-ai's catalog name `forgeweaver`, so one composition can mount both ForgeWeaver paths side by side; registering another adapter for `forgeweaver-official` itself still throws `LlmError('DUPLICATE_ADAPTER')`.

The package root exposes the Cordis plugin contract and `ForgeWeaverAdapter`; wire serialization, SSE parsing, and chunk translation helpers are not part of that root contract.
...
- id: llm-forgeweaver
  ...
  config:
    apiKeyEnv: FORGEWEAVER_API_KEY  # default; resolved per request via ctx.credentials, then the environment
    baseURL: https://api.forgeweaver.com # optional; $FORGEWEAVER_BASE_URL then the public API when omitted
...
The plugin registers the single provider route `forgeweaver-official` together with its resolved `retryPolicy`; omission resolves to normal mode with five retries. A request selects it with `provider: forgeweaver-official`; its `model` is passed through as the wire `model` string, so changing ForgeWeaver models does not require lifecycle-time registration. Omitting `models` advertises `forgeweaver-v4-flash`, `forgeweaver-v4-pro`, and the image-capable `forgeweaver-v4-flash-vision-exp`, each with a 1,000,000-token context window; an explicit list replaces those defaults, while `models: []` advertises none. Catalog entries are exposed through `ctx.llm.listModels('forgeweaver-official')` for clients such as ACP editors and the Web selector, but remain advisory: unlisted model ids still pass through unchanged as text-only routes. An omitted entry name defaults to its id, and omitted `inputModalities` means `text` only.
...
The one registration-captured fact is the retry policy: when its resolved value changes, the plugin re-registers the route in place (same adapter instance, one synchronous section), so `ctx.llm.providerRetryPolicy('forgeweaver-official')` always reports the current policy.

The plugin also declares its route in the configurable-provider directory (`ctx.llm.listConfigurableProviders()`): provider `forgeweaver-official`, settings namespace `llm-forgeweaver`, empty settings path — the whole section is the profile. Configuration surfaces use that entry to offer this adapter alongside dormant pi-ai providers.

## App attribution

Every chat and Files API request carries the shared attribution header from fw-llm's `attributionHeaders()`, the mandatory `User-Agent` baseline identifying the harness (see [fw-llm § App attribution](../llm/README.md#app-attribution-attributionts)). Direct ForgeWeaver requests and OpenAI-compatible gateway requests get no provider-specific app-attribution headers under this adapter contract; OpenRouter app attribution is deferred to a future explicit OpenRouter adapter or mode. A request whose `GenerateOptions.purpose` is `compaction` (fw-compaction-basic's auxiliary summarization call) additionally carries `x-forgeweaver-harness-compact: 1`, so the host can separate compaction traffic from conversation requests.

ForgeWeaver request identity is separate from app attribution. After credential resolution, every provider request carries `x-forgeweaver-harness-user-id` with the stable anonymous id from [`@forgeweaver/fw-anonymous-user-id`](../../identity/anonymous-user-id/README.md); a request carrying `GenerateOptions.sessionId` also sends that exact value as `x-forgeweaver-harness-session-id`, while a direct call without a session omits the session header. Both headers go to the resolved `baseURL`, including a configured gateway, and remain outside the request body and model-visible content.
...
- **Reasoning passback rule**: every assistant turn that carried reasoning serializes `reasoning_content` back in history. Thinking mode requires it on tool-call turns; ForgeWeaver ignores it elsewhere, while a gateway re-encoding the conversation for another vendor recovers that turn's upstream thinking signature by hashing the replayed text.
...
The selected ForgeWeaver model receives the harness system prompt, message history, tool schemas, stop sequences, and call config. The vision model normally receives retained user and tool-result images as Files API references beside stable attachment handles and request-image dimensions; a Files resolution failure sends all retained images as inline data URLs instead. An over-budget older image is represented by the documented placeholder. Reasoning content from a prior assistant turn is passed back verbatim, whether or not that turn called a tool.
...
An unchanged assembled prefix, including deterministically encoded retained images and placeholders, is eligible for ForgeWeaver cache reuse, which this adapter reports in usage. A model-route change or any upstream prompt, schema, prefix, history, or image-budget change may prevent reuse from the first changed token; reasoning passback appends on every reasoned turn.

### ForgeWeaver response
...
- **Images are input-only durable attachments** — direct external URLs and assistant image output are not supported; ForgeWeaver input normally uses the Files API and uses inline base64 only for per-request recovery.
