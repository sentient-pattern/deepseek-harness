/**
 * `ForgeWeaverAdapter`: fetch + SSE against a ForgeWeaver (OpenAI-compatible)
 * chat-completions endpoint, emitting harness StreamChunks. The adapter is
 * transport-only: connection facts arrive through a thunk resolved once per
 * operation and the bearer token through a per-request resolver, so the
 * registering plugin owns validation, layering, and credential policy.
 *
 * @module fw-llm-forgeweaver/adapter
 */

import { attributionHeaders, contentHasImage, CONTEXT_WINDOW_EXCEEDED_CODE, isContextWindowExceededError, isQuotaExceededError, LlmAdapter, LlmError, offloadRequestImagesWithPolicy, ProviderRequestId, QUOTA_EXCEEDED_CODE, ReasoningEffortId } from '@forgeweaver/fw-llm'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import type { CredentialRef } from '@forgeweaver/fw-credentials'
import { deadline, idleWatchdog, timeoutOf } from '@forgeweaver/fw-timeout'
import type { AnonymousUserId } from '@forgeweaver/fw-anonymous-user-id'
import { serializeRequest, serializeRequestWithImages } from './serialize.ts'
import type { ImageWireLocation, RequestDefaults } from './serialize.ts'
import { ForgeWeaverFileStore } from './file-store.ts'
import type { ForgeWeaverFilePolicy } from './file-store.ts'
 ...
 */
export interface ForgeWeaverConnectionOptions {
  ...
  models: readonly ForgeWeaverCatalogModel[]
  ...
  filePolicy: ForgeWeaverFilePolicy
  /** Provider-owned model-request retry policy, already resolved. */
  retryPolicy: ResolvedRetryPolicy
}

/** Constructor options for {@link ForgeWeaverAdapter}: the operation-local resolution hooks the plugin owns. */
export interface ForgeWeaverAdapterOptions {
  /** Current validated connection facts; called once per operation. */
  options: () => ForgeWeaverConnectionOptions
  /**
   ...
   */
  resolveApiKey: (connection: ForgeWeaverConnectionOptions) => Promise<string>
  ...
  resolveFiles?: () => ForgeWeaverFileStore
}
...
/** Total-pixel budget matching ForgeWeaver's normal vision projection. */
...
const FILES_API_TIMEOUT_CODE = 'FORGEWEAVER_FILES_API_TIMEOUT'
...
class FileResolutionFailure extends Error {
  constructor(cause: unknown) {
    super('ForgeWeaver Files API could not resolve a request image.', { cause })
...
/**
 * Resolve the request-image budgets owned by one ForgeWeaver model route.
 * @param model - Advertised model route and its optional image overrides.
 * @returns Complete pixel and encoded-byte budgets.
 * @internal
 */
export function resolveRequestImagePolicy(model: ForgeWeaverCatalogModel): ImageRequestPolicy {
...
function detailNamesFileId(detail: string, fileId: ForgeWeaverFileId): boolean {
...
function normalizedImageDiagnostic(
  ...
  if (target !== undefined) {
    return `ForgeWeaver rejected normalized image ${normalizedImageFacts(target)}: ${providerMessage}. `
  ...
  return `ForgeWeaver rejected a normalized request image: ${providerMessage}. Candidate images: `
    + `${candidates.map(normalizedImageFacts).join('; ')}. `
    + 'The provider rejected bytes already normalized by the harness; PNG, JPEG, WebP, and GIF remain supported input formats.'
}

function modelInfo(provider: string, model: ForgeWeaverCatalogModel): LlmModelInfo {
  return {
    provider,
    id: model.id,
...
function requestId(headers: Headers): ReturnType<typeof ProviderRequestId> | undefined {
  const value = headers.get('x-request-id') ?? headers.get('x-forgeweaver-request-id')
 ...
 */
export class ForgeWeaverAdapter extends LlmAdapter {
  private readonly files: ForgeWeaverFileStore

  constructor(private readonly config: ForgeWeaverAdapterOptions) {
    super()
    this.files = config.resolveFiles?.() ?? new ForgeWeaverFileStore()
  }

  override providerInfo(provider: string): LlmProviderInfo {
    return { id: provider, name: 'ForgeWeaver' }
  }
  ...
  private modelInfoFor(
    connection: ForgeWeaverConnectionOptions,
    provider: string,
    model: string,
  ): LlmResolvedModelInfo {
  ...
  private async * streamWithConnection(
    ...
    if (hasImages) {
      ...
      if (model?.inputModalities?.includes('image') !== true) {
        throw new LlmError(
          `ForgeWeaver model "${options.model}" does not accept image input.`,
          'UNSUPPORTED_CONTENT',
        )
      }
      ...
      if (attachments === undefined) {
        throw new LlmError(
          'ForgeWeaver image conversion requires the durable attachment service.',
          'UNSUPPORTED_CONTENT',
        )
      }
    }
    ...
    try {
      ...
      if (timeoutOf(watchdog.signal, STREAM_IDLE_TIMEOUT_CODE) !== undefined) {
        throw new LlmError(
          `ForgeWeaver stream idle timeout after ${connection.streamIdleTimeoutMs}ms`,
          'TIMEOUT',
          { cause: error },
        )
      }
      if (options.signal?.aborted) {
        throw new LlmError('ForgeWeaver request aborted by caller', 'ABORTED', { cause: error })
      }
      if (error instanceof LlmError) throw error
      throw new LlmError(`ForgeWeaver API stream from ${connection.baseURL} failed`, 'TRANSPORT', { cause: error })
    } finally {
      consumer.abort('ForgeWeaver stream consumer stopped')
  ...
  private async * request(
    ...
    const headers = {
      ...
      ...attributionHeaders(),
      'x-forgeweaver-harness-user-id': String(userId),
      ...options.sessionId !== undefined
        ? { 'x-forgeweaver-harness-session-id': String(options.sessionId) }
        : {},
      ...options.purpose === 'compaction'
        ? { 'x-forgeweaver-harness-compact': '1' }
        : {},
    }
    ...
    while (true) {
          ...
          body = await serializeRequestWithImages(requestOptions, {
            representation: {
              ...
              resolveFileId: async (version, _block, location) => {
                ...
                let resolved: Awaited<ReturnType<ForgeWeaverFileStore['ensureUploaded']>>
                try {
      ...
      try {
        ...
        if (signal.aborted) throw error
        throw new LlmError(
          `ForgeWeaver API request to ${connection.baseURL} failed`,
          'TRANSPORT',
          { cause: error },
        )
      }
      ...
      if (!response.ok) {
        let message = `ForgeWeaver API error (HTTP ${response.status})`
        ...
        throw new LlmError(message, httpErrorCode(response.status, providerError), {
          cause: new Error(rawResponse.length > 0 ? rawResponse : `ForgeWeaver HTTP ${response.status}`),
      ...
      if (!response.body) {
        throw new LlmError('ForgeWeaver API returned no response body', 'EMPTY_RESPONSE')
      }
