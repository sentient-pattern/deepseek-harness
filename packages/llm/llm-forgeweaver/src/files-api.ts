/** OpenAI-compatible ForgeWeaver Files API transport. @module fw-llm-forgeweaver/files-api */

import { attributionHeaders, LlmError } from '@forgeweaver/fw-llm'
import type { ImageMediaType } from '@forgeweaver/fw-attachment'
import { ForgeWeaverFileId } from './file-id.ts'
import type { ForgeWeaverFileId as ForgeWeaverFileIdType } from './file-id.ts'
...
export interface ForgeWeaverFileObject {
  id: ForgeWeaverFileIdType
  bytes: number
  createdAt: number
  filename: string
  purpose: 'user_data'
...
export interface ForgeWeaverFilePage {
  data: ForgeWeaverFileObject[]
  firstId?: ForgeWeaverFileIdType
  lastId?: ForgeWeaverFileIdType
  hasMore: boolean
}

/** Files API operation failure with its HTTP status retained for recovery policy. */
export class ForgeWeaverFilesError extends LlmError {
    ...
    super(message, status === 401 || status === 403
      ...
      : status === 429
        ...
        : status >= 500
          ? 'SERVER'
          : 'FILES_API', { status })
    this.name = 'ForgeWeaverFilesError'
    this.detail = detail
  }
}
 ...
 */
export function isFilesQuotaError(error: unknown): error is ForgeWeaverFilesError {
  return error instanceof ForgeWeaverFilesError
...
function invalidResponse(operation: string): LlmError {
  return new LlmError(`ForgeWeaver Files API returned an invalid ${operation} response.`, 'INVALID_RESPONSE')
}

function parseFileObject(value: unknown, operation: string): ForgeWeaverFileObject {
...
export class ForgeWeaverFilesClient {
  ...
  private async request(path: string, init: RequestInit, signal?: AbortSignal): Promise<Response> {
    ...
    try {
      ...
      throw new LlmError(`ForgeWeaver Files API request to ${this.baseURL} failed`, 'TRANSPORT', { cause: error })
    }
    ...
    throw new ForgeWeaverFilesError(
      message ?? `ForgeWeaver Files API error (HTTP ${response.status})`,
      response.status,
      detail,
    )
  }
  ...
  async upload(input: {
    ...
    signal?: AbortSignal
  }): Promise<ForgeWeaverFileObject & { expiresAt: number }> {
    if (input.data.byteLength > MAX_FILE_UPLOAD_BYTES) {
      throw new LlmError('ForgeWeaver Files API upload exceeds 128 MiB.', 'INVALID_REQUEST')
    }
    if (!Number.isSafeInteger(input.expiresAfterSeconds)
      ...
      throw new LlmError('ForgeWeaver file expiry must be between 3600 and 2592000 seconds.', 'INVALID_REQUEST')
    }
   ...
   */
  async list(options: {
    after?: ForgeWeaverFileIdType
    limit?: number
    order?: 'asc' | 'desc'
    signal?: AbortSignal
  } = {}): Promise<ForgeWeaverFilePage> {
    ...
    return {
      ...
      ...typeof wire.first_id === 'string' ? { firstId: ForgeWeaverFileId(wire.first_id) } : {},
      ...typeof wire.last_id === 'string' ? { lastId: ForgeWeaverFileId(wire.last_id) } : {},
   ...
   */
  async retrieve(fileId: ForgeWeaverFileIdType, signal?: AbortSignal): Promise<ForgeWeaverFileObject> {
   ...
   * @param signal - request cancellation.
   */
  async delete(fileId: ForgeWeaverFileIdType, signal?: AbortSignal): Promise<void> {
