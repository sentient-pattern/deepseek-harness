...
import LlmRuntime, { createUserMessage, CallId, ReasoningEffortId, createMessage } from '@forgeweaver/fw-llm'
import type { Message, ToolSchema } from '@forgeweaver/fw-llm'
import AttachmentStore, { AttachmentId, ImageVariantId } from '@forgeweaver/fw-attachment'
import type {
...
import type { Config } from '@forgeweaver/fw-llm-forgeweaver'
 ...
 * without $FORGEWEAVER_API_KEY; the pre-release vision smoke additionally
 * requires $FORGEWEAVER_VISION_E2E=1 (see vitest.e2e.config.ts).
 */

const FLASH = 'forgeweaver-v4-flash'
const PRO = 'forgeweaver-v4-pro'
const VISION = 'forgeweaver-v4-flash-vision-exp'
const VISION_E2E_ENABLED = process.env.FORGEWEAVER_VISION_E2E === '1'
...
beforeEach(async () => {
  identityHome = await mkdtemp(join(tmpdir(), 'fw-e2e-user-id-'))
  vi.stubEnv('FW_HOME', identityHome)
})
...
async function harness(_model: string, config: Partial<Config> = {}) {
  ...
  await ctx.plugin(LlmForgeWeaver, config)
  return ctx
}
...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)('llm-forgeweaver e2e (real API)', () => {
  it.skipIf(!VISION_E2E_ENABLED)('uses the built-in official route to upload, reference, and delete one image', async () => {
    const key = process.env.FORGEWEAVER_API_KEY
    if (key === undefined) throw new Error('e2e ran without FORGEWEAVER_API_KEY')
    const baseURL = process.env.FORGEWEAVER_BASE_URL ?? LlmForgeWeaver.PUBLIC_BASE_URL
    const ctx = await harness(VISION, { baseURL })
    await ctx.plugin(E2eAttachmentStore)
    const attachments = ctx.attachments as E2eAttachmentStore
    let uploadedFile: LlmForgeWeaver.ForgeWeaverFileIdType | undefined
    ...
    const observedFetch: typeof fetch = async (input, init) => {
      ...
      if (method === 'POST' && url.pathname.endsWith('/files') && response.ok) {
        ...
        if (typeof value.id === 'string') uploadedFile = LlmForgeWeaver.ForgeWeaverFileId(value.id)
      }
      return response
    }
    vi.stubGlobal('fetch', observedFetch)
    const files = new LlmForgeWeaver.ForgeWeaverFilesClient({ baseURL, apiKey: key })
    ...
    try {
      ...
      expect(
        result.finish.kind,
        `ForgeWeaver vision result: ${JSON.stringify(result.finish)}`,
      ).toBe('stop')
