...
import { AttachmentId, ImageVariantId } from '@forgeweaver/fw-attachment'
import type { AttachmentStore, ImageAttachmentRef, RequestImageAttachment } from '@forgeweaver/fw-attachment'
import { createLaunchEnvironmentSnapshot } from '@forgeweaver/fw-launch-environment'
...
import { getOrCreateAnonymousUserId, type AnonymousUserId } from '@forgeweaver/fw-anonymous-user-id'
import { SessionId } from '@forgeweaver/fw-session'
import * as LlmForgeWeaver from '@forgeweaver/fw-llm-forgeweaver'
import { ForgeWeaverAdapter, resolveAdapterOptions } from '@forgeweaver/fw-llm-forgeweaver'
...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), 'fw-llm-forgeweaver-'))
  vi.stubEnv('FW_HOME', testHome)
})
...
async function harness(baseURL: string, config: object = {}) {
  ...
  vi.stubEnv('FORGEWEAVER_API_KEY', 'test-key')
  const ctx = new Context()
  await ctx.plugin(LlmRuntime)
  await ctx.plugin(LlmForgeWeaver, { baseURL, ...config })
  return ctx
}

/** Direct adapter over the plugin's real resolve step, with a static key. */
function adapterOf(
  config: Partial<LlmForgeWeaver.Config> & { apiKey?: string } = {},
  attachments?: AttachmentStore,
  files?: LlmForgeWeaver.ForgeWeaverFileStore,
): ForgeWeaverAdapter {
  const { apiKey, ...rest } = config
  return new ForgeWeaverAdapter({
...
function fileStoreOf(
  implementation: (...args: Parameters<LlmForgeWeaver.ForgeWeaverFileStore['ensureUploaded']>) => ReturnType<LlmForgeWeaver.ForgeWeaverFileStore['ensureUploaded']>,
) {
  const ensureUploaded = vi.fn(implementation)
  const invalidate = vi.fn(() => Promise.resolve())
  return {
    store: { ensureUploaded, invalidate } as unknown as LlmForgeWeaver.ForgeWeaverFileStore,
    ensureUploaded,
    invalidate,
  }
}

function fileReference(fileId: string): Awaited<ReturnType<LlmForgeWeaver.ForgeWeaverFileStore['ensureUploaded']>> {
  return {
    record: { fileId: LlmForgeWeaver.ForgeWeaverFileId(fileId) },
    uploaded: true,
  } as Awaited<ReturnType<LlmForgeWeaver.ForgeWeaverFileStore['ensureUploaded']>>
}
...
describe('ForgeWeaverAdapter against a mock server', () => {
  it('streams a text generation end to end through the assembler', async () => {
    ...
    // App attribution and ForgeWeaver request identity are independent wire facts.
    expect(server.headers[0]?.['user-agent']).toBe(userAgent())
    expect(server.headers[0]?.['x-forgeweaver-harness-user-id']).toBe(getOrCreateAnonymousUserId())
    expect(server.headers[0]).not.toHaveProperty('x-forgeweaver-harness-session-id')
    ...
    expect(server.headers[0]).not.toHaveProperty('x-forgeweaver-harness-compact')
  })
  ...
  it('uploads a durable image once and sends only its Files API id to the vision model', async () => {
    ...
    await drain(adapter.stream({
      ...
      model: 'forgeweaver-v4-flash-vision-exp',
      messages: [createUserMessage({
        content: [
    ...
    expect(server.fileRequests).toEqual([{
      method: 'POST',
      path: '/files',
      filename: `fw-${'a'.repeat(16)}-${'b'.repeat(8)}.png`,
      bytes: 3,
    }])
  ...
  it('falls back to one all-base64 request when Files API resolution fails', async () => {
    ...
    const adapter = adapterOf({
      baseURL: server.url,
      models: [{ id: 'forgeweaver-v4-flash-vision-exp', inputModalities: ['text', 'image'] }],
  ...
  it('falls back after the configured Files API deadline without aborting chat', async () => {
    ...
    const adapter = adapterOf({
      baseURL: 'https://forgeweaver.invalid',
  ...
  it('projects nested tool-result images with route-owned request budgets', async () => {
    ...
    await drain(adapter.stream({ provider: 'forgeweaver-official', model: 'vision-low', messages: [nested] }))
    await drain(adapter.stream({ provider: 'forgeweaver-official', model: 'vision-custom', messages: [nested] }))
  ...
  it('reuses the exact request version between agent and compaction calls', async () => {
    ...
    await drain(adapter.stream({ provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash-vision-exp', messages }))
    await drain(adapter.stream({
      provider: 'forgeweaver-official',
      model: 'forgeweaver-v4-flash-vision-exp',
      messages,
    ...
    expect(server.headers[1]?.['x-forgeweaver-harness-compact']).toBe('1')
  })
  ...
  it.each(['forgeweaver-v4-flash', 'unlisted-pass-through'])(
    ...
    async (model) => {
      ...
      const adapter = new ForgeWeaverAdapter({
  ...
  it('forwards the harness user and session ids for host-side trajectory routing', async () => {
    ...
    expect(server.headers[0]?.['x-forgeweaver-harness-session-id']).toBe('child-session')
    expect(server.headers[0]?.['x-forgeweaver-harness-user-id']).toBe(getOrCreateAnonymousUserId())
  })
  ...
  it('marks the auxiliary compaction call on the wire', async () => {
    ...
    expect(server.headers[0]?.['x-forgeweaver-harness-compact']).toBe('1')
  })
  ...
  it('uses the configured maxTokens default and preserves an explicit request cap', async () => {
    ...
    await assemble(ctx, { model: 'forgeweaver-v4-flash', messages: [], maxTokens: 8_192 })
  ...
  it('publishes only off and omits the wire effort when thinking is disabled', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'forgeweaver-v4-flash'))
  ...
  it.each([
    ...
    const ctx = await harness(server.url)
    const result = await assemble(ctx,{ model: 'forgeweaver-v4-flash', messages: [] })
  ...
  it('uses the HTTP status as the cause when an error response has no body', async () => {
    ...
    await expect(drain(adapter.stream({
      ...
      code: 'SERVER',
      cause: { message: 'ForgeWeaver HTTP 500' },
    })
  })
  ...
  it('parses a future Retry-After HTTP date and the ForgeWeaver request-id fallback', async () => {
    ...
    try {
      const server = await mockServer([{
        ...
        headers: {
          ...
          'x-forgeweaver-request-id': 'forgeweaver-503',
        },
      }])
      const ctx = await harness(server.url)
      const result = await assemble(ctx, { model: 'forgeweaver-v4-flash', messages: [] })
      expect(result.finish).toEqual({
        ...
        failure: {
          ...
          requestId: ProviderRequestId('forgeweaver-503'),
        },
      })
    } finally {
      dateNow.mockRestore()
  ...
  it('reports a transport failure with the endpoint in the message', async () => {
    ...
    expect(result.finish).toMatchObject({
      ...
      failure: {
        code: 'TRANSPORT',
        message: 'ForgeWeaver API request to http://127.0.0.1:1 failed',
      },
    })
  })
  ...
  it('throws EMPTY_RESPONSE when the response has no body', async () => {
    ...
    try {
      const iterate = async (): Promise<void> => {
        for await (const _chunk of adapter.stream({ provider: 'forgeweaver-official', model: 'm', messages: [] })) { /* drain */ }
      }
  ...
  it('classifies an abrupt body close as TRANSPORT', async () => {
    ...
    expect(result.finish.failure.message).toMatch(/^ForgeWeaver API stream from .* failed$/)
  })
  ...
  it('renders a non-Error transport rejection without losing its cause', async () => {
    ...
    try {
      ...
      await expect(drain()).rejects.toMatchObject({
        message: 'ForgeWeaver API request to https://example.invalid failed',
        code: 'TRANSPORT',
        cause: 'offline',
      })
    } finally {
  ...
  it('keeps an idle provider read alive through SSE comments', async () => {
    ...
    try {
      const chunks: string[] = []
      const drain = (async () => {
        for await (const chunk of adapter.stream({ provider: 'forgeweaver-official', model: 'm', messages: [] })) {
...
describe('plugin registration and config', () => {
  it('keeps wire helpers off the package root', () => {
    for (const helper of [
      ...
      'mapUsage',
      'translate',
    ]) expect(LlmForgeWeaver).not.toHaveProperty(helper)
  })

  it('registers the forgeweaver provider and unregisters on dispose (HMR safety)', async () => {
    const server = await mockServer([])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    const fiber = await ctx.plugin(LlmForgeWeaver, {
      baseURL: server.url,
    })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'forgeweaver-official', name: 'ForgeWeaver' }])
  ...
  it('registers retryPolicy from the provider config', async () => {
    ...
    expect(ctx.llm.providerRetryPolicy('forgeweaver-official')).toEqual({
      mode: 'always',
      initialDelayMs: 25,
      maxDelayMs: 100,
      jitterRatio: 0.2,
  ...
  it('owns the forgeweaver provider and advertises the default models', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmForgeWeaver, { baseURL: 'http://127.0.0.1:1' })
    expect(ctx.llm.listProviders()).toEqual([{ id: 'forgeweaver-official', name: 'ForgeWeaver' }])
    await expect(ctx.llm.listModels('forgeweaver-official')).resolves.toEqual([
      { provider: 'forgeweaver-official', id: 'forgeweaver-v4-flash', name: 'ForgeWeaver-V4-Flash', inputModalities: ['text'] },
      { provider: 'forgeweaver-official', id: 'forgeweaver-v4-pro', name: 'ForgeWeaver-V4-Pro', inputModalities: ['text'] },
      { provider: 'forgeweaver-official', id: 'forgeweaver-v4-flash-vision-exp', name: 'ForgeWeaver-V4-Flash-Vision-Exp', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'forgeweaver-v4-flash'))
      .resolves.toMatchObject({
        provider: 'forgeweaver-official',
    ...
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'forgeweaver-v4-flash-vision-exp'))
      .resolves.toMatchObject({
        provider: 'forgeweaver-official',
        id: 'forgeweaver-v4-flash-vision-exp',
        name: 'ForgeWeaver-V4-Flash-Vision-Exp',
        inputModalities: ['text', 'image'],
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it.each(['off', 'low', 'max'] as const)('uses the configured %s reasoning default', async (effort) => {
    ...
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'unlisted-pass-through'))
      .resolves.toMatchObject({
        reasoning: {
          efforts: [
            { id: ReasoningEffortId('off'), name: 'Off' },
  ...
  it.each(['low', 'high', 'max'] as const)(
    ...
    async (reasoningEffort) => {
      const ctx = new Context()
      await ctx.plugin(LlmRuntime)
      await expect(ctx.plugin(LlmForgeWeaver, {
        baseURL: 'http://127.0.0.1:1',
        thinking: 'disabled',
        reasoningEffort,
  ...
  it('accepts disabled thinking with off at the resolver boundary', async () => {
    ...
    await expect(adapter.resolveModel('forgeweaver-official', 'pass-through')).resolves.toMatchObject({
      reasoning: {
        efforts: [{ id: ReasoningEffortId('off'), name: 'Off' }],
        defaultEffort: ReasoningEffortId('off'),
      },
  ...
  it('uses the default model catalog when apply is called directly', async () => {
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    LlmForgeWeaver.apply(ctx, { baseURL: 'http://127.0.0.1:1' })
    await expect(ctx.llm.listModels('forgeweaver-official')).resolves.toEqual([
      { provider: 'forgeweaver-official', id: 'forgeweaver-v4-flash', name: 'ForgeWeaver-V4-Flash', inputModalities: ['text'] },
      { provider: 'forgeweaver-official', id: 'forgeweaver-v4-pro', name: 'ForgeWeaver-V4-Pro', inputModalities: ['text'] },
      { provider: 'forgeweaver-official', id: 'forgeweaver-v4-flash-vision-exp', name: 'ForgeWeaver-V4-Flash-Vision-Exp', inputModalities: ['text', 'image'] },
  ...
  it('defaults an adapter-supplied catalog entry to text input', async () => {
    ...
    await expect(adapter.listModels('forgeweaver-official')).resolves.toEqual([{
      provider: 'forgeweaver-official',
      id: 'adapter-model',
      name: 'adapter-model',
  ...
  it('advertises configured models without restricting arbitrary request ids', async () => {
    ...
    await expect(ctx.llm.listModels('forgeweaver-official')).resolves.toEqual([
      { provider: 'forgeweaver-official', id: 'private-fast', name: 'private-fast', inputModalities: ['text'] },
      { provider: 'forgeweaver-official', id: 'private-reasoner', name: 'Private Reasoner', description: 'Higher reasoning budget', inputModalities: ['text', 'image'] },
    ])
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'private-fast'))
      .resolves.toMatchObject({ context: { contextWindow: 32_000 } })
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'private-reasoner'))
      .resolves.toMatchObject({
        name: 'Private Reasoner',
        description: 'Higher reasoning budget',
        inputModalities: ['text', 'image'],
      })
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'arbitrary-unlisted'))
      .resolves.toMatchObject({
        context: { contextWindow: 1_000_000 },
        defaultMaxTokens: 256_000,
      })
  ...
  it('uses exact model capacity before the adapter-wide default', async () => {
    ...
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'inherits-default'))
      .resolves.toMatchObject({ context: { contextWindow: 256_000 } })
    await expect(ctx.llm.resolveModelInfo('forgeweaver-official', 'exact-override'))
  ...
  const invalidModels: Array<[LlmForgeWeaver.ForgeWeaverCatalogModel[], RegExp]> = [
    ...
    [[{
      id: 'm',
      inputModalities: ['audio'] as unknown as NonNullable<LlmForgeWeaver.ForgeWeaverCatalogModel['inputModalities']>,
    }], /expected "text" \| "image"/],
  ...
  const invalidProgrammaticModalities: Array<[LlmForgeWeaver.ForgeWeaverCatalogModel[], RegExp]> = [
  ...
  it('prefers a model\'s own output cap over the profile default', async () => {
    ...
    await expect(adapter.resolveModel('forgeweaver-official', 'capped'))
      .resolves.toMatchObject({ defaultMaxTokens: 512 })
    await expect(adapter.resolveModel('forgeweaver-official', 'uncapped'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
    await expect(adapter.resolveModel('forgeweaver-official', 'not-in-catalog'))
      .resolves.toMatchObject({ defaultMaxTokens: 4096 })
  })
  ...
  it('falls back to FORGEWEAVER_API_KEY and FORGEWEAVER_BASE_URL env vars', async () => {
    vi.stubEnv('FORGEWEAVER_API_KEY', 'env-key')
    vi.stubEnv('FORGEWEAVER_BASE_URL', 'http://127.0.0.1:1')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmForgeWeaver, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'forgeweaver-official', name: 'ForgeWeaver' }])
  ...
  it('loads keyless, keeps the catalog browsable, and fails the request actionably', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([{ id: 'forgeweaver-official', name: 'ForgeWeaver' }])
    await expect(ctx.llm.listModels('forgeweaver-official')).resolves.toHaveLength(3)
    const first = await assemble(ctx, { model: 'forgeweaver-v4-flash', messages: [] })
    expect(first.finish).toMatchObject({ kind: 'error', failure: { code: 'MISSING_CREDENTIAL' } })
    // The guidance leads with the managed credential store.
    const second = await assemble(ctx, { model: 'forgeweaver-v4-flash', messages: [] })
    ...
    expect(second.finish.failure.message)
      .toMatch(/store FORGEWEAVER_API_KEY through the credentials service.*export FORGEWEAVER_API_KEY/s)
  })
  ...
  it('reads the ambient variable when no credentials seam is mounted', async () => {
    ...
    vi.stubEnv('FORGEWEAVER_API_KEY', 'ambient-key')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmForgeWeaver, { baseURL: server.url })
    await assemble(ctx, { model: 'forgeweaver-v4-flash', messages: [] })
  ...
  it('prefers explicit config over env for key and base URL', async () => {
    vi.stubEnv('FORGEWEAVER_API_KEY', 'env-key')
    vi.stubEnv('FORGEWEAVER_BASE_URL', 'http://env-host:1')
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
  ...
  it('uses FORGEWEAVER_BASE_URL when config omits baseURL', async () => {
    const server = await mockServer([{ kind: 'sse', events: textEvents }])
    vi.stubEnv('FORGEWEAVER_BASE_URL', server.url)
    vi.stubEnv('FORGEWEAVER_API_KEY', 'test-key')
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmForgeWeaver, {})
  ...
  it('takes FORGEWEAVER_BASE_URL from any environment layer, with explicit config still on top', () => {
    const trusted = createLaunchEnvironmentSnapshot([
      { source: 'user-env', path: '/home/.fw/.env', values: { FORGEWEAVER_BASE_URL: 'https://user.example' } },
    ])
    ...
    const project = createLaunchEnvironmentSnapshot([
      { source: 'project-env', path: '/work/.env', values: { FORGEWEAVER_BASE_URL: 'https://project.example' } },
    ])
    ...
    const shell = createLaunchEnvironmentSnapshot([
      { source: 'process', values: { FORGEWEAVER_BASE_URL: 'https://stale.example' } },
    ])
  ...
  it('defaults to the public base URL without config or env', async () => {
    vi.stubEnv('FORGEWEAVER_API_KEY', 'k')
    vi.stubEnv('FORGEWEAVER_BASE_URL', undefined)
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    // Registration succeeds; no call is made (would hit api.forgeweaver.com).
    await ctx.plugin(LlmForgeWeaver, {})
    expect(ctx.llm.listProviders()).toEqual([{ id: 'forgeweaver-official', name: 'ForgeWeaver' }])
  })
  ...
  it('adapter is constructible directly for embedding over the shared resolver', async () => {
    const adapter = adapterOf()
    expect(adapter).toBeInstanceOf(ForgeWeaverAdapter)
    ...
    await expect(adapter.listModels('forgeweaver-official')).resolves.toHaveLength(3)
  })
  ...
  it('resolves connection facts and the credential exactly once per stream call', async () => {
    ...
    const adapter = new ForgeWeaverAdapter({ options, resolveApiKey, resolveUserId })
