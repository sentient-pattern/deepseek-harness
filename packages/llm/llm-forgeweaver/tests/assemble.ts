...
import { BlockAssembler } from '@forgeweaver/fw-llm'
import type { Context } from '@forgeweaver/cordis'
import type { FinishReason, GenerateOptions, Message, TokenUsage } from '@forgeweaver/fw-llm'
...
export async function assemble(ctx: Context, options: Omit<GenerateOptions, 'provider'> & { provider?: string }): Promise<AssembledResult> {
  ...
  const request = { provider: 'forgeweaver-official', ...options }
