...
import LlmRuntime, { createUserMessage, INVALID_CREDENTIAL_CODE } from '@forgeweaver/fw-llm'
...
async function home(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'fw-llm-dynamic-'))
 ...
 * llm-forgeweaver over one temp harness home. `watch: false` keeps every change
...
function prompt(ctx: Context) {
  return assemble(ctx, { model: 'forgeweaver-v4-flash', messages: [] })
}
...
describe('request-level dynamic configuration', () => {
  ...
  it('advertises a live settings catalog without re-registration', async () => {
    ...
    await expect(ctx.llm.listModels('forgeweaver-official')).resolves.toEqual([
      { provider: 'forgeweaver-official', id: 'settings-model', name: 'From Settings', inputModalities: ['text', 'image'] },
    ])
  })
  ...
  it('applies changed request file limits to the next request', async () => {
    ...
    await assemble(ctx, { model: 'forgeweaver-v4-flash-vision-exp', messages })
  ...
  it('re-registers the route in place when the captured retry policy changes, without an empty-registry window', async () => {
    ...
    expect(observed).toEqual([['forgeweaver-official']])
  })
  ...
  it('keeps the last good options when a settings snapshot fails beyond-schema validation', async () => {
    ...
    await expect(ctx.llm.listModels('forgeweaver-official')).resolves.toEqual([
      { provider: 'forgeweaver-official', id: 'recovered', name: 'recovered', inputModalities: ['text'] },
    ])
  })
  ...
  it('keeps the whole last-good snapshot when a rejected one changed the URL', async () => {
    ...
    vi.stubEnv('FORGEWEAVER_API_KEY', 'good-key')
  ...
  it('falls back to the composition entry when settings detach', async () => {
    vi.stubEnv('FORGEWEAVER_API_KEY', '')
    const dir = await home()
    await writeFile(join(dir, '.credentials.yaml'), 'version: 1\nrefs:\n  FORGEWEAVER_API_KEY: steady-key\n', { mode: 0o600 })
    const serverA = await mockServer([{ kind: 'sse', events: textEvents }])
    const serverB = await mockServer([{ kind: 'sse', events: textEvents }])
