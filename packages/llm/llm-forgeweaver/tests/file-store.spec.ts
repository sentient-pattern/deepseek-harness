...
import type { ImageAttachmentRef, RequestImageAttachment } from '@forgeweaver/fw-attachment'
import { ForgeWeaverFileStore, MAX_CHAT_IMAGE_BYTES } from '../src/file-store.ts'
import { ForgeWeaverFileId } from '../src/file-id.ts'
import { deepSeekFileScope, ForgeWeaverUploadIndex } from '../src/upload-index.ts'
...
const CONNECTION = { baseURL: 'https://api.forgeweaver.com', apiKey: 'key' }
...
describe('ForgeWeaverFileStore', () => {
  it('singleflights the first upload and reuses the durable mapping across store instances', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'fw-file-store-'))
    const index = new ForgeWeaverUploadIndex(join(dir, 'index.json'))
    const remote = uploadFetch()
    const first = new ForgeWeaverFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
    ...
    const resumed = new ForgeWeaverFileStore({ index, now: () => NOW, fetch: remote.fetchImpl })
  ...
  it('keeps a shared upload alive while another waiter remains', async () => {
    ...
    const store = new ForgeWeaverFileStore({ index, now: () => NOW, fetch: fetchImpl })
  ...
  it('normalizes a non-Error cancellation reason', async () => {
    ...
    const store = new ForgeWeaverFileStore({
      index: new ForgeWeaverUploadIndex(join(dir, 'index.json')),
      now: () => NOW,
      fetch: fetchImpl,
    })
    const controller = new AbortController()
    ...
    await expect(upload).rejects.toMatchObject({
      message: 'ForgeWeaver file upload cancelled with a non-Error reason.',
      cause: 'cancelled',
    })
  })
  ...
  it('starts a fresh upload while the cancelled transport is settling', async () => {
    ...
    const fetchImpl = vi.fn((_url: string | URL | Request, init?: RequestInit) => {
      ...
      return Promise.resolve(new Response(JSON.stringify({
        ...
        filename: 'fw-retry.png', purpose: 'user_data',
        expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
      }), { status: 200 }))
    }) as typeof fetch
    const store = new ForgeWeaverFileStore({
  ...
  it('rejects a request version above the chat per-image limit before transport', async () => {
    ...
    const store = new ForgeWeaverFileStore({ now: () => NOW, fetch: fetchImpl })
  ...
  it('rejects an upload response whose byte count differs from the request version', async () => {
    ...
    const fetchImpl = vi.fn(() => Promise.resolve(new Response(JSON.stringify({
      ...
      filename: 'fw-wrong.png', purpose: 'user_data',
      expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
  ...
  it.each([
    ...
    const remote = uploadFetch()
    const store = new ForgeWeaverFileStore({
      index: new ForgeWeaverUploadIndex(join(dir, `${extension}.json`)),
      now: () => NOW,
  ...
  it('normalizes a non-Error failure from the durable upload index', async () => {
    ...
    const store = new ForgeWeaverFileStore({ index, now: () => NOW, fetch: vi.fn() as typeof fetch })

    await expect(store.ensureUploaded(VERSION, CONNECTION, POLICY)).rejects.toMatchObject({
      message: 'ForgeWeaver file upload failed with a non-Error reason.',
  ...
  it('reuses local expires_at above the refresh margin and uploads again at the margin', async () => {
    ...
    const store = new ForgeWeaverFileStore({ index, now: () => now, fetch: remote.fetchImpl })
  ...
  it('removes a losing upload and keeps the winning durable mapping when duplicate cleanup fails', async () => {
    ...
    vi.spyOn(index, 'commit').mockResolvedValue({
      ...
      record: {
        ...
        fileId: ForgeWeaverFileId('file-api-winner'),
        bytes: 3,
        createdAt: NOW,
    ...
    const fetchImpl = vi.fn((input: string | URL | Request, init?: RequestInit) => {
      if (init?.method === 'POST') {
        ...
        return Promise.resolve(new Response(JSON.stringify({
          ...
          filename: 'fw-recovered.png', purpose: 'user_data',
          expires_at: NOW / 1_000 + POLICY.expiresAfterSeconds,
        }), { status: 200 }))
      }
      if (init?.method === 'DELETE') {
      ...
      return Promise.resolve(new Response(JSON.stringify({
        ...
        data: [{
          ...
          filename: 'fw-old.png', purpose: 'user_data',
        }],
  ...
  it('stops pagination when a page omits or repeats its cursor', async () => {
    ...
    for (const mode of ['missing', 'repeated'] as const) {
      ...
      const store = new ForgeWeaverFileStore({
        index: new ForgeWeaverUploadIndex(join(dir, `${mode}.json`)),
        now: () => NOW,
        fetch: fetchImpl,
      })
