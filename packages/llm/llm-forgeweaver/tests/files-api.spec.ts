...
import { userAgent } from '@forgeweaver/fw-llm'
import { ForgeWeaverFileId } from '../src/file-id.ts'
import {
  ForgeWeaverFilesClient,
  ForgeWeaverFilesError,
  isFilesQuotaError,
...
describe('ForgeWeaverFilesClient', () => {
  it('uploads multipart bytes with the required purpose and explicit expiry', async () => {
    ...
    const client = new ForgeWeaverFilesClient({ baseURL: 'https://api.forgeweaver.com/', apiKey: 'key', fetch: fetchImpl })
    ...
    await expect(client.upload({
      ...
      id: ForgeWeaverFileId('file-api-one'),
      bytes: 3,
  ...
  it('validates list, retrieve, and delete responses', async () => {
    ...
    await expect(client.list({ after: ForgeWeaverFileId('file-api-before'), limit: 20, order: 'desc' })).resolves.toMatchObject({
      data: [{ id: 'file-api-one' }], firstId: 'file-api-one', lastId: 'file-api-one', hasMore: false,
    })
    await expect(client.retrieve(ForgeWeaverFileId('file-api-one'))).resolves.toMatchObject({ id: 'file-api-one' })
    await expect(client.delete(ForgeWeaverFileId('file-api-one'))).resolves.toBeUndefined()
  })
  ...
  it.each([
    ...
    const client = new ForgeWeaverFilesClient({
      baseURL: 'https://api.forgeweaver.com',
      apiKey: 'key',
      fetch: vi.fn(() => Promise.resolve(new Response('not-json', { status }))),
    })
    await expect(client.retrieve(ForgeWeaverFileId('missing'))).rejects.toMatchObject({
      name: 'ForgeWeaverFilesError',
      code,
      detail: '',
    })
  })
  ...
  it.each([
    ...
    const error = await client.retrieve(ForgeWeaverFileId('missing')).catch((caught: unknown) => caught)
    expect(error).toBeInstanceOf(ForgeWeaverFilesError)
    expect(error).toMatchObject({ message: 'ForgeWeaver Files API error (HTTP 400)', detail: '' })
  })
  ...
  it('wraps transport failures but preserves an aborted request reason', async () => {
    ...
    await expect(client.retrieve(ForgeWeaverFileId('one'))).rejects.toMatchObject({
      code: 'TRANSPORT',
      cause: transport,
    })
    ...
    await expect(client.retrieve(ForgeWeaverFileId('one'), controller.signal)).rejects.toBe(transport)
  })
  ...
  it.each([
    ...
    await expect(client.retrieve(ForgeWeaverFileId('one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
  ...
  it('accepts a list without cursors and uses the global fetch default', async () => {
    ...
    try {
      const client = new ForgeWeaverFilesClient({ baseURL: 'https://api.forgeweaver.com///', apiKey: 'key' })
  ...
  it.each([
    ...
    await expect(client.delete(ForgeWeaverFileId('file-api-one'))).rejects.toMatchObject({ code: 'INVALID_RESPONSE' })
  })
})
