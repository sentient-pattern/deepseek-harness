...
describe('ForgeWeaverUploadIndex', () => {
  it('normalizes trailing endpoint slashes in the credential scope', () => {
    expect(deepSeekFileScope('https://api.forgeweaver.com///', 'key'))
      .toBe(deepSeekFileScope('https://api.forgeweaver.com', 'key'))
  })

  it('isolates API-key namespaces and reuses only records above the refresh margin', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'fw-upload-index-'))
    const index = new ForgeWeaverUploadIndex(join(dir, 'index.json'))
    const first = deepSeekFileScope('https://api.forgeweaver.com', 'first-key')
    const second = deepSeekFileScope('https://api.forgeweaver.com', 'second-key')
    const record = {
      scope: first,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: ForgeWeaverFileId('file-api-one'),
      bytes: 3,
      createdAt: 1_000,
      expiresAt: 10_000,
    }
  ...
  it('keeps a reusable cross-process winner and removes only an exact generation', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'fw-upload-index-'))
    const index = new ForgeWeaverUploadIndex(join(dir, 'index.json'))
    const scope = deepSeekFileScope('https://api.forgeweaver.com', 'key')
    const first = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: ForgeWeaverFileId('file-api-first'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
    const duplicate = { ...first, fileId: ForgeWeaverFileId('file-api-duplicate') }
  ...
  it('treats a corrupt upload cache as empty and repairs it on the next commit', async () => {
    ...
    const index = new ForgeWeaverUploadIndex(path)
    const scope = deepSeekFileScope('https://api.forgeweaver.com', 'key')
    const record = {
      scope,
      attachmentId: ATTACHMENT,
      variantId: VARIANT,
      fileId: ForgeWeaverFileId('file-api-repaired'),
      bytes: 3,
      createdAt: 1,
      expiresAt: 10_000,
    }
  ...
  it.each([
    ...
    await expect(index.get(
      deepSeekFileScope('https://api.forgeweaver.com', 'key'), VARIANT, 1, 1,
  ...
  it('rejects duplicate persisted mappings as a corrupt cache', async () => {
    ...
    const record = {
      scope, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: ForgeWeaverFileId('file-api-one'), bytes: 3, createdAt: 1, expiresAt: 10_000,
    }
  ...
  it('drops expired records on commit and clears only the selected namespace', async () => {
    const dir = await mkdtemp(join(tmpdir(), 'fw-upload-index-'))
    const index = new ForgeWeaverUploadIndex(join(dir, 'index.json'))
    const first = deepSeekFileScope('https://api.forgeweaver.com', 'first')
    const second = deepSeekFileScope('https://api.forgeweaver.com', 'second')
    const expired = {
      scope: first, attachmentId: ATTACHMENT, variantId: VARIANT,
      fileId: ForgeWeaverFileId('file-api-expired'), bytes: 3, createdAt: 1, expiresAt: 2,
    }
    const live = {
      ...expired, scope: second, fileId: ForgeWeaverFileId('file-api-live'), expiresAt: 10_000,
    }
