 ...
 *
 * @module fw-llm-pi-ai/replay
 */

import { LlmError } from '@forgeweaver/fw-llm'
import type { Message, ModelMessageSource, ReplayEnvelope } from '@forgeweaver/fw-llm'
...
function foreignAssistant(message: Message): AssistantMessage {
  ...
  return {
    ...
    api: 'fw-foreign',
    provider: source?.provider ?? 'fw-foreign',
    model: source?.model ?? 'fw-foreign',
    usage: emptyPiUsage(),
