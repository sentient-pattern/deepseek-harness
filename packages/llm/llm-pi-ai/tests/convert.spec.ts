...
import type { AttachmentStore, ImageAttachmentRef, ImageRequestPolicy, RequestImageAttachment } from '@forgeweaver/fw-attachment'
import { createUserMessage, CallId, CONTEXT_WINDOW_EXCEEDED_CODE, EMPTY_RESPONSE_CODE, createMessage } from '@forgeweaver/fw-llm'
import type { ContentBlock, StreamChunk } from '@forgeweaver/fw-llm'
...
describe('toPiContext', () => {
  ...
  it('degrades the flat pre-envelope replay state a legacy session log carries', () => {
    ...
    expect(context.messages[0]).toMatchObject({ role: 'assistant', api: 'fw-foreign' })
