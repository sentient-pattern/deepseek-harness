...
import LlmRuntime, { createMessage, createUserMessage } from '@forgeweaver/fw-llm'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'fw-pi-composition-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@forgeweaver/fw-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
