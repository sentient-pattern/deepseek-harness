...
import AuthorizationService from '@forgeweaver/fw-authorization'
import type { AuthorizationInteraction, AuthorizationNotice, AuthorizationPrompt } from '@forgeweaver/fw-authorization'
import LocalCredentialProvider from '@forgeweaver/fw-credentials-local'
import type { CredentialKey } from '@forgeweaver/fw-credentials'
...
async function harness(): Promise<Context> {
  const dir = await mkdtemp(join(tmpdir(), 'fw-pi-login-'))
  dirs.push(dir)
