...
The executor has no policy config. Multi-provider adapters such as `fw-llm-pi-ai` place `retryPolicy` inside each provider profile, avoiding a second provider-name list.
