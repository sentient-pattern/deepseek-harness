...
/**
 * Keyless built-artifact smoke: plain Node imports `@forgeweaver/fw-lsp` and
 * `@forgeweaver/fw-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@forgeweaver/fw-lsp')
      const LspLocal = await import('@forgeweaver/fw-lsp-stdio')
      const { default: LocalFileSystem } = await import('@forgeweaver/fw-fs-local')
