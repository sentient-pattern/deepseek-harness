...
| 包 | 职责 |
|---|---|
| `@forgeweaver/fw-lsp`（本包） | Service Definition：服务、以品牌化 id + 扩展名映射为 key 的提供方注册表、逐查询选择、请求／结果词汇、`LspError` 分类体系 |
| `@forgeweaver/fw-lsp-stdio` | Service Provider：通用本地后端，注册已配置的 stdio 语言服务器提供方 |
| `@forgeweaver/fw-tool-lsp` | Consumer：面向模型的 `lsp` 工具，基于 `ctx.lsp` |
...
提供方注册的是**能力**而非工具。`fw-tool-lsp` 是面向模型的名称、描述、提示词指引、schema 和呈现的唯一 owner。
...
通过 `fw-tool-lsp` 间接影响；该工具拥有面向模型的 `lsp` schema、提示词与渲染结果，本注册表自身不贡献提示词或 schema。

#### KV Cache 影响

不会直接失效；请求前缀变更由 `fw-tool-lsp` 负责。
