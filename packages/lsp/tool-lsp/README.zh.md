...
| `timeoutMs` | `60000` | 由 `fw-tool-call-timeout-policy` 强制执行的工具调用超时预算；覆盖完整的排队打开／查询／关闭生命周期，且模型不可配置。 |
...
模型会看到生成的 [`lsp` schema](../../../docs/tool-catalog.zh.md#forgeweaverfw-tool-lsp)。
