/**
 * End-to-end tests for fw-mcp-client. Exercises the REAL MCP protocol against:
...
import { apply } from '@forgeweaver/fw-mcp-client/src/index.ts'
import { publicToolName } from '@forgeweaver/fw-mcp-client/src/tools.ts'
...
async function mountImageRegistry(fwHome: string): Promise<Context> {
  const ctx = await mountRegistry()
  await ctx.plugin(LocalAttachmentStore, { fwHome })
