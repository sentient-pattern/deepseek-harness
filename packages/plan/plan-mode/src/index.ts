 ...
 *
 * @module @forgeweaver/fw-plan-mode
 */
...
import type { CommandId } from '@forgeweaver/fw-commands'
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('fw-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'fw-plan-mode: close service lifetime')
