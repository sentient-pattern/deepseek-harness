 ...
 * @module @forgeweaver/fw-agent-presets
 */

import { stat } from 'node:fs/promises'
import { Context, Service } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import { bindScopeParent, createScope, scopeOf, type Scope, type ScopeKey, type ScopeParentBinding } from '@forgeweaver/fw-scope'
// Type-only: resolves the `agent/created` lifecycle event this service watches.
import type {} from '@forgeweaver/fw-agent'
import { settingsNamespace, type SettingsScope, type default as SettingsService } from '@forgeweaver/fw-settings'
...
export class AgentPresets extends Service {
  ...
  constructor(ctx: Context, public config: Config) {
    ...
    this.resolvedRoots = config.includeUserRoot
      ? [...config.roots, { path: fwHomePath(USER_PRESET_DIR), trust: 'user' }]
      : [...config.roots]
   ...
   * scope key. The binding is fw-scope's only re-link capability; holding it
  ...
  private async ensureStanding(preset: AgentPreset): Promise<StandingMount> {
    ...
    if (pending !== undefined) {
      ...
      // it is gone. The subtree is not inert — `fw-skill-filesystem` watches its
