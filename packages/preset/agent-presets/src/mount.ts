 ...
 * @module @forgeweaver/fw-agent-presets/mount
 */
...
import type { EntryTree } from '@forgeweaver/cordis-plugin-loader'
import { scopeOf, scopeParentOf, type ScopeKey } from '@forgeweaver/fw-scope'
...
class PresetTree extends Include {
   ...
   * dependencies, so every `@forgeweaver/fw-*` row would fail to import. The
 ...
 * invariant companion's service listener, and `fw-invariants` is a
