...
beforeEach(async () => {
  userRoot = await mkdtemp(join(tmpdir(), 'fw-preset-authoring-'))
  ctx = new Context()
...
describe('a deployment with more than one user root', () => {
  it('refuses to delete a preset the writable root does not own', async () => {
    const second = await mkdtemp(join(tmpdir(), 'fw-preset-second-'))
...
describe('a user root that does not exist yet', () => {
  it('is created by the first copy', async () => {
    const absent = join(await mkdtemp(join(tmpdir(), 'fw-preset-absent-')), 'nested', 'preset')
