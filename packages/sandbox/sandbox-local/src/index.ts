 ...
 * @module @forgeweaver/fw-sandbox-local
 */
...
} from '@forgeweaver/node-addon-landlock-run'
import { Context } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import { assertNever } from '@forgeweaver/fw-llm'
import { SandboxProvider, SandboxUnavailableError } from '@forgeweaver/fw-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@forgeweaver/fw-sandbox'
import type { SessionId } from '@forgeweaver/fw-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@forgeweaver/fw-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@forgeweaver/fw-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@forgeweaver/fw-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@forgeweaver/fw-sandbox-windows-acl/src/runner.ts'))
