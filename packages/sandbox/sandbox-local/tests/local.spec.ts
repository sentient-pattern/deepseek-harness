...
import { LAUNCHER_FAILURE_EXIT } from '@forgeweaver/node-addon-landlock-run'
import { SANDBOX_UNAVAILABLE, SandboxUnavailableError } from '@forgeweaver/fw-sandbox'
import type { SandboxPolicy } from '@forgeweaver/fw-sandbox'
import {
  LocalSandboxProvider,
} from '@forgeweaver/fw-sandbox-local'
import type { Config } from '@forgeweaver/fw-sandbox-local'
...
function absentRunnerEntry(): string {
  return join(mkdtempSync(join(tmpdir(), 'fw-absent-acl-entry-')), 'runner.js')
}
...
function fakeLauncher(report = 'landlock: fully enforced'): string {
  const dir = mkdtempSync(join(tmpdir(), 'fw-fake-landlock-'))
...
function fakeSeatbeltExec(status: number): string {
  const dir = mkdtempSync(join(tmpdir(), 'fw-fake-seatbelt-'))
...
describe('the platform chains', () => {
  ...
  // live in @forgeweaver/fw-sandbox-windows-acl/tests/provider-chain.spec.ts
...
describe('probeTimeoutMs config', () => {
  ...
  it('bounds the default probes: a launcher slower than the configured timeout reads as unusable', async () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'fw-slow-landlock-'))
    const launcher = join(dir, 'landlock-run')
...
describe('the windows-acl probe (runner invocation contract)', () => {
  ...
  it('prefers the built lib/runner.js entry when the resolved file exists', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'fw-fake-acl-entry-'))
