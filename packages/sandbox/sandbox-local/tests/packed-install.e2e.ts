...
const platformPackageName = `@forgeweaver/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @forgeweaver peer (koffi arrives
  // from the registry).
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'fw-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'fw-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'fw-packed-work-'))
