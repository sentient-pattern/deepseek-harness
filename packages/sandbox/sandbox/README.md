...
Implementations: [`@forgeweaver/fw-sandbox-local`](../sandbox-local/) (Linux: `bwrap`, else the per-platform Landlock launcher; macOS: `sandbox-exec`/Seatbelt). Consumers: [`@forgeweaver/fw-bash-sandbox`](../../shell/bash-sandbox/) (wraps `['bash', '-c', command]`).
...
Through [`fw-bash-sandbox`](../../shell/bash-sandbox/README.md) and [`fw-tool-bash`](../../shell/tool-bash/README.md), failure to enforce a requested mode produces code `SANDBOX_UNAVAILABLE` and the exact error below. An execution-time runner failure adds ` Runner failure: <detail>`.
