...
实现：[`@forgeweaver/fw-sandbox-local`](../sandbox-local/)（Linux：`bwrap`，否则使用相应平台的 Landlock launcher；macOS：`sandbox-exec`／Seatbelt）。消费方：[`@forgeweaver/fw-bash-sandbox`](../../shell/bash-sandbox/)（包装 `['bash', '-c', command]`）。
...
通过 [`fw-bash-sandbox`](../../shell/bash-sandbox/README.zh.md) 和 [`fw-tool-bash`](../../shell/tool-bash/README.zh.md)，无法强制执行所请求模式时会产生错误码 `SANDBOX_UNAVAILABLE` 及以下精确错误。执行期 runner 失败会追加 ` Runner failure: <detail>`。
