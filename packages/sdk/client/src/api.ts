/**
 * High-level run API over {@link HarnessClient}: `ForgeWeaverHarness` owns one
 ...
 * Mirrors the Python SDK's `ForgeWeaverHarness`/`Session` pair.
 *
 * @module @forgeweaver/fw-sdk-client/api
 */
...
import type { ContentBlock, ForgeWeaverHarnessOptions, HarnessClientOptions, HarnessNotification, RunResult } from './types.ts'

/**
 * Reusable SDK for running ForgeWeaver agent turns in a runtime
 ...
 * child is reaped.
 */
export class ForgeWeaverHarness implements AsyncDisposable {
  ...
  constructor(options: ForgeWeaverHarnessOptions) {
    ...
    this.provider = options.provider ?? 'forgeweaver-official'
    this.model = options.model ?? 'forgeweaver-v4-flash'
...
export class HarnessSession {
   ...
   */
  constructor(readonly harness: ForgeWeaverHarness, readonly id: string) {}
