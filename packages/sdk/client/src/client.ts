/**
 * Low-level JSON-RPC client for a ForgeWeaver SDK runtime subprocess.
 * {@link HarnessClient} owns the child process: it spawns the runtime, speaks
 * the `@forgeweaver/fw-sdk-protocol` wire over the child's stdio, fans
 ...
 * spawns directly rather than through the `fw-subprocess` service — the
 * seam's documented exception for SDK-managed transports.
 *
 * @module @forgeweaver/fw-sdk-client/client
 */
...
/**
 * JSON-RPC client for the ForgeWeaver SDK runtime over subprocess stdio.
 *
...
export class HarnessClient {
   ...
   */
  start(): void {
    if (this.closeTask !== undefined) throw new TransportClosedError('ForgeWeaver runtime client is closed')
    ...
    child.once('error', (error) => {
      ...
      this.failSubscriptions(this.closedError('ForgeWeaver runtime failed to start'))
    })
    ...
    child.once('exit', (code) => {
      ...
      settled.exited = true
      maybeSettle()
      this.failSubscriptions(this.closedError('ForgeWeaver runtime exited'))
    })
    child.once('close', () => {
  ...
  async request(method: string, params?: object, timeoutMs?: number): Promise<unknown> {
    ...
    if (this.exitCode !== undefined || this.spawnError !== undefined) {
      ...
      throw this.closedError('ForgeWeaver runtime is not running')
    }
    const transport = this.transport
    /* v8 ignore next -- start() either sets the transport or throws */
    if (transport === undefined) throw new TransportClosedError('ForgeWeaver runtime is not running')
    ...
    try {
      ...
      const timer = setTimeout(() => {
        abandon.abort(new RequestTimeoutError(`${method} timed out after ${timeout}ms waiting for the ForgeWeaver runtime`))
      }, timeout)
      try {
  ...
  subscribe(filter?: NotificationFilter): NotificationSubscription {
    ...
    if (this.closeTask !== undefined || this.exitCode !== undefined || this.spawnError !== undefined) {
      subscription.fail(this.closedError('ForgeWeaver runtime closed'))
      return subscription
    }
  ...
  private async performClose(): Promise<void> {
    ...
    this.transport?.close()
    this.failSubscriptions(this.closedError('ForgeWeaver runtime closed'))
  }
