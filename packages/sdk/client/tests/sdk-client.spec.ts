...
function harnessWith(env: Record<string, string> = {}, extra: LaunchOverrides = {}): ForgeWeaverHarness {
  const harness = new ForgeWeaverHarness({ launch: fakeLaunch(env, extra) })
...
describe('ForgeWeaverHarness', () => {
  it('ignores notifications that precede the submitted message receipt', async () => {
    ...
    } as unknown as ForgeWeaverHarness
  ...
  it('resolves a relative launch cwd to an absolute workspace before the handshake', async () => {
    ...
    const dir = await mkdtemp(join(process.cwd(), '.fw-sdk-client-relcwd-'))
    cleanups.push(() => rm(dir, { recursive: true, force: true }))
    const recordFile = join(dir, 'init.jsonl')
  ...
  it('supports await using disposal', async () => {
    let captured: ForgeWeaverHarness
    {
      await using harness = new ForgeWeaverHarness({ launch: fakeLaunch() })
      captured = harness
...
describe('HarnessClient', () => {
  ...
  it('fails fast when the command does not exist', async () => {
    const client = new HarnessClient({ command: join(tmpdir(), 'fw-no-such-runtime-bin') })
    cleanups.push(() => client.close())
  ...
  it('delivers notifications to unfiltered and filtered subscriptions in wire order', async () => {
    ...
    expect(identity.serverInfo.name).toBe('forgeweaver-harness-sdk-runtime')
