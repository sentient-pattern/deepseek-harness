/**
 * Named wire types for the ForgeWeaver SDK runtime protocol: the three
 * request/result pairs and the four server-to-client notification payloads
 * exchanged over the newline-delimited JSON-RPC stdio transport. The server
 * plugin (`@forgeweaver/fw-sdk-jsonrpc-server`) and SDK clients share these shapes;
 * `serverInfo.name` stays the wire-stable `forgeweaver-harness-sdk-runtime`.
 *
 * @module @forgeweaver/fw-sdk-protocol/types
 */

import type { ContentBlock } from '@forgeweaver/fw-llm'
import type { SessionEvent } from '@forgeweaver/fw-session'
import type { SubagentStopReason } from '@forgeweaver/fw-subagent'
...
export interface InitializeResult {
  /** Wire-stable server identity (`forgeweaver-harness-sdk-runtime`) and version. */
