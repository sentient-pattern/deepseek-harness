...
describe('fw-sdk-jsonrpc-server plugin apply', () => {
  it('serves initialize over the injected stdio pair', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'fw-jsonrpc-apply-init-'))
    vi.stubEnv('FORGEWEAVER_API_KEY', 'test-key')
    const harness = await mountPlugin(storageDir)
    try {
      harness.send({ jsonrpc: '2.0', id: 'init-1', method: 'initialize', params: { cwd: storageDir, provider: 'forgeweaver-official', model: 'apply-model' } })

      const response = await harness.waitForFrame(frame => frame.id === 'init-1', 'initialize response')
      expect(response).toEqual({
        jsonrpc: '2.0',
        id: 'init-1',
        result: { serverInfo: { name: 'forgeweaver-harness-sdk-runtime', version: '0.0.1' } },
      })
  ...
  it('does not answer initialize until async sibling Loader entries settle', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'fw-jsonrpc-apply-readiness-'))
    vi.stubEnv('FORGEWEAVER_API_KEY', 'test-key')
    ...
    try {
      const initialize = {
        ...
        id: 'init-delayed',
        method: 'initialize',
        params: { cwd: storageDir, provider: 'forgeweaver-official', model: 'apply-model' },
      }
      ...
      expect(response).toMatchObject({
        id: 'init-delayed',
        result: { serverInfo: { name: 'forgeweaver-harness-sdk-runtime' } },
      })
    } finally {
      release()
  ...
  it('drives a session/prompt turn end-to-end and forwards session notifications as output frames', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'fw-jsonrpc-apply-prompt-'))
    const llmServer = await mockCompletionServer()
    vi.stubEnv('FORGEWEAVER_API_KEY', 'test-key')
    vi.stubEnv('FORGEWEAVER_BASE_URL', llmServer.url)
    const harness = await mountPlugin(storageDir)
    try {
      harness.send({ jsonrpc: '2.0', id: 1, method: 'initialize', params: { cwd: storageDir, provider: 'forgeweaver-official', model: 'dsagent-model' } })
  ...
  it('answers shutdown before exiting 0 exactly once, even against a racing second shutdown', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'fw-jsonrpc-apply-shutdown-'))
    ...
    try {
      ...
      harness.send({ jsonrpc: '2.0', id: 'after-exit', method: 'initialize', params: { cwd: storageDir, provider: 'forgeweaver-official', model: 'x' } })
      await settle()
  ...
  it('still disposes and exits once when the flush callback fails', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'fw-jsonrpc-apply-flush-failure-'))
    ...
    try {
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'after-flush-failure', method: 'initialize', params: { cwd: storageDir, provider: 'forgeweaver-official', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
    } finally {
      await harness.dispose()
  ...
  it('stops serving on a bare fiber dispose (HMR-style unload) without calling exit', async () => {
    const storageDir = await mkdtemp(join(tmpdir(), 'fw-jsonrpc-apply-dispose-'))
    const harness = await mountPlugin(storageDir)
    try {
      ...
      expect(error.error).toMatchObject({
        code: -32603,
        message: 'unknown ForgeWeaver SDK runtime method: nope/unknown',
      })
      ...
      const before = harness.frames().length
      harness.send({ jsonrpc: '2.0', id: 'probe-2', method: 'initialize', params: { cwd: storageDir, provider: 'forgeweaver-official', model: 'x' } })
      await settle()
      expect(harness.frames().length).toBe(before)
      expect(harness.exits()).toEqual([])
    } finally {
