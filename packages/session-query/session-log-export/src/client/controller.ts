...
import { createSnapshotStore, type SessionId, type SnapshotStore } from '@forgeweaver/fw-client-runtime/client'
...
export function sessionLogZipFilename(sessionId: SessionId): string {
  return `fw-session-${String(sessionId).replace(/[^A-Za-z0-9_-]/g, '_')}.zip`
}
...
function hostBase(): string {
  ...
  return origin !== undefined && origin !== 'null' ? origin : 'http://fw.internal'
}
