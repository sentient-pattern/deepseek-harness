 ...
 *
 * @module @forgeweaver/fw-session-query-sqlite/tests/load-path
 */

import { afterEach, describe, expect, it } from 'vitest'
import { Context } from '@forgeweaver/cordis'
import Loader from '@forgeweaver/cordis-plugin-loader'
import { SESSION_FORMAT_VERSION, SessionId } from '@forgeweaver/fw-session'
...
import SqliteSessionQueryEngine, * as queryModule from '@forgeweaver/fw-session-query-sqlite'
...
async function temporaryPath(name: string): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'fw-session-search-loader-'))
  temporaryDirectories.push(directory)
  return join(directory, name)
}

describe('fw-session-query-sqlite real Loader path', () => {
