import { createAssistantMessage, createUserMessage } from '@forgeweaver/fw-llm'
...
import SessionStore, { SESSION_FORMAT_VERSION, SessionId } from '@forgeweaver/fw-session'
import type { SessionEvent, SessionHeader, SessionId as SessionIdType } from '@forgeweaver/fw-session'
import SessionPersistence, { SessionPersistenceRevision } from '@forgeweaver/fw-session-persistence'
import type { SessionPersistenceSnapshot } from '@forgeweaver/fw-session-persistence'
import SqliteSessionPersistence from '@forgeweaver/fw-session-persistence-sqlite'
import SqliteSessionQueryEngine, {
  SESSION_QUERY_SQLITE_SCHEMA_VERSION,
} from '@forgeweaver/fw-session-query-sqlite'
import {
...
async function temporaryPath(name = 'search.db'): Promise<string> {
  const directory = await mkdtemp(join(tmpdir(), 'fw-session-search-'))
