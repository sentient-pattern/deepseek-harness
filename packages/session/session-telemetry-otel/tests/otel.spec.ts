...
import { recordFeedback } from '@forgeweaver/fw-command-feedback'
...
// pin FW_HOME to a temp dir so the suite never touches the ambient ~/.fw.
let tempHome: string
let previousDshHome: string | undefined
beforeAll(() => {
  tempHome = mkdtempSync(join(tmpdir(), 'fw-otel-home-'))
  previousDshHome = process.env.FW_HOME
  process.env.FW_HOME = tempHome
})
afterAll(() => {
...
describe('OpenTelemetrySessionBackend wire', () => {
  it('ships session records and the ops shutdown marker through the real SDK pipeline', async () => {
    ...
    expect(resource).toContainEqual({ key: 'service.name', value: { stringValue: 'forgeweaver-harness' } })
    expect(resource).toContainEqual({ key: 'user.id', value: { stringValue: getOrCreateAnonymousUserId() } })

    const records = allRecords(captures)
    const ledger = records.filter(r => r.scope === '@forgeweaver/fw-session-telemetry-otel')
    const ops = records.filter(r => r.scope === '@forgeweaver/fw-session-telemetry-otel/ops')
  ...
  it('drains records enqueued after a timer export began: dispose during an in-flight batch', async () => {
    ...
    const ops = allRecords(captures).filter(r => r.scope === '@forgeweaver/fw-session-telemetry-otel/ops')
    expect(ops).toHaveLength(1)
    expect(ops[0]!.record.attributes).toContainEqual({ key: 'telemetry.op', value: { stringValue: 'shutdown' } })
  })
...
describe('fw-session-telemetry-otel real-load-path guard', () => {
