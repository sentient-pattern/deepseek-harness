...
import * as FirstMessageTitleProvider from '@forgeweaver/fw-session-title-first-prompt-llm'
...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)('first-prompt title provider with real ForgeWeaver API', () => {
  it('replaces the fallback with a short model title', async () => {
    const ctx = new Context()
    contexts.push(ctx)
    await ctx.plugin(LlmRuntime)
    await ctx.plugin(LlmForgeWeaver, { thinking: 'disabled' })
    ...
    expect(title).toMatchObject({
      ...
      source: {
        ...
        model: { provider: 'forgeweaver-official', model: 'forgeweaver-v4-flash' },
      },
    })
