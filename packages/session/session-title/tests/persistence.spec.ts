...
import SessionTitleService, { foldSessionTitle } from '@forgeweaver/fw-session-title'
...
describe('session title persistence round trips', () => {
  it('round-trips through a remounted JSONL backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'fw-title-jsonl-'))
    roots.push(root)
  ...
  it('round-trips through a remounted SQLite backend', async () => {
    const root = await mkdtemp(join(tmpdir(), 'fw-title-sqlite-'))
    roots.push(root)
