...
| `fwHome` | `path` 省略时使用的 harness home | `$FW_HOME` 或 `~/.fw` |
...
- **Host 配置适配器会收到解析后的路径。** `ctx.settings.documentPath` 是 `resolveSpec()` 得出的绝对文件名，包括自定义 YAML/JSON 路径；`prepareDocument()` 会保留现有文件，或在 Host 打开文档前，以仅属主可访问的权限独占创建缺失的空文件。浏览器只收到可用性标志，绝不重建 `$FW_HOME`，也绝不提交文件系统目标。
