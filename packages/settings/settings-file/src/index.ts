 ...
 * leaf-level diff.
 * @module @forgeweaver/fw-settings-file
 */
...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@forgeweaver/fw-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveDshHome(config.fwHome), 'settings.yaml'))
