...
这是使用沙箱能力的 [`@forgeweaver/fw-shell`](../shell/) 执行器 seam 的 Service Provider。加载它时，应**用它替代** `@forgeweaver/fw-bash-local`，并同时加载 [`ctx.sandbox`](../../sandbox/sandbox/) 提供方（例如 [`@forgeweaver/fw-sandbox-local`](../../sandbox/sandbox-local/)）及 [`ctx.sandboxPolicy`](../../sandbox/sandbox-policy/)；默认模式和工作区根目录由后者负责，并与受沙箱约束的文件系统共享这些设置。无需使用替代工具插件；`fw-tool-bash` 会检测执行器的 `sandboxMode` 能力并添加升权字段。
...
- **只限制文件影响。** 模式词汇只声称文件影响。网络仍不受限制；进程可见性因后端而异，具体见 [`fw-sandbox-local`](../../sandbox/sandbox-local/)。
- 进程机制（spawn、进程组终止、输出收集／spill、后台句柄、凭证清理）继承自 [`fw-bash-local`](../bash-local/)；runner 选择位于 [`fw-sandbox-local`](../../sandbox/sandbox-local/)。

该 seam 只报告拒绝：拒绝是一项结果事实，本执行器绝不自行协商权限。批准问题位于工具层（`fw-tool-bash`），由它设置本包所遵守的模式覆盖值。
...
基线是生成的 [`fw-tool-bash` schema](../../../docs/tool-catalog.zh.md#forgeweaverfw-tool-bash)。通过公布表明启用隔离的 `sandboxMode` 能力，此后端会为 `bash` 增加 `sandbox_permissions`，其 enum 为 `workspace-write` | `danger-full-access`，并增加 `justification`。策略归属方会另行贡献当前且不区分具体能力的 `sandbox:policy` 上下文。

#### Token 影响

在 `bash` 可见的请求上，schema 固定增加少量内容，另有一条由 `fw-sandbox-policy` 负责的当前策略子句。
...
如果没有 runner 能强制执行受限模式，前台调用会传播 [`SANDBOX_UNAVAILABLE` 错误](../../sandbox/sandbox/README.zh.md#confinement-error-indirectly)；该错误由 `fw-sandbox` 定义。判定为 runner 失败的 spawn 错误会以原始 spawn 错误作为详细信息；如果拒绝没有通过 `ENOENT`／`EACCES` 的 `path` 或 `syscall` 证据指明 `argv[0]`，它仍是普通的命令启动错误。已结算的 runner 失败则以匹配到的致命 stderr 行作为详细信息，并保留原始 stderr 收集结果。如果追加了 `Runner failure: <detail>`，它就是权威诊断；前面的后端安装文本只是通用的 `SANDBOX_UNAVAILABLE` 前缀。
