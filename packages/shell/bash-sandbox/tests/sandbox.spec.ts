...
import type { ShellRunResult, CollectedOutput } from '@forgeweaver/fw-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@forgeweaver/fw-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@forgeweaver/fw-sandbox'
...
import type { Config } from '@forgeweaver/fw-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'fw-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'FW_WRAP=1', 'bash', '-c', 'printf "%s" "$FW_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$FW_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'fw-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.FW_ORDER_FILE, "runner\\n");',
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'fw-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'fw-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
