 ...
 *
 * @module @forgeweaver/fw-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors fw-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@forgeweaver/fw-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@forgeweaver/fw-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@forgeweaver/fw-subprocess'
import { installSettingsSection } from '@forgeweaver/fw-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@forgeweaver/fw-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of fw-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `fw-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.fwEnv },
    }
  }
   ...
   * `fw-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@forgeweaver/fw-pwsh-sandbox`.
