/**
 * Real-process tests for `@forgeweaver/fw-pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@forgeweaver/fw-pwsh-local'
import LocalSubprocessRuntime from '@forgeweaver/fw-subprocess-local'
import SubprocessRuntime from '@forgeweaver/fw-subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@forgeweaver/fw-subprocess'
import { MAX_TIMER_DELAY_MS } from '@forgeweaver/fw-timeout'
import type { ShellProcess } from '@forgeweaver/fw-shell'

const spillDir = mkdtempSync(join(tmpdir(), 'fw-pwsh-exec-spec-'))
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'fw-pwsh-resolve-'))
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'fw-pwsh-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'fw-pwsh-resolve-dir-'))
    const store = join(dir, 'store')
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'fw-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'fw-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-fw' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/fwEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:FW_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      fwEnv: { FW_SEAM_VAR: 'fw-ok' },
    })
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:FW_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      fwEnv: { FW_BG_VAR: 'bg-fw-env' },
    }))
    const partialOutput = await readUntil(proc, '[bg-env][bg-fw-env]')
    await proc.done
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-fw' }))
