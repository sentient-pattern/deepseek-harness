 ...
 * @module @forgeweaver/fw-tool-bash-persistent
 */
...
import type { TerminalReadResult, TerminalSessionId } from '@forgeweaver/fw-terminal'
...
function markers(): CommandMarkers {
  ...
  return {
    start: `__FW_PERSISTENT_BASH_START_${nonce}__`,
    end: `__FW_PERSISTENT_BASH_END_${nonce}:`,
  }
}
...
function wrapCommand(command: string, marker: CommandMarkers): string {
  ...
  return `printf '%s\\n' ${quoteForBash(marker.start)}; eval -- ${quoteForBash(command)}; __fw_persistent_bash_status=$?; printf '%s%s\\n' ${quoteForBash(marker.end)} "$__fw_persistent_bash_status"`
}
