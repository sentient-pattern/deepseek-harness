 ...
 * @module @forgeweaver/fw-tool-bash
 */
...
import { ESCALATION_TARGETS, approveEscalation, canonicalPath, validateEscalationArgs } from '@forgeweaver/fw-sandbox'
...
function bashDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a bash command (`bash -c`) and return its stdout/stderr. '
    ...
    + `Current harness environment facts are exposed through managed \`$${FW_ENV_PREFIX}*\` variables; inspect them when needed. `
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args: BashToolArgs, exec) {
      ...
      const fwEnv = ctx.shellEnv.collect(exec)
      const request = {
