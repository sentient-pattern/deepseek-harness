...
The model-facing `pwsh` tool registered over the `ctx.shell` executor seam. Intended for Windows compositions where a PowerShell executor (e.g. `@forgeweaver/fw-pwsh-local`) backs `ctx.shell`; the tool contract is PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables. Behavior mirrors `fw-tool-bash` call-for-call — foreground and `run_in_background` execution through the generic job runtime, the managed `FW_*` environment through the shared `shell-env` registry, the sandbox denial rendering with the same-turn `sandbox_permissions` escalation surface, and the bash marker/truncation rendering story (a clean exit produces no marker).
...
Every foreground and background model pwsh call receives a freshly collected trusted `FW_*` environment through the shared [`fw-shell-env`](../shell-env/) registry: `FW_HOME` (the absolute Harness home), `FW_SHELL=1`, the agent's `FW_SESSION_ID`, and `FW_SESSION_JSONL` when the active persistence backend locates one. Plugins contributing `FW_*` facts to `ctx.shellEnv` apply to pwsh calls exactly as they do to bash calls. The snapshot passes through the dedicated `ShellExecRequest.fwEnv` channel; `process.env` is never modified. The description teaches the generic `$env:FW_*` convention rather than naming persistence-specific variables.
...
The model sees the generated [`pwsh` schema](../../../docs/tool-catalog.md#forgeweaverfw-tool-pwsh). Agent-scoped tool restrictions can remove the definition for that agent.
...
- **No persistent shell** — every call starts a fresh `pwsh -Command`; the persistent-shell counterpart is [`@forgeweaver/fw-tool-pwsh-persistent`](../tool-pwsh-persistent/README.md), which keeps one owner-scoped pwsh alive across calls on Windows (ConPTY) and POSIX hosts with pwsh.
