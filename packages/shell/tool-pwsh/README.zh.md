...
注册在 `ctx.shell` 执行器 seam 之上的面向模型的 `pwsh` 工具。面向由 PowerShell 执行器（如 `@forgeweaver/fw-pwsh-local`）支撑 `ctx.shell` 的 Windows 组合；工具约定是 PowerShell 方言：原生 `C:\...` 路径与 `$env:NAME` 变量。行为与 `fw-tool-bash` 逐调用对齐——通过通用任务运行时执行前台与 `run_in_background`、通过共享 `shell-env` 注册表管理 `FW_*` 环境、sandbox 拒绝渲染与同轮次 `sandbox_permissions` 升级面、以及 bash 的 marker/截断渲染故事（干净退出不产生 marker）。
...
每次前台与后台模型 pwsh 调用都会通过共享的 [`fw-shell-env`](../shell-env/) 注册表收到一份新收集的受信任 `FW_*` 环境：`FW_HOME`（Harness 主目录绝对路径）、`FW_SHELL=1`、agent 的 `FW_SESSION_ID`，以及活跃持久化后端定位到 JSONL 时的 `FW_SESSION_JSONL`。向 `ctx.shellEnv` 贡献 `FW_*` 事实的插件对 pwsh 调用与 bash 调用一视同仁。快照通过专用的 `ShellExecRequest.fwEnv` 通道传递；`process.env` 永不被修改。描述只教授通用的 `$env:FW_*` 约定，而不是点名持久化相关的变量。
...
模型看到生成的 [`pwsh` schema](../../../docs/tool-catalog.zh.md#forgeweaverfw-tool-pwsh)。按 agent 作用域的工具限制可以移除该 agent 的定义。
...
- **无持久 shell** — 每次调用都启动全新的 `pwsh -Command`；持久 shell 对应物是 [`@forgeweaver/fw-tool-pwsh-persistent`](../tool-pwsh-persistent/README.zh.md)，它在 Windows（ConPTY）以及装有 pwsh 的 POSIX 主机上跨调用保持一个 owner 作用域的 pwsh 存活。
