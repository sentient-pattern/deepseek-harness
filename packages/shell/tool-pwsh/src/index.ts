 ...
 * `@forgeweaver/fw-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `fw-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `FW_*` environment through the
 ...
 * exit-status parse from `@forgeweaver/fw-shell`.
 *
 * @module @forgeweaver/fw-tool-pwsh
 */

import { isAbsolute, resolve as resolvePath } from 'node:path'
import type { Context } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import { defineTool, TOOL_ABORTED } from '@forgeweaver/fw-tools'
import type { GenericCallView, TerminalCallView, ToolExecution, ToolResult, ToolResultView } from '@forgeweaver/fw-tools'
...
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@forgeweaver/fw-sandbox'
import type { SandboxPolicyService } from '@forgeweaver/fw-sandbox-policy'
import type { ShellRunResult } from '@forgeweaver/fw-shell'
import { parseExitStatus } from '@forgeweaver/fw-shell'
...
/* jscpd:ignore-start -- minimal mirror of fw-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:FW_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror fw-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of fw-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  /* jscpd:ignore-start -- deliberate mirror of fw-tool-bash's escalation resolver (pwsh-tool-and-executor Agent Note). */
  /**
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of fw-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors fw-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with fw-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors fw-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        fwEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @forgeweaver/fw-jobs and @forgeweaver/fw-tool-jobs')
        }
