/**
 * Integration tests: the REAL `@forgeweaver/fw-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@forgeweaver/fw-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@forgeweaver/fw-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'fw-tool-pwsh-'))
