 ...
 * managed `FW_*` collection, abort translation, canonical result projection,
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@forgeweaver/fw-shell'
...
import type { ShellProcessRead } from '@forgeweaver/fw-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, fwHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, fwHome === undefined ? {} : { fwHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, fwHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed FW_* environment', async () => {
    const fwHome = mkdtempSync(join(tmpdir(), 'fw-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, fwHome)
    ...
    expect(request?.fwEnv).toEqual({
      FW_HOME: fwHome,
      FW_SHELL: '1',
      FW_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const fwEnv = bash.requests[0]?.fwEnv
    expect(fwEnv).toBeDefined()
    expect(fwEnv?.['FW_SHELL']).toBe('1')
    expect(fwEnv?.['FW_HOME']).toEqual(expect.any(String))
    expect(fwEnv).not.toHaveProperty('FW_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'fw-tool-pwsh-policy-'))
