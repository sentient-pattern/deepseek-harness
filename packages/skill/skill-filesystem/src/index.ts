 ...
 *
 * @module @forgeweaver/fw-skill-filesystem
 */
...
import type Schema from '@forgeweaver/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@forgeweaver/fw-fs'
import { canonicalizeWatchPath, resolveDshHome } from '@forgeweaver/fw-home-paths'
import {
  BUNDLED_SKILL_RANK,
  isSkillName,
  type SkillCandidate,
  ...
  type SkillSource,
} from '@forgeweaver/fw-skill'

const PROJECT_FW_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_FW_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.FW_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.FW_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.fw/skills'), source: 'project-fw', rank: PROJECT_FW_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.fwHome, 'skills'), source: 'user-fw', rank: USER_FW_RANK, skipSystem: true },
