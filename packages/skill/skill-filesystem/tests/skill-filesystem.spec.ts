...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@forgeweaver/fw-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `fw-${name}-`)))
}
...
describe('fw-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.fw/skills'), 'same', 'user fw skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.fw/skills'), 'same', 'project fw skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.fw/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project fw skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-fw')
    ...
    await writeSkill(join(noGit, '.fw/skills'), 'fallback-root', 'Fallback root')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.fw/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.fw/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.fw/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.fw/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.fw/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.fw/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.fw/skills/broken-link'))
    await symlink('/dev/null', join(home, '.fw/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { fwHome: join(home, '.fw'), agentsHome: join(home, '.agents'), watch: false })
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.fw/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.fw/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    const previousAgentsHome = process.env.FW_AGENTS_HOME
    const previousBundledSkillDir = process.env.FW_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.FW_HOME = join(envHome, '.fw')
      process.env.FW_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.FW_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.fw/skills'), 'env-skill', 'Env skill')
      ...
      process.env.FW_HOME = join(envHome, 'empty-fw')
      delete process.env.FW_BUNDLED_SKILL_DIR
      process.env.FW_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { fwHome: join(envHome, 'empty-fw') }).name).toBe('filesystem')
    } finally {
      if (previousDshHome === undefined) {
        delete process.env.FW_HOME
      } else {
        process.env.FW_HOME = previousDshHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.FW_AGENTS_HOME
      } else {
        process.env.FW_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.FW_BUNDLED_SKILL_DIR
      } else {
        process.env.FW_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
