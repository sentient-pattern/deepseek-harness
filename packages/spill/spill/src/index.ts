 ...
 * `@forgeweaver/fw-spill-local` (host filesystem) is the first.
 *
 * The Service Definition is deliberately minimal: `saveText` and nothing else. It owns NO
 * retention policy (that is `@forgeweaver/fw-output-retention`), NO tool-result
 * replacement (that is `@forgeweaver/fw-spill-policy`), and NO retrieval or
 * search API. The backend supplies the locator and retrieval hint appropriate
 * for its storage substrate.
 *
 * @module @forgeweaver/fw-spill
 */
