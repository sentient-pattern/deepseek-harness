...
| [`subagent-fw-sdk/`](subagent-fw-sdk/README.md) | Starts an out-of-process Harness child through the TypeScript SDK | registers on `ctx.subagents` |
...
The Codex and Claude Code packages are independent optional Profile Bundles. Install either or both with `fw plugin --profile <name> add @forgeweaver/fw-subagent-codex @forgeweaver/fw-subagent-claude-code`, then restart that Profile; each package registers only its dormant Host provider. To grant a tool, copy a complete Agent Preset, remove `disabled` from each matching tool row, and start a new Session. Removing one package withdraws only that provider and its private runtime closure on the next Profile start.
