...
function deepSeekBaseUrl(): string {
  ...
  if (configured !== OFFICIAL_FORGEWEAVER_BASE_URL) {
    throw new Error('Claude Code ForgeWeaver e2e requires the official ForgeWeaver base URL')
  }
  return configured
}
...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)(
  'Claude Code provider with real ForgeWeaver API',
  () => {
    it('returns one unique nonce through the production provider and real SDK/CLI', async () => {
      ...
      const root = mkdtempSync(join(tmpdir(), 'fw-claude-forgeweaver-e2e-'))
      roots.push(root)
      ...
      const env = {
        ...
        ANTHROPIC_MODEL: 'forgeweaver-v4-pro[1m]',
        ANTHROPIC_DEFAULT_OPUS_MODEL: 'forgeweaver-v4-pro[1m]',
        ANTHROPIC_DEFAULT_SONNET_MODEL: 'forgeweaver-v4-pro[1m]',
        ANTHROPIC_DEFAULT_HAIKU_MODEL: 'forgeweaver-v4-flash',
        CLAUDE_CODE_SUBAGENT_MODEL: 'forgeweaver-v4-flash',
      ...
      const nonce = `FW_CLAUDE_FORGEWEAVER_${randomUUID()}`
      const parent = {
