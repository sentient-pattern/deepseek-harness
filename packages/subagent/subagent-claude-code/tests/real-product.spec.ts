...
const settingsModel = 'fw-settings-inheritance-marker'
const fakeKey = 'fw-fake-anthropic-key'
...
async function realInstanceFixture(
  ...
  const root = mkdtempSync(join(tmpdir(), 'fw-claude-code-real-'))
  roots.push(root)
...
describe('real Claude Agent SDK 0.3.220 and its distributed Claude Code 2.1.220 fixture', {
  ...
  it('maps a real SDK max-turns result to safe query-run facts', async () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-claude-code-max-turns-'))
    roots.push(root)
  ...
  it('overrides interactive settings, denies a write, and returns a safe diagnostic', async () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-claude-code-denied-target-'))
    roots.push(root)
  ...
  it('runs an explicitly selected bypass write in the isolated workspace', async () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-claude-code-bypass-target-'))
    roots.push(root)
