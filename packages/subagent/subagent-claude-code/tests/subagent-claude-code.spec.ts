...
describe('task admission and package contracts', () => {
  it('ships one independently installable provider-only Bundle patch', () => {
    ...
    expect(manifest.fw?.bundle?.patch).toBe('./cordis.patch.yml')
    ...
    expect(manifest.dependencies).not.toHaveProperty('@forgeweaver/fw-subagent-codex')
    ...
    const parsed = yaml.load(readFileSync(resolve(root, manifest.fw!.bundle!.patch!), 'utf8'))
  ...
  it('keeps named instances, runs, and HMR ownership isolated', async () => {
    ...
    vi.spyOn(ctx.subprocess, 'spawn').mockImplementation((spec) => {
      ...
      return spec.env?.FW_CLAUDE_INSTANCE === 'safe'
        ? safeChild.handle
        : bypassChild.handle
    })
    ...
    expect(queryOptions.map(options => ({
      instance: options.env?.FW_CLAUDE_INSTANCE,
    ...
    expect(spawnSpecs.map(spec => ({
      instance: spec.env?.FW_CLAUDE_INSTANCE,
  ...
  it('starts through the registered provider with its resolved config and diagnostics', async () => {
    ...
    await ctx.plugin(claudeCode, {
      ...
      env: {
        ...
        CLAUDE_CONFIG_DIR: '/private/tmp/fw-claude-code-unit-config',
        HOME: '/private/tmp/fw-claude-code-unit-home',
      },
      permissionMode: 'auto',
      disposeGraceMs: 29,
    })
...
describe('query options and result mapping', () => {
  it('builds the fixed unattended options over the scrubbed environment', async () => {
    ...
    vi.stubEnv('FW_INTERNAL', 'must-not-leak')
    const child = fakeChild()
    ...
    expect(options.env).not.toHaveProperty('FW_INTERNAL')
