...
import type { SubprocessHandle } from '@forgeweaver/fw-subprocess'
import LocalSubprocessRuntime from '@forgeweaver/fw-subprocess-local'
import * as codex from '../src/index.ts'
import {
  startForgeWeaverResponsesBridge,
  type ForgeWeaverResponsesBridge,
} from './forgeweaver-responses-bridge.ts'
...
const bridges: ForgeWeaverResponsesBridge[] = []
...
describe.skipIf(!process.env.FORGEWEAVER_API_KEY)(
  'Codex provider with real ForgeWeaver API',
  () => {
    it('returns one unique nonce through the production provider and real Codex', async () => {
      ...
      if (apiKey === undefined) throw new Error('e2e ran without FORGEWEAVER_API_KEY')
      const root = mkdtempSync(join(tmpdir(), 'fw-codex-forgeweaver-e2e-'))
      roots.push(root)
      const workspace = join(root, 'workspace')
      const codexHome = join(root, 'codex-home')
      mkdirSync(workspace)
      mkdirSync(codexHome)
      const nonce = `FW_CODEX_FORGEWEAVER_${randomUUID()}`
      const bridge = await startForgeWeaverResponsesBridge(nonce)
      bridges.push(bridge)
      writeFileSync(join(codexHome, 'config.toml'), [
        'model = "forgeweaver-v4-flash"',
        'model_provider = "forgeweaver-e2e"',
        'approval_policy = "never"',
        'sandbox_mode = "read-only"',
        'disable_response_storage = true',
        'check_for_update_on_startup = false',
        '',
        '[model_providers.forgeweaver-e2e]',
        'name = "ForgeWeaver E2E bridge"',
        `base_url = "${bridge.baseUrl}"`,
        'env_key = "FORGEWEAVER_API_KEY"',
        'wire_api = "responses"',
