...
async function realInstanceFixture(
  ...
  const root = mkdtempSync(join(tmpdir(), 'fw-codex-real-'))
  roots.push(root)
  ...
  const env = {
    OPENAI_API_KEY: 'fw-fake-openai-key',
...
describe('real @openai/codex 0.147.0 product', () => {
  it('starts approve-for-me through the real app-server and returns exact text', async () => {
    ...
    const schemaRoot = mkdtempSync(join(tmpdir(), 'fw-codex-schema-'))
    ...
    expect(recorded.headers.authorization).toBe('Bearer fw-fake-openai-key')
  ...
  it('fails a missing platform payload without falling back to a host codex', async () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-codex-missing-payload-'))
    roots.push(root)
  ...
  it('overrides on-request with never and reports a denied command safely', async () => {
    ...
    expect(fixture.requests.every(requestEntry =>
      requestEntry.headers.authorization === 'Bearer fw-fake-openai-key',
    )).toBe(true)
    await expectQuiescent(harness.handles)
  }, 60_000)
