...
运行生命周期、模型继承与深度跟踪均为共享行为，见 [`fw-subagent-spawn-in-process`](../subagent-spawn-in-process/README.zh.md)。
...
父 agent 只通过 `fw-tool-subagent` 接收子 agent 自身的最终输出，不接收继承的前缀或中间工作。
