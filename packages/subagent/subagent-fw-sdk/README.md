...
The SDK provider runs each subagent as a complete ForgeWeaver runtime in a fresh subprocess, driven over stdio JSON-RPC through the [TypeScript SDK client](../../sdk/client/README.md). It is the second out-of-process backend beside [`subagent-acp`](../subagent-acp/README.md), differing in the wire and the child contract: the ACP backend drives any Agent Client Protocol agent; this backend drives specifically a harness SDK runtime (`fw-jsonrpc-agent` bin or packaged executable), so the child is a full peer harness — own `cordis.yml`-decided composition, session persistence, model route, and tools.

## Start and ownership

`start(request)` resolves the child's working directory, spawns the runtime through `ForgeWeaverHarness`, and completes the `initialize` handshake (with the configured `provider`/`model` route and optional `maxTokens` output cap) before it fulfills. Fulfillment therefore means the child runtime is ready and ownership has transferred to the caller. A spawn, handshake, or pre-publication cancellation failure rejects only after the subprocess has been reaped; a working-directory resolution failure rejects before anything is spawned.

The working directory resolves exactly like the ACP backend, through the seam's shared out-of-process helpers ([`fw-subagent`](../subagent/README.md)): the configured `cwd` override when set (validated once at load), else the delegating parent session's cwd — never the server process's own cwd. The resolved path becomes the child process cwd and the workspace cwd of its SDK session.
...
The provider advertises no start-time capabilities (`outputSchema`/`depthLimit`/`toolFilter`/`persona` all false) and `inheritsParentContext: false`: the child is a fresh runtime in another process, and the only parent-derived input is the workspace cwd. `fw-tool-subagent` deployments over this provider set `maxDepth: 'provider-managed'` — the child harness owns its own recursion budget.

## Configuration

| Key | Default | Meaning |
|---|---|---|
| `providerName` | `fw-sdk` | Registry name on `ctx.subagents`. |
...
| `provider` | `forgeweaver-official` | Provider route sent in the child's `initialize`. |
| `model` | `forgeweaver-v4-flash` | Model sent in the child's `initialize`. |
...
| `env` | `{}` | Explicit child environment layered over a credential-scrubbed parent environment (e.g. the child's own `FORGEWEAVER_API_KEY`, or `FW_CORDIS_CONFIG`). |
...
```yaml
- id: subagent-fw-sdk
  name: '@forgeweaver/fw-subagent-fw-sdk'
  config:
    providerName: fw-sdk
    command: node
    ...
    maxTokens: 49152
    env:
      FORGEWEAVER_API_KEY: !!js process.env.FORGEWEAVER_API_KEY
- id: tool-subagent
  name: '@forgeweaver/fw-tool-subagent'
  config: { provider: fw-sdk, toolName: subagent, maxDepth: 'provider-managed' }
```

## Process boundary

The child environment is the [`fw-subprocess`](../../subprocess/README.md) seam's `scrubbedParentEnv()` base — ambient credential-shaped and `FW_*` names dropped — with explicit `config.env` values merged after the scrub. The child is spawned by the SDK client rather than through `ctx.subprocess` (the subprocess README's documented exception for SDK-managed transports), which is why this backend applies the scrub itself. The JSON-RPC wire is the real serialization boundary.
...
Through `fw-tool-subagent`, the parent receives only the child's final assistant text (or accumulated partial text) or that consumer's exact stop-reason error, not intermediate messages or tool traffic.
