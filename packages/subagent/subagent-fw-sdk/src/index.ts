/**
 * Out-of-process SDK subagent backend. Each child is a complete ForgeWeaver
 ...
 * @module @forgeweaver/fw-subagent-fw-sdk
 */

import type { Context } from '@forgeweaver/cordis'
import z from '@forgeweaver/schemastery'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@forgeweaver/fw-subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@forgeweaver/fw-subagent'
import {
...
export const name = 'subagent-fw-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('fw-sdk'),
  command: z.string().required(),
  args: z.array(z.string()).default([]),
  cwd: z.string(),
  provider: z.string().default('forgeweaver-official'),
  model: z.string().default('forgeweaver-v4-flash'),
...
class SdkSubagentProvider implements SubagentProvider {
  ...
  start(request: SubagentStartRequest) {
    const spec: SdkRunSpec = {
      ...
      cwd: resolveChildCwd('subagent-fw-sdk', this.config.cwd, request.parent.session.header.cwd),
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-fw-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-fw-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-fw-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-fw-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-fw-sdk maxTokens must be a positive safe integer')
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-fw-sdk', resolved.cwd)
