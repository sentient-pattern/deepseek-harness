/**
 * Fresh-process SDK subagent client. Drives one child ForgeWeaver
 * runtime over stdio JSON-RPC through `@forgeweaver/fw-sdk-client` and owns
 * cancellation and quiescent disposal. Structure mirrors the ACP backend
 * (`@forgeweaver/fw-subagent-acp`): publish after the child handshake,
 ...
 *
 * @module @forgeweaver/fw-subagent-fw-sdk/run
 */

import { randomUUID } from 'node:crypto'
import { ForgeWeaverHarness, type HarnessNotification } from '@forgeweaver/fw-sdk-client'
import type { ContentBlock } from '@forgeweaver/fw-llm'
import { SessionId, type SessionEvent, type TurnEndReason } from '@forgeweaver/fw-session'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@forgeweaver/fw-subagent'
import { AssistantOutputFold, settleRunResult, subprocessRunHandle } from '@forgeweaver/fw-subagent'
...
export interface SdkRunSpec {
  /** The executable to spawn (the child runtime — a `fw-jsonrpc-agent` bin or packaged exe). */
  command: string
   ...
   * runtime's own `FORGEWEAVER_API_KEY`, or `FW_CORDIS_CONFIG`). Merged after
   * the seam's `scrubbedParentEnv()` base, so an explicit credential or
   * current `FW_*` fact survives while ambient namesakes never leak.
   */
...
export async function startSdkRun(request: SubagentStartRequest, spec: SdkRunSpec): Promise<SubagentRun> {
  ...
  const harness = new ForgeWeaverHarness({
    launch: {
