...
import { resolveExampleLaunch, runLoaderSmoke } from '@forgeweaver/fw-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/subagent/subagent-fw-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'fw-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'fw-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        FW_TEST_CHILD_COMMAND: childLaunch.command,
        FW_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        FW_TEST_CHILD_ENV: JSON.stringify({
