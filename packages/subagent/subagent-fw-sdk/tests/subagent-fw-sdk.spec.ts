...
describe('fw-subagent-fw-sdk provider', () => {
  it('runs a child turn end to end with a parent-unique run id', async () => {
    ...
    const nextRun = await ctx.subagents.start('fw-sdk', request('again'))
  ...
  it('initializes the child with the configured provider/model/maxTokens and the parent cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-fw-sdk-init-'))
    const recordFile = join(tmp, 'init.jsonl')
    try {
      const ctx = await setup({ FAKE_RECORD_INIT: recordFile }, { maxTokens: 4096 })
      const run = await ctx.subagents.start('fw-sdk', request())
      await run.result
      await run.dispose()
  ...
  it('scrubs ambient credentials but forwards explicit config env', async () => {
    process.env.FW_TEST_AMBIENT_SECRET_KEY = 'leak-me-not'
    try {
      const ctx = await setup({
        FAKE_ECHO_ENV: 'FW_TEST_AMBIENT_SECRET_KEY,FORGEWEAVER_API_KEY',
        FORGEWEAVER_API_KEY: 'explicit-child-key',
        FAKE_TEXT: 'done',
      })
      const run = await ctx.subagents.start('fw-sdk', request())
      const result = await run.result
      const answer = text(result.output)
      expect(answer).toContain('FW_TEST_AMBIENT_SECRET_KEY=\n')
      expect(answer).toContain('FORGEWEAVER_API_KEY=explicit-child-key')
      await run.dispose()
      await ctx.fiber.dispose()
    } finally {
      delete process.env.FW_TEST_AMBIENT_SECRET_KEY
    }
  })
  ...
  it('aborting the required signal settles a hung child as aborted', async () => {
    ...
    const run = await ctx.subagents.start('fw-sdk', request('p', controller.signal))
  ...
  it('cancelling between handshake and publish rejects start after reap', async () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-fw-sdk-midcancel-'))
  ...
  it('rejects WITHOUT spawning when the signal is already aborted', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-fw-sdk-preabort-'))
  ...
  it('rejects after reaping when the child dies before the handshake', async () => {
    ...
    const failure = await ctx.subagents.start('fw-sdk', request()).then(
  ...
  it('routes provider-level onError through ctx.logger.warn', async () => {
    ...
    expect(warnings[0]).toContain('subagent-fw-sdk "fw-sdk": child run failed (error)')
    await run.dispose()
    await ctx.fiber.dispose()
  })
  ...
  it('uses a validated config cwd override instead of the parent session cwd', async () => {
    const tmp = mkdtempSync(join(tmpdir(), 'subagent-fw-sdk-cwd-'))
    try {
  ...
  it('fails loud when neither config cwd nor parent session cwd exists', async () => {
    ...
    await expect(ctx.subagents.start('fw-sdk', {
  ...
  it('keeps named plugin exports with no default export (loader shape)', () => {
    expect(sdk.name).toBe('subagent-fw-sdk')
