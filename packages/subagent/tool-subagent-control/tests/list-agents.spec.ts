...
import type { SubagentListEntry } from '@forgeweaver/fw-subagent'
...
async function setupWith(adapter: MockAdapter | GatedAdapter) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'fw-tool-list-agents-'))
  roots.push(root)
...
describe('fw-tool-subagent-control/list-agents', () => {
