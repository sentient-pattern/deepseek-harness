...
import { assembleContextFor, type Agent } from '@forgeweaver/fw-agent'
...
import type { SubagentStartRequest } from '@forgeweaver/fw-subagent'
...
/**
 * Drives the REAL plugin body: mounts `fw-tool-subagent` on a real
...
describe('fw-tool-subagent background mode', () => {
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @forgeweaver/fw-jobs')
  })
...
describe('fw-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'fw-tool-subagent-continuable-'))
    roots.push(root)
