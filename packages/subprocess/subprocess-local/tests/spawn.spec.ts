...
import type { SubprocessHandle, SubprocessOutputReader } from '@forgeweaver/fw-subprocess'
...
function shellArgv(command: string): string[] {
  ...
  switch (command) {
    ...
    case 'echo "[${FW_STALE:-absent}|$FW_SHELL|$FW_SESSION_ID]"':
      return node('console.log("[" + [process.env.FW_STALE ?? "absent", process.env.FW_SHELL, process.env.FW_SESSION_ID].join("|") + "]")')
    case 'echo "[${FW_TEST_API_KEY:-absent}|${FW_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${FW_TEST_PLAIN:-absent}]"':
      return node('console.log("[" + [process.env.FW_TEST_API_KEY ?? "absent", process.env.FW_TEST_TOKEN ?? "absent", process.env.SUBPROCESS_TEST_PASSWORD ?? "absent", process.env.FW_TEST_PLAIN ?? "absent"].join("|") + "]")')
...
const spillDir = mkdtempSync(join(tmpdir(), 'fw-subprocess-spec-'))
...
describe('spawnSubprocess', () => {
  ...
  it('rejects with a spawn error for a nonexistent cwd', async () => {
    await expect(spawnSubprocess(spec('echo hi', { cwd: '/nonexistent-dir-fw-test' })).done)
...
describe.skipIf(process.platform === 'win32')('tree-survivor escalation (terminate and bounded waits reach helpers the leader left behind)', () => {
  ...
  it('service teardown awaits tree survivors, not just handle settlement', async () => {
    const { Context } = await import('@forgeweaver/cordis')
    const { default: LocalSubprocessRuntime } = await import('@forgeweaver/fw-subprocess-local')
...
describe('coverage seams', () => {
  ...
  it('childEnv keeps the POSIX spread on non-Windows hosts', () => {
    ...
    try {
      expect(childEnv({ FW_X: '1' }).FW_X).toBe('1')
    } finally {
  ...
  it('a spawn-failed handle rejects done while waitForExit reports gone', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-fw-dispose-test' }))
  ...
  it('waitForExit on a failed spawn reports exited immediately', async () => {
    const running = spawnSubprocess(spec('true', { cwd: '/nonexistent-dir-fw-spawn-test' }))
    await expect(running.done).rejects.toThrow()
    await expect(running.waitForExit()).resolves.toBe(true)
  })
...
describe('environment and spill-file hardening', () => {
  it('scrubs credential-shaped and ambient DSH env vars from child processes', async () => {
    process.env.FW_TEST_API_KEY = 'super-secret'
    process.env.FW_TEST_TOKEN = 'also-secret'
    process.env.SUBPROCESS_TEST_PASSWORD = 'password-secret'
    process.env.FW_TEST_PLAIN = 'visible'
    try {
      const result = await finish(spawnSubprocess(spec(
        'echo "[${FW_TEST_API_KEY:-absent}|${FW_TEST_TOKEN:-absent}|${SUBPROCESS_TEST_PASSWORD:-absent}|${FW_TEST_PLAIN:-absent}]"',
      )))
      expect(result.stdout.text.trim()).toBe('[absent|absent|absent|absent]')
    } finally {
      delete process.env.FW_TEST_API_KEY
      delete process.env.FW_TEST_TOKEN
      delete process.env.SUBPROCESS_TEST_PASSWORD
      delete process.env.FW_TEST_PLAIN
    }
  })

  it('forwards explicit FW_* env entries while scrubbing ambient ones', async () => {
    // Both facts through one explicit map: the ambient FW_STALE is dropped by
    // the scrub, and the deliberately supplied current values merge after it.
    process.env.FW_STALE = 'old-value'
    try {
      const result = await finish(spawnSubprocess(spec('echo "[${FW_STALE:-absent}|$FW_SHELL|$FW_SESSION_ID]"', {
        env: { FW_SHELL: '1', FW_SESSION_ID: 'current-session' },
      })))
      expect(result.stdout.text.trim()).toBe('[absent|1|current-session]')
    } finally {
      delete process.env.FW_STALE
    }
  })
  ...
  it.skipIf(process.platform === 'win32')('creates spill files with owner-only permissions and random names', async () => {
    ...
    expect(path).toMatch(/fw-subprocess-\d+-\d+-[0-9a-f]{12}-stdout\.log$/)
  ...
  it.skipIf(process.platform === 'win32')('defaults spills into a private per-process directory', async () => {
    ...
    expect(dir).toMatch(/fw-subprocess-/)
