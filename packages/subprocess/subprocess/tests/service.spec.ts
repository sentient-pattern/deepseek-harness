...
import { scrubbedParentEnv, SubprocessRuntime } from '@forgeweaver/fw-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and FW_ names (case-insensitively) but keeps PATH', () => {
    process.env.FW_SCRUB_PROBE = 'stale'
    process.env.fw_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.FW_SCRUB_PROBE).toBeUndefined()
      expect(env.fw_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.FW_SCRUB_PROBE
      delete process.env.fw_scrub_probe_lower
