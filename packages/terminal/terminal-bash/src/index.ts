 ...
 * @module @forgeweaver/fw-terminal-bash
 */

import { Context } from '@forgeweaver/cordis'
import type { Agent } from '@forgeweaver/fw-agent'
import type { Session, SessionEvent } from '@forgeweaver/fw-session'
import { TerminalBackendCleanupError } from '@forgeweaver/fw-terminal'
import type { TerminalBackend, TerminalBackendSpawnSpec } from '@forgeweaver/fw-terminal'
import type { SubprocessTerminalHandle, SubprocessTerminalSpawnSpec } from '@forgeweaver/fw-subprocess'
import type { SandboxExecutionPolicy } from '@forgeweaver/fw-sandbox'
import { effectiveSandboxMode } from '@forgeweaver/fw-sandbox-policy'
...
function childEnvironment(spec: TerminalBackendSpawnSpec, dialect: ShellDialect): Record<string, string> {
  ...
  const common = {
    ...
    GIT_PAGER: 'cat',
    FW_SHELL: '1',
    FW_SESSION_ID: spec.owner.id,
    FW_PTY_SESSION_ID: spec.sessionId,
  }
  if (dialect === 'pwsh') {
