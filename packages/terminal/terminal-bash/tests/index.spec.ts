...
import AgentRegistry, { Inbox, type Agent } from '@forgeweaver/fw-agent'
import SandboxProvider from '@forgeweaver/fw-sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@forgeweaver/fw-sandbox'
import SandboxPolicyService, { setSandboxMode } from '@forgeweaver/fw-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@forgeweaver/fw-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@forgeweaver/fw-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@forgeweaver/fw-terminal-bash'
import { ENCODING_PREAMBLE } from '@forgeweaver/fw-pwsh-local'
import * as ptyLocal from '@forgeweaver/fw-terminal-bash'
import type { ResolvedConfig } from '@forgeweaver/fw-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@forgeweaver/fw-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'fw> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'fw> \'',
        FW_SHELL: '1', FW_SESSION_ID: 'agent', FW_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07fw> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo fw> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo fw> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', FW_SHELL: '1', FW_SESSION_ID: 'agent', FW_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'fw> ' : 'PowerShell 7.6.4\n',
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'fw> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
