...
import { normalizeTerminalText, TerminalSanitizer } from '@forgeweaver/fw-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07fw> ')).toEqual({ text: 'fw> ', prompt: true, promptTail: 'fw> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('fw> ')).toEqual({ text: 'fw> ', prompt: false, promptTail: 'fw> ' })
  })
