...
import { LocalPtySession } from '@forgeweaver/fw-terminal-bash/src/session.ts'
import type { ResolvedConfig } from '@forgeweaver/fw-terminal-bash/src/config.ts'
import type { TerminalSendOperation, TerminalSessionStatus, TerminalSignal } from '@forgeweaver/fw-terminal'
import type {
  SubprocessOutcome,
  SubprocessTerminalHandle,
  SubprocessTerminalSignal,
} from '@forgeweaver/fw-subprocess'
import { TerminalError } from '@forgeweaver/fw-terminal'
import type {
  ProcessIdentity,
  ProcessInspector,
} from '@forgeweaver/fw-subprocess-local/src/process-inspector.ts'
...
async function initialize(session: LocalPtySession, terminal: FakeTerminal): Promise<void> {
  ...
  terminal.emitData('\x1b]133;D;0\x07fw> ')
...
describe('LocalPtySession readiness and output', () => {
  ...
  it('cancels with foreground-group SIGINT, observes AbortSignal, and contains write failures', async () => {
    ...
    terminal.emitData('\x1b]133;D;130\x07fw> ')
    await vi.advanceTimersByTimeAsync(10)
  ...
  it('does not attribute a delayed prior prompt to the current send', async () => {
    ...
    terminal.emitData('\x1b]133;D;0\x07fw> printf \'PID=%s\\n\' "$!"\r\n')
    await vi.advanceTimersByTimeAsync(20)
    expect(settled).toBe(false)
