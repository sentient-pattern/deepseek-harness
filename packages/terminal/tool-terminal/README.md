...
The six generated schemas are listed in the [`fw-tool-terminal` catalog section](../../../docs/tool-catalog.md#forgeweaverfw-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@forgeweaver/fw-jobs` and its model-facing controller.
