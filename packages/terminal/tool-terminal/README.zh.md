...
6 个生成的 schema 列在 [`fw-tool-terminal` 目录章节](../../../docs/tool-catalog.zh.md#forgeweaverfw-tool-terminal)中。此插件活跃时，请求中会包含它们的固定 schema token；按 agent 范围过滤工具时可能隐藏这些 schema。
...
- 后台模式同时依赖 `@forgeweaver/fw-jobs` 及其面向模型的控制器。
