...
function snapshotMode(value: string | undefined): SnapshotSuiteOptions['mode'] {
  switch (value) {
    ...
    default: throw new Error(`unknown FW_SNAPSHOT mode: ${value}`)
  }
}
...
defineAcpSnapshotSuite({
  ...
  mode: snapshotMode(process.env.FW_SNAPSHOT),
})
```
...
示例还发布 `cordis.snapshot.yml` 回放 overlay，位于 `cordis.yml` 旁边（bin 在 `FW_SNAPSHOT=replay` 下交换它们，见[单源回放配置 Agent Note](../../../.agents/notes/archived/testing/2026-07-04-single-source-acp-replay-config.md)）；回放 fixture 由 [`fw-llm-replay`](../llm-replay/README.zh.md) 提供，本包通过为子进程设置的 `FW_SNAPSHOT_*` env var 指向它。`pnpm run test:snapshot:record` 调用在线 LLM（大语言模型），并重写已记录场景的模型 fixture；`pnpm run test:snapshot:refresh` 保持无密钥，运行回放 overlay，并从已提交模型脚本重写 stdout、可比较会话日志预期输出，以及各 pin 自有的提示词与工具 schema 伴随文件。Fixture 角色、录制/回放/刷新语义和场景表字段记录在 `Scenario` 以及[快照 Agent Note](../../../.agents/notes/implemented/testing/2026-06-19-acp-snapshot-tests.zh.md) 中。
...
- **构建 mode 需要当前产物**：先运行 `pnpm run build`，再选择 `FW_EXAMPLE_MODE=lib`；源 mode 仍是零构建路径。
