...
|---|---|---|---|
| `file` | string | `$FW_SNAPSHOT_FILE` | Path to the primary (parent) `session.jsonl` fixture. Required (config or env). |
| `overrideFile` | string | `$FW_SNAPSHOT_OVERRIDE` | Optional `ReplayOverrideDoc` sidecar for the primary session: a bare `ReplayEntry[]` replaces its derived script, while `{ patches }` augments it by call index. |
| `childFiles` | string[] | `$FW_SNAPSHOT_CHILD_FILES` (path-delimited) | Recorded subagent child-session logs for a nested scenario; empty for a single-session scenario. |
...
```yaml
- id: llm-replay
  name: '@forgeweaver/fw-llm-replay'
  config:
    providers:
  ...
  # file/overrideFile/childFiles default to $FW_SNAPSHOT_FILE /
  # $FW_SNAPSHOT_OVERRIDE / $FW_SNAPSHOT_CHILD_FILES, set by the snapshot
