 ...
 * @module @forgeweaver/fw-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@forgeweaver/cordis'
import type {} from '@forgeweaver/fw-compaction'
import { decodeStorageRecord, type SessionEvent } from '@forgeweaver/fw-session'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@forgeweaver/fw-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.FW_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $FW_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.FW_SNAPSHOT_OVERRIDE
  const childEnv = process.env.FW_SNAPSHOT_CHILD_FILES
