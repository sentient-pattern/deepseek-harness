...
import LlmRuntime, { CallId, createUserMessage, GenerateOptions, LlmAdapter, StreamChunk } from '@forgeweaver/fw-llm'
import {
  type Config,
  type ReplayEntry,
  type SessionScript,
...
describe('installLlmReplay (through the real LlmRuntime)', () => {
  ...
  it('registers a replay-only provider catalog when configured', async () => {
    ...
    expect(ctx.llm.listProviders()).toEqual([
      { id: 'forgeweaver', name: 'ForgeWeaver' },
      { id: 'empty', name: 'empty' },
    ])
    await expect(ctx.llm.listModels('forgeweaver')).resolves.toEqual([
      { provider: 'forgeweaver', id: 'flash', name: 'flash', inputModalities: ['text', 'image'] },
      { provider: 'forgeweaver', id: 'pro', name: 'Pro', description: 'Larger model' },
    ])
    await expect(ctx.llm.listModels('empty')).resolves.toEqual([])
    await expect(ctx.llm.resolveModelInfo('forgeweaver', 'flash')).resolves.toMatchObject({
    ...
    await expect(ctx.llm.resolveModelInfo('forgeweaver', 'pro')).resolves.not.toHaveProperty('inputModalities')
    await expect(ctx.llm.resolveModelInfo('forgeweaver', 'pro')).resolves.not.toHaveProperty('context')
    // Efforts without a configured default preserve the provider's own default.
    await expect(ctx.llm.resolveModelInfo('forgeweaver', 'pro')).resolves.toMatchObject({
      reasoning: { efforts: [{ id: 'high', name: 'high' }] },
    })
    await expect(ctx.llm.resolveModelInfo('forgeweaver', 'pro')).resolves.not.toHaveProperty('defaultMaxTokens')
    await expect(ctx.llm.resolveModelInfo('forgeweaver', 'unlisted')).resolves.not.toHaveProperty('context')
    await expect(ctx.llm.resolveModelInfo('empty', 'unlisted')).resolves.not.toHaveProperty('context')
    expect(ctx.llm.providerRetryPolicy('forgeweaver')).toMatchObject({
      mode: 'normal',
      maxRetries: 2,
      initialDelayMs: 1,
      maxDelayMs: 1,
    ...
    expect(await drain(ctx.llm.stream({ provider: 'forgeweaver', model: 'pro', messages: [] }))).toEqual(TEXT_CHUNKS)
  ...
  it('rejects an invalid replay-provider retry policy during registration', async () => {
    ...
    expect(() => {
      installLlmReplay(ctx, {
        file,
        providers: [{ id: 'forgeweaver', retryPolicy: { mode: 'normal', maxRetries: -1 } }],
      })
    }).toThrow(/llm-replay: provider "forgeweaver" retryPolicy\.maxRetries/)
  })
...
describe('apply (the plugin entry)', () => {
  const ORIG = {
    file: process.env.FW_SNAPSHOT_FILE,
    override: process.env.FW_SNAPSHOT_OVERRIDE,
    children: process.env.FW_SNAPSHOT_CHILD_FILES,
  }
  afterEach(() => {
    if (ORIG.file === undefined) delete process.env.FW_SNAPSHOT_FILE
    else process.env.FW_SNAPSHOT_FILE = ORIG.file
    if (ORIG.override === undefined) delete process.env.FW_SNAPSHOT_OVERRIDE
    else process.env.FW_SNAPSHOT_OVERRIDE = ORIG.override
    if (ORIG.children === undefined) delete process.env.FW_SNAPSHOT_CHILD_FILES
    else process.env.FW_SNAPSHOT_CHILD_FILES = ORIG.children
  })
  ...
  it('falls back to $FW_SNAPSHOT_FILE / $FW_SNAPSHOT_OVERRIDE when config is empty', async () => {
    writeFileSync(file, sessionJsonl([]), 'utf8')
    const overrideFile = join(dir, 'replay.override.json')
    writeFileSync(overrideFile, JSON.stringify([{ kind: 'chunks', chunks: TEXT_CHUNKS }]), 'utf8')
    process.env.FW_SNAPSHOT_FILE = file
    process.env.FW_SNAPSHOT_OVERRIDE = overrideFile
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    expect(await drain(ctx.llm.stream({ provider: 'm', model: 'm', messages: [] }))).toEqual(TEXT_CHUNKS)
  ...
  it('uses only the file when no override path is configured or in the env', async () => {
    writeFileSync(file, sessionJsonl(TEXT_CHUNKS.map((c, i) => chunkEvent(i + 1, 1, 1, c))), 'utf8')
    process.env.FW_SNAPSHOT_FILE = file
    delete process.env.FW_SNAPSHOT_OVERRIDE
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    expect(await drain(ctx.llm.stream({ provider: 'm', model: 'm', messages: [] }))).toEqual(TEXT_CHUNKS)
  ...
  it('throws when no fixture path is given by config or env', async () => {
    delete process.env.FW_SNAPSHOT_FILE
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
  ...
  it('falls back to $FW_SNAPSHOT_CHILD_FILES (path-delimited) when config omits childFiles', async () => {
    ...
    process.env.FW_SNAPSHOT_FILE = file
    process.env.FW_SNAPSHOT_CHILD_FILES = childFile // single entry, no delimiter needed
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    const live = (id: string): GenerateOptions =>
  ...
  it('ignores an empty $FW_SNAPSHOT_CHILD_FILES (single-session)', async () => {
    writeFileSync(file, sessionJsonl(TEXT_CHUNKS.map((c, i) => chunkEvent(i + 1, 1, 1, c)), { id: 'p', createdAt: 1 }), 'utf8')
    process.env.FW_SNAPSHOT_FILE = file
    process.env.FW_SNAPSHOT_CHILD_FILES = ''
    const ctx = new Context()
    await ctx.plugin(LlmRuntime)
    apply(ctx)
    expect(await drain(ctx.llm.stream({ provider: 'm', model: 'm', messages: [] }))).toEqual(TEXT_CHUNKS)
