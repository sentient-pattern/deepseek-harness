 ...
 * zero-build dev path, resolving `@forgeweaver/fw-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @forgeweaver/fw-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'FW_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.FW_EXAMPLE_MODE`.
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { FW_HOME: join(cwd, '.fw'), FW_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
