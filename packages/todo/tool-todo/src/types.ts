 ...
 * imports (fw-tools, zod). Two namespace projections serve it — `./types`
 * for host consumers, `./client/types` (the browser half-entry's re-export)
 * for client aggregates — with zero content duplication.
 *
 * @module @forgeweaver/fw-tool-todo/types
 */

import type { TodoItem } from '@forgeweaver/fw-session/types'

export type { TodoItem } from '@forgeweaver/fw-session/types'
