...
function copyFixture(sourceRoot = fixtureRoot): string {
  const root = mkdtempSync(join(tmpdir(), 'fw-typert-remote-model-'))
...
`
  ...
  writeFileSync(configPath, JSON.stringify({
    ...
    compilerOptions: {
      ...
      skipLibCheck: false,
      paths: {
        '@forgeweaver/fw-typert-protocol': ['./typert-protocol.d.ts'],
...
import type { TypertRemoteNamespaceMap } from '@forgeweaver/fw-typert-protocol'
declare const ctx: { remote: TypertRemoteNamespaceMap }
