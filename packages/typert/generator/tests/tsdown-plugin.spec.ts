...
const discovered = vi.hoisted(() => vi.fn(() => [
  { package: '@forgeweaver/fw-tools', root: 'packages/core/tools', faces: ['host'] },
...
describe('typertPlugin', () => {
  ...
  it('skips outputs that do not identify a Typert contributor', async () => {
    const plugin = typertPlugin()
    expect(plugin.name).toBe('fw-typert-generator')
    plugin.writeBundle({})
  ...
  it('emits every explicit workspace contributor once from a host-only prepass', async () => {
    const root = await workspace()
    const trigger = await packageOutput(root, 'generator', { name: '@forgeweaver/fw-typert-generator' })
    ...
    expect(generated).toHaveBeenCalledOnce()
    expect(generated).toHaveBeenCalledWith(
      ['@forgeweaver/fw-tools', '@fixture/remote-only'],
      ['host'],
    )
...
async function workspace(): Promise<string> {
  const root = mkdtempSync(join(tmpdir(), 'fw-typert-tsdown-'))
  roots.push(root)
