...
import * as typertLoader from '@forgeweaver/fw-typert-loader'
import { validateTypertManifest } from '@forgeweaver/fw-typert-loader'
import { z } from 'zod'
...
afterEach(async () => {
  ...
  context = undefined
  Reflect.deleteProperty(globalThis, '__fwTypertLoaderGate')
...
describe('typert loader', () => {
  it('registers an explicit package without a Loader entry and withdraws it with the loader', LOADER_TEST_TIMEOUT, async () => {
    root = await mkdtemp(join(tmpdir(), 'fw-typert-loader-'))
    await linkZod(root)
  ...
  it('drops an in-flight manifest when the loader is disposed before import settles', LOADER_TEST_TIMEOUT, async () => {
    ...
    Reflect.set(globalThis, '__fwTypertLoaderGate', {
    ...
    await writePackage(root, '@fixture/pending', {
      typertSource: [
        'import { z } from \'zod\'',
        'globalThis.__fwTypertLoaderGate.started()',
        'await globalThis.__fwTypertLoaderGate.wait',
