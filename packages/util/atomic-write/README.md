...
Zero-dependency atomic file replacement shared by file-backed stores that must never leave partial, symlink-hijacked, or wider-than-intended content on disk — the user-settings document (`fw-settings-file`) and the credentials store (`fw-credentials-local`).
...
await writeFileAtomic('/home/u/.fw/settings.yaml', text, { mode: 0o600 })

// Read-modify-write against the same file from several processes.
await withFileLock('/home/u/.fw/settings.yaml', async () => {
  await writeFileAtomic('/home/u/.fw/settings.yaml', render(text), { mode: 0o600 })
})
```
