...
包为自己拥有的 id 添加品牌：`CallId` 位于 `fw-llm`，共享的 agent/会话 `SessionId` 位于 `fw-session`，`JobId` 位于 `fw-jobs`。为可能被混淆的跨包 id 添加品牌，但无需为每个字符串都添加。

该包只负责这一原语。保持无依赖意味着，例如 `fw-jobs` 可以为 `JobId` 使用品牌类型，而无需仅为使用 `Branded` 而导入不相关的功能包。
