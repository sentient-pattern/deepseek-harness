...
ForgeWeaver 用户数据的共享文件系统路径辅助工具。

## DSH 主目录

`resolveDshHome()` 解析 ForgeWeaver 的单根主目录。优先级从高到低为：显式配置的路径、`$FW_HOME`、`~/.fw`。harness 将所有用户数据保存在同一根目录下。

`fwHomePath(...segments)` 使用 Node 的平台路径规则，将子路径段拼接到解析后的主目录下。不传入任何路径段时，返回主目录本身。

`fwHomeDisplay()` 以符号方式表示当前根目录，用于面向用户的路径：默认主目录表示为 `~/.fw`，任何已配置的主目录表示为 `$FW_HOME`。它绝不会泄露机器的绝对路径。

`FW_HOME_DIR_NAME` 定义默认用户数据目录名：`.fw`。

`defaultDshHome()` 使用 Node 的平台路径规则，将操作系统主目录与 `.fw` 拼接，并返回默认 ForgeWeaver 主目录。
