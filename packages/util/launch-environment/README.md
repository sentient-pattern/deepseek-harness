...
| `$FW_HOME/.env` | `user-env` | The user's own machine-level defaults |
...
Names match the way the platform matches them: exactly on POSIX, case-insensitively on Windows. A case-sensitive lookup there would rank the wrong layer — a shell's `forgeweaver_api_key` and a project `.env`'s `FORGEWEAVER_API_KEY` are one variable to the OS, and treating them as two would let the project win.
...
const endpoint = launchEnvironmentOf(ctx).get('FORGEWEAVER_BASE_URL')?.value
```
...
- **The snapshot is not a subprocess boundary** — every layer is also materialized into `process.env`, so ordinary project variables reach child processes under [`fw-subprocess`](../../subprocess/subprocess/README.md)'s scrub. The product launcher's [`.env` contract](../../boot/app-boot/README.md#profiles) rejects bootstrap variables before materialization.
