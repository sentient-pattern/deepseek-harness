...
import type { WebSearchProvider, WebSearchResult } from '@forgeweaver/fw-web'
...
} from '@forgeweaver/fw-tool-web'
import type { ContentBlock } from '@forgeweaver/fw-llm'
import type { ToolResult } from '@forgeweaver/fw-tools'
...
async function mountTools(opts: {
  ...
  fetchProvider?: import('@forgeweaver/fw-web').WebFetchProvider
...
describe('tool-web execution through the real registry', () => {
  ...
  it('executes web_fetch, forwarding the url (no timeout param) and the abort signal to the seam', async () => {
    ...
    // tool-call budget is owned by fw-tool-call-timeout-policy over exec.signal.
