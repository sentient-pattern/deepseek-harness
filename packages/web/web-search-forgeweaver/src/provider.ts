/**
 * ForgeWeaver search through an Anthropic-compatible Messages model call with the native
 ...
 * @module @forgeweaver/fw-web-search-forgeweaver/provider
 */

import { WebError } from '@forgeweaver/fw-web'
import type {
  WebSearchProvider,
  WebSearchRequest,
  WebSearchResult,
  WebSearchSource,
} from '@forgeweaver/fw-web'
...
export const FORGEWEAVER_PROVIDER_ID = 'forgeweaver-official'

/**
 * Default endpoint: ForgeWeaver's Anthropic-compatible API, `/v1` included
 * (`/messages` is appended). This is NOT the chat-completions base
 * (`https://api.forgeweaver.com`) `@forgeweaver/fw-llm-forgeweaver` uses, so this
 * provider does NOT reuse `$FORGEWEAVER_BASE_URL` — only the API key is shared.
 */
export const FORGEWEAVER_DEFAULT_BASE_URL = 'https://api.forgeweaver.com/anthropic/v1'

/** Default Anthropic-format model name (aligned with the repo's ForgeWeaver model vocabulary). */
export const FORGEWEAVER_DEFAULT_MODEL = 'forgeweaver-v4-flash'

/** Default `anthropic-version` header value. */
export const FORGEWEAVER_DEFAULT_API_VERSION = '2023-06-01'

/** Default upper bound on generated tokens for the Messages request. */
export const FORGEWEAVER_DEFAULT_MAX_TOKENS = 4096

/** Default maximum `web_search` server-tool uses per request. */
export const FORGEWEAVER_DEFAULT_MAX_USES = 5

/** Attribution header sent on every request. Bump with the package version. */
const USER_AGENT = 'forgeweaver-harness/0.0.1'

/**
 * Exact secret-free ForgeWeaver Messages request recorded immediately before one
 * auxiliary search dispatch.
 */
export interface ForgeWeaverSearchLlmRequest {
...
declare module '@forgeweaver/fw-session/types' {
  interface SessionEventMap {
    /** Secret-free auxiliary ForgeWeaver search request recorded before dispatch. */
    'web/forgeweaver-search-llm-request': ForgeWeaverSearchLlmRequest
  }
}

/** Resolved provider options (the plugin's `apply` supplies credential and constant defaults). */
export interface ForgeWeaverSearchProviderOptions {
  /** Literal ForgeWeaver API key; when present it wins over {@link resolveApiKey}. */
  apiKey?: string
  /** Resolve the current ForgeWeaver API key for one search operation. */
   ...
   */
  recordRequest?: (request: ForgeWeaverSearchLlmRequest) => void
}
...
/**
 * Map a ForgeWeaver Anthropic Messages response to a normalized search result. Walks
...
export function mapAnthropicResponse(response: AnthropicResponse): WebSearchResult {
  ...
  if (resultBlocks.length === 0) {
    throw new WebError(
      'ForgeWeaver returned no web_search_tool_result blocks; the request may not have triggered native web search',
      'WEB_PROVIDER_ERROR',
    )
  }
...
/** The ForgeWeaver-backed search provider; HTTP redirects fail as `WEB_PROVIDER_ERROR`. */
export class ForgeWeaverSearchProvider implements WebSearchProvider {
  readonly id = FORGEWEAVER_PROVIDER_ID
   ...
   */
  constructor(private readonly resolveOptions: () => ForgeWeaverSearchProviderOptions) {}
  ...
  async search(request: WebSearchRequest, signal?: AbortSignal): Promise<WebSearchResult> {
    ...
    const body: ForgeWeaverSearchLlmRequest['body'] = {
      model: options.model,
    ...
    try {
      response = await fetch(endpoint, {
        ...
        redirect: 'error',
        headers: {
          // Official ForgeWeaver expects `x-api-key`; an Anthropic-compatible proxy
      ...
      throw new WebError(`ForgeWeaver search request failed: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }

    if (!response.ok) {
      const status = response.status
      let message = `ForgeWeaver API error (HTTP ${status})`
      try {
    ...
    try {
      ...
      throw new WebError(`ForgeWeaver returned an unprocessable response body: ${String(error)}`, 'WEB_PROVIDER_ERROR', { cause: error })
    }
  }
   ...
   */
  private async apiKey(options: ForgeWeaverSearchProviderOptions, signal?: AbortSignal): Promise<string> {
    throwIfSearchAborted(signal)
    ...
    try {
      ...
      if (signal?.aborted === true || isAbortError(error)) throw searchAborted(signal, error)
      throw new WebError(
        `ForgeWeaver search credential resolution failed: ${String(error)}`,
        'WEB_PROVIDER_ERROR',
        { cause: error },
      )
    }
    if (resolved !== undefined && resolved.length > 0) return resolved
    const ref = options.apiKeyEnv ?? 'FORGEWEAVER_API_KEY'
    throw new WebError(
      `ForgeWeaver search has no API key for "${ref}"; store it through the credentials service`
      + ' (the web Models page writes it), export it in the launching environment, or set a literal'
      + ' "apiKey" in the web-search-forgeweaver config',
...
function searchAborted(signal?: AbortSignal, fallback?: unknown): WebError {
  return new WebError('ForgeWeaver search aborted', 'WEB_ABORTED', {
...
/** True for ForgeWeaver request limits that can be sent to the Messages API. */
