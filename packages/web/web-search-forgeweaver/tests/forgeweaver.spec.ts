...
import * as forgeweaverPlugin from '@forgeweaver/fw-web-search-forgeweaver'
import { citationSnippets, mapAnthropicResponse } from '../src/provider.ts'
import type { AnthropicResponse } from '@forgeweaver/fw-web-search-forgeweaver/src/types.ts'
...
const options = {
  apiKey: 'ds-key',
  baseURL: 'https://api.forgeweaver.test/anthropic/v1',
...
describe('ForgeWeaverSearchProvider availability', () => {
...
describe('ForgeWeaverSearchProvider request mapping', () => {
  it('records and posts the same Anthropic Messages request with the web_search server tool', async () => {
    ...
    expect(url).toBe('https://api.forgeweaver.test/anthropic/v1/messages')
...
describe('ForgeWeaverSearchProvider settings changes mid-search', () => {
  it('serves one search from one section even when settings land during credential resolution', async () => {
    ...
    const provider = new ForgeWeaverSearchProvider(() => ({ ...current, resolveApiKey }))
...
describe('ForgeWeaverSearchProvider error handling', () => {
  ...
  it('maps a credential resolver rejection under an active signal to WEB_PROVIDER_ERROR', async () => {
    ...
    await expect(searchProvider({
      ...
      .rejects.toThrow(expect.objectContaining({
        ...
        message: 'ForgeWeaver search credential resolution failed: Error: credential backend failed',
      }))
  })
  ...
  it('uses the default credential reference when no resolver is configured', async () => {
    await expect(searchProvider({ ...options, apiKey: '' }).search({ query: 'q' }))
      .rejects.toThrow('ForgeWeaver search has no API key for "FORGEWEAVER_API_KEY"')
  })
  ...
  it('keeps a status-line message when the error body is not JSON', async () => {
    ...
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({ message: 'ForgeWeaver API error (HTTP 503)' }))
  })

  it('keeps the status-line message when the JSON error body carries no detail', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse({}, { status: 500 })))
    await expect(searchProvider(options).search({ query: 'q' }))
      .rejects.toThrow(expect.objectContaining({ message: 'ForgeWeaver API error (HTTP 500)' }))
  })
...
describe('web-search-forgeweaver plugin registration', () => {
  it('registers the provider into ctx.web (HMR-safe)', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => jsonResponse(searchResponse())))
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
    const fiber = await ctx.plugin(forgeweaverPlugin, { apiKey: 'ds-key' })
  ...
  it('rejects maxTokens: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
    await expect(ctx.plugin(forgeweaverPlugin, { apiKey: 'ds-key', maxTokens: 0 }))
  ...
  it('rejects maxUses: 0 at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
    await expect(ctx.plugin(forgeweaverPlugin, { apiKey: 'ds-key', maxUses: 0 }))
  ...
  it('rejects a fractional maxUses at plugin construction', async () => {
    const ctx = new Context()
    await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
    await expect(ctx.plugin(forgeweaverPlugin, { apiKey: 'ds-key', maxUses: 1.5 }))
      .rejects.toThrow(/maxUses expected number multiple of 1/)
  })

  it('has no default export (namespace plugin export shape)', () => {
    expect('default' in forgeweaverPlugin).toBe(false)
  })
  ...
  it('survives the real Loader unwrapExports path keeping name/inject/Config', () => {
    ...
    const unwrapped = loader.unwrapExports(forgeweaverPlugin) as Record<string, unknown>
    expect(unwrapped).toBe(forgeweaverPlugin)
    expect(unwrapped.name).toBe('web-search-forgeweaver')
  ...
  it('boots over ctx.web through the unwrapped module without an inject error', async () => {
    ...
    await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
    const loader = Object.create(Loader.prototype) as Loader
    const unwrapped = loader.unwrapExports(forgeweaverPlugin) as Parameters<Context['plugin']>[0]
  ...
  it('falls back to the env key and defaults when config omits them', async () => {
    const prev = process.env.FORGEWEAVER_API_KEY
    process.env.FORGEWEAVER_API_KEY = 'env-key'
    try {
      const fetchMock = vi.fn(async () => jsonResponse(searchResponse()))
      vi.stubGlobal('fetch', fetchMock)
      const ctx = new Context()
      await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
      forgeweaverPlugin.apply(ctx, {})
      await ctx.web.search({ query: 'q' })
      const [url, init] = fetchMock.mock.calls[0] as unknown as [string, RequestInit]
      expect(url).toBe('https://api.forgeweaver.com/anthropic/v1/messages')
      expect((init.headers as Record<string, string>)['x-api-key']).toBe('env-key')
      expect(JSON.parse(init.body as string)).toMatchObject({ model: 'forgeweaver-v4-flash' })
      await ctx.fiber.dispose()
    } finally {
      if (prev === undefined) delete process.env.FORGEWEAVER_API_KEY
      else process.env.FORGEWEAVER_API_KEY = prev
    }
  })

  it('resolves the credential for each search so a stored or rotated key needs no restart', async () => {
    const previous = process.env.FORGEWEAVER_API_KEY
    delete process.env.FORGEWEAVER_API_KEY
    const dir = await mkdtemp(join(tmpdir(), 'fw-web-search-credentials-'))
    ...
    try {
      ...
      await ctx.plugin(forgeweaverPlugin, { baseURL: 'https://api.forgeweaver.test/anthropic/v1' })
      ...
      if (previous === undefined) delete process.env.FORGEWEAVER_API_KEY
      else process.env.FORGEWEAVER_API_KEY = previous
    }
  })
  ...
  it('reports an actionable credential error when neither config nor env supplies a key', async () => {
    ...
    try {
      const ctx = new Context()
      await ctx.plugin(WebRuntime, { searchProvider: FORGEWEAVER_PROVIDER_ID })
      await ctx.plugin(forgeweaverPlugin, {})
      let caught: unknown
      try {
        await ctx.web.search({ query: 'q' })
      } catch (error: unknown) {
      ...
      if (prev !== undefined) process.env.FORGEWEAVER_API_KEY = prev
    }
  })
})
