/** The `web-search-forgeweaver` settings section layered over the composition entry. */
...
import { WEB_SEARCH_FORGEWEAVER_SETTINGS_NAMESPACE } from '@forgeweaver/fw-web-search-forgeweaver'
...
async function boot(): Promise<{ ctx: Context; settingsFiber: Fiber; pluginFiber: Fiber }> {
  ...
  const pluginFiber = ctx.plugin(forgeweaverPlugin, { apiKey: 'ds-key', baseURL: 'https://search.entry.test/v1' })
...
describe('web-search-forgeweaver settings section', () => {
  it('serves a stored endpoint to the next search without re-registering the provider', async () => {
    ...
    await bench.ctx.settings.update(WEB_SEARCH_FORGEWEAVER_SETTINGS_NAMESPACE, {
  ...
  it('keeps the literal key out of every described layer', async () => {
    const bench = await boot()
    await bench.ctx.settings.update(WEB_SEARCH_FORGEWEAVER_SETTINGS_NAMESPACE, { apiKey: 'ds-stored-secret' })

    const [descriptor] = bench.ctx.settings.describe({ redactSecrets: true })
      .filter(row => String(row.ns) === 'web-search-forgeweaver')
  ...
  it('releases the namespace when the plugin unloads', async () => {
    const bench = await boot()
    expect(bench.ctx.settings.describe().map(row => String(row.ns))).toContain('web-search-forgeweaver')

    await bench.pluginFiber.dispose()

    expect(bench.ctx.settings.describe().map(row => String(row.ns))).not.toContain('web-search-forgeweaver')
    await bench.ctx.fiber.dispose()
  })
})
