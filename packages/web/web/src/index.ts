...
export class WebRuntime extends Service {
   ...
   * `$FW_WEB_SEARCH_PROVIDER` / `$FW_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.FW_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.FW_WEB_FETCH_PROVIDER
  }
