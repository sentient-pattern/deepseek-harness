...
- Set `FW_RUNTIME_MODE=node` to use the built Node carrier on system Node `>=22.19`. The build script refreshes this carrier, but distributions never include or auto-select it.
...
The root `package.json` version is authoritative for both Python distributions. The staging script injects that version into both wheels and pins the SDK to the same `forgeweaver-harness-runtime-bin` version.
...
python scripts/build-python-release.py --package runtime --platform macos-arm64 --runtime-exe dist-exe/fw-jsonrpc-agent-pkg-macos-arm64 --output-dir dist-python
pip install \
  "dist-python/forgeweaver_harness_sdk-$version-py3-none-any.whl" \
  "dist-python/forgeweaver_harness_runtime_bin-$version-py3-none-macosx_14_0_arm64.whl"
```
