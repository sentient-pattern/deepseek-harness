...
class ForgeWeaverHarnessConfig:
    ...
    FORGEWEAVER_API_KEY and FORGEWEAVER_BASE_URL settings keep working. Use ``env`` to
    intentionally override or inject variables for a subprocess.
    """

    provider: str = "forgeweaver-official"
    model: str = "forgeweaver-v4-flash"
...
class ForgeWeaverHarness:
    """Reusable synchronous SDK for running ForgeWeaver agent turns.
    ...
    def __init__(self, config: ForgeWeaverHarnessConfig | None = None, **kwargs: object) -> None:
        if config is not None and kwargs:
            raise TypeError("pass either ForgeWeaverHarnessConfig or keyword options, not both")
        self.config = config or ForgeWeaverHarnessConfig(**kwargs)
        ...
        if self.config.session_root is not None:
            env["FW_SESSION_ROOT"] = self.config.session_root
        if self.config.cordis is not None:
            env["FW_CORDIS_CONFIG"] = self.config.cordis
        env["FW_CWD"] = cwd
        if self.config.base_url is not None:
            env["FORGEWEAVER_BASE_URL"] = self.config.base_url
        if self.config.api_key is not None:
            env["FORGEWEAVER_API_KEY"] = self.config.api_key
    ...
    def __enter__(self) -> "ForgeWeaverHarness":
        self.start()
        return self
...
class Session:
    def __init__(self, harness: ForgeWeaverHarness, session_id: str) -> None:
