...
import forgeweaver_harness_runtime as runtime
import pytest

from forgeweaver_harness_runtime import (
...
def test_default_config_is_shipped_with_the_package() -> None:
    ...
    assert "@forgeweaver/fw-agent-spine-demo" in config
    assert "@forgeweaver/fw-session-persistence-jsonl" in config
    assert "@forgeweaver/fw-session-checkpoint-policy" in config
...
def test_runtime_requires_spawn_helper_only_on_macos(
    ...
    runtime_dir.mkdir()
    linux = runtime_dir / "fw-jsonrpc-agent-pkg-linux-x64"
    linux.touch()
    Path(f"{linux}-rg").touch()
    macos = runtime_dir / "fw-jsonrpc-agent-pkg-macos-arm64"
    macos.touch()
...
def test_runtime_requires_ripgrep_sidecar(
    ...
    runtime_dir = tmp_path / "runtime"
    runtime_dir.mkdir()
    (runtime_dir / "fw-jsonrpc-agent-pkg-linux-x64").touch()
    monkeypatch.setattr(runtime, "bundled_package_dir", lambda: tmp_path)
    monkeypatch.setattr(runtime, "_current_platform_tag", lambda: "linux-x64")
