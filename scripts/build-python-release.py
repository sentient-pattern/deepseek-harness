...
SDK_DISTRIBUTION = "forgeweaver-harness-sdk"
RUNTIME_DISTRIBUTION = "forgeweaver-harness-runtime-bin"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-python-release-") as temporary:
        staging = Path(temporary) / args.package
        if args.package == "sdk":
            stage_sdk(staging, wheel_version)
            environment = None
            expected = output_dir / f"forgeweaver_harness_sdk-{wheel_version}-py3-none-any.whl"
        else:
            ...
            environment = {"FW_RUNTIME_PLATFORM_TAG": platform_tag}
            expected = output_dir / f"forgeweaver_harness_runtime_bin-{wheel_version}-py3-none-{platform_tag}.whl"
...
def stage_sdk(destination: Path, version: str) -> None:
    ...
    text, count = re.subn(
        r'"forgeweaver-harness-runtime-bin==[^"]+"',
        f'"forgeweaver-harness-runtime-bin=={version}"',
        pyproject.read_text(),
        count=1,
    )
    if count != 1:
...
def stage_runtime(destination: Path, version: str, executable: Path, executable_name: str) -> None:
    ...
    runtime_dir = destination / "src" / "forgeweaver_harness_runtime" / "runtime"
...
def verify_wheel(
    ...
    with zipfile.ZipFile(wheel) as archive:
        ...
        runtime_files = [
            name for name in archive.namelist() if "/runtime/fw-jsonrpc-agent-pkg-" in name
        ]
        if package == "runtime":
