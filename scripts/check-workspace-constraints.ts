...
const vendoredPackages = new Set([
  '@forgeweaver/cordis',
  '@forgeweaver/cosmokit',
  '@forgeweaver/schemastery',
  '@forgeweaver/cordis-plugin-loader',
  '@forgeweaver/cordis-plugin-include',
  '@forgeweaver/cordis-plugin-group',
  '@forgeweaver/cordis-plugin-timer',
  '@forgeweaver/cordis-plugin-hmr',
  '@forgeweaver/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@forgeweaver/node-addon-landlock-run',
  '@forgeweaver/node-addon-landlock-run-linux-arm64',
  '@forgeweaver/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@forgeweaver/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/sentient-pattern/deepseek-harness.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/sentient-pattern/deepseek-harness.git'
...
const experimentalPackageNamePrefix = '@forgeweaver/fw-experimental-'
/** Directories whose packages this repository publishes: one release member each. */
const releaseMemberDirectory = /^(?:packages\/(?!experimental\/)[^/]+\/[^/]+|apps\/[^/]+|vendor\/[^/]+)$/

const localArtifactDirs = new Set(['node_modules'])
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  '@forgeweaver/fw': ['lib/*.js', 'config'],
  // The Web build emits sourcemaps for browser debugging; publishing them is
  // what the payload policy forbids, so the bundle ships without them.
  '@forgeweaver/fw-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@forgeweaver/fw-client-ui-primitives': ['lib/**/*.css'],
  '@forgeweaver/fw-client-web': ['lib/**/*.css'],
  '@forgeweaver/fw-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@forgeweaver/fw-code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@forgeweaver/fw-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@forgeweaver/fw-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
  // SQLite loads every statement from immutable package resources at runtime.
  '@forgeweaver/fw-session-persistence-sqlite': ['resources/sql/**/*.sql'],
  '@forgeweaver/fw-skill-badge': ['assets'],
  '@forgeweaver/fw-subprocess-local': ['scripts/ensure-spawn-helper.mjs'],
}
...
function expectedDshPackageFiles(manifest: PackageManifest): readonly string[] {
  const declaredPatch = manifest.fw?.bundle?.patch
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the fw family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@forgeweaver/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@forgeweaver/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@forgeweaver/fw-')) {
    const peer = manifest.peerDependencies?.['@forgeweaver/cordis']
...
function checkRepositoryVersion(): string[] {
  // The root carries the fw release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:fw` and its publication.
