...
describe('CI workflow', () => {
  ...
  it('keeps a required Wine Windows job, a non-blocking native Windows job with failover, and a master-only standby', () => {
    ...
    expect(windowsNative['runs-on']).toContain('FW_CI_FAILOVER_WINDOWS')
    expect(windowsNative['runs-on']).not.toContain('FW_CI_FAILOVER_LINUX')
    expect(windowsNative['runs-on']).toContain('self-hosted')
    expect(windowsNative['runs-on']).toContain('fw-win-ci')
    expect(windowsNative['runs-on']).toContain('fw-windows-2025-16core')
    expect(windowsNative.name).toBe('windows node 24 / native complete')
    expect(windowsNative.if).toBe("github.event_name == 'pull_request'")
    expect(windowsNative.env).toMatchObject({
      FW_COVERAGE_TEST_TIMEOUT_MS: '30000',
    })
    ...
    expect(serialWindows['runs-on']).toEqual(['self-hosted', 'fw-win-ci', 'windows'])
    ...
    // and the verdict job resolve their pool through FW_CI_FAILOVER_LINUX,
    // never the Windows switch.
    for (const [jobName, job] of [['node-24', node24], ['node-24-coverage', node24Coverage], ['node-24-consumers', node24Consumers]] as const) {
      expect(typeof job['runs-on']).toBe('string')
      expect(job['runs-on'], `${jobName} runs-on must use the Linux failover switch`).toContain('FW_CI_FAILOVER_LINUX')
      expect(job['runs-on'], `${jobName} runs-on must not use the Windows failover switch`).not.toContain('FW_CI_FAILOVER_WINDOWS')
      expect(job['runs-on']).toContain('vm-backup')
    }
    expect(aggregate['runs-on']).toContain('FW_CI_FAILOVER_LINUX')
    expect(aggregate['runs-on']).not.toContain('FW_CI_FAILOVER_WINDOWS')
...
describe('ForgeWeaver e2e workflow', () => {
  it('prepares bubblewrap from the pinned payload without a package transaction', () => {
    const workflow = loadWorkflow('.github/workflows/e2e.yml')
    const e2e = workflowJob(workflow, 'e2e')
    if (!Array.isArray(e2e.steps)) throw new TypeError('ForgeWeaver e2e workflow must define steps')
...
describe('Python release workflows', () => {
  it('keeps complete wheel validation separate from protected public publication', () => {
    ...
    expect(pythonCompatSteps).toContain('dist/forgeweaver_harness_sdk-$VERSION-py3-none-any.whl')
    expect(pythonCompatSteps).toContain('dist/forgeweaver_harness_runtime_bin-$VERSION-py3-none-manylinux_2_28_x86_64.whl')
  ...
  it('exposes the native wheel builder to the release caller with normalized versions', () => {
    ...
    expect(JSON.stringify(manylinuxSmoke)).toContain('-e FW_TELEMETRY_DISABLED')
  })
...
describe('Documentation site publication', () => {
  it('keeps Pages deployment dispatch-only from a fw-v* tag', () => {
    ...
    // RELEASE_PUBLISH makes release:verify reject every ref that is not a fw-v*
    ...
    expect(verify).toMatchObject({
      ...
      run: 'pnpm run release:verify --family fw',
    })
