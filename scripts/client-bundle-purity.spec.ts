...
const REQUESTING_PACKAGE = '@forgeweaver/fw-client-ui-conversation'
...
describe('client bundle build faces', () => {
  it('watches source in development and consumes emitted JavaScript in the Client build', () => {
    const bundle = clientBundle('@forgeweaver/fw-client-test', ['lib/types/index.js'])
    const development = bundle({ env: {} }).find(config => config.platform === 'browser')
    const artifact = bundle({ env: { FW_BUILD_FACE: 'client' } })
...
function purityResolveId(id = REQUESTING_PACKAGE): ResolveId {
  ...
  const gate = plugins.find(p => p.name === 'fw-client-bundle-purity')
...
function cssModulePlugin(): CssModulePlugin {
  ...
  const plugin = plugins.find(candidate => candidate.name === 'fw-css-modules-inline')
...
describe('client bundle purity gate', () => {
  ...
  it('leaves default externals and non-scoped specifiers alone', () => {
    expect(resolveId('@forgeweaver/fw-client-ui-slots')).toBeNull()
    expect(resolveId('@forgeweaver/fw-client-ui-primitives')).toBeNull()
    expect(resolveId('@forgeweaver/fw-client-runtime/client')).toBeNull()
    expect(resolveId('react')).toBeNull()
    expect(resolveId('zod')).toBeNull()
  })

  it('rejects the retired web-react platform package', () => {
    expect(() => resolveId('@forgeweaver/fw-client-web-react')).toThrow(/purity/)
    expect(() => resolveId('@forgeweaver/fw-client-web-react/store')).toThrow(/purity/)
  })

  it('lets inline-safe wire layers inline', () => {
    expect(resolveId('@forgeweaver/fw-host-apiproxy/api')).toBeNull()
    expect(resolveId('@forgeweaver/fw-session/surface')).toBeNull()
    expect(resolveId('@forgeweaver/fw-brand')).toBeNull()
  })

  it('lets exact generated Remote contributions inline without admitting their package implementation', () => {
    expect(resolveId('@forgeweaver/fw-goal/remote')).toBeNull()
    expect(() => resolveId('@forgeweaver/fw-goal')).toThrow(/purity/)
    expect(() => resolveId('@forgeweaver/fw-goal/client')).toThrow(/purity/)
    expect(() => resolveId('@forgeweaver/fw-goal/remote/nested')).toThrow(/purity/)
  })

  it('throws on any other @forgeweaver leak', () => {
    expect(() => resolveId('@forgeweaver/fw-agent')).toThrow(/purity/)
    expect(() => resolveId('@forgeweaver/fw-client-web')).toThrow(/purity/)
  })

  it('throws on cross-plugin value imports — bare plugin names and /client subpaths alike', () => {
    expect(() => resolveId('@forgeweaver/fw-client-connection')).toThrow(/purity/)
    expect(() => resolveId('@forgeweaver/fw-client-runtime')).toThrow(/purity/)
    expect(() => resolveId('@forgeweaver/fw-client-ui-layout/client')).toThrow(/purity/)
  })

  it('admits the parser-preloaded runtime for every dynamic bundle', () => {
    expect(resolveId('@forgeweaver/fw-client-runtime/client')).toBeNull()
    const withoutRequest = purityResolveId('@forgeweaver/fw-client-ui-goal')
    expect(withoutRequest('@forgeweaver/fw-client-runtime/client')).toBeNull()
  })
  ...
  it('externalizes the baseline independently of each package manifest', () => {
    ...
    const plain = clientConfigs('@forgeweaver/fw-client-connection')[0]?.deps as {
    ...
    expect(plain.neverBundle('@forgeweaver/fw-client-runtime/client')).toBe(true)
  })
})

describe('client bundle module requests', () => {
  it('requests what the declaration lists', () => {
    const requests = requestedExternals('@forgeweaver/fw-client-fixture', {
      external: ['react', 'react/jsx-runtime', '@forgeweaver/fw-client-ui-slots'],
    })

    expect([...requests].sort()).toEqual([
      '@forgeweaver/fw-client-ui-slots', 'react', 'react/jsx-runtime',
    ])
  })

  it('requests nothing when the declaration is absent', () => {
    expect(requestedExternals('@forgeweaver/fw-client-fixture', {}).size).toBe(0)
  })

  it('rejects a malformed declaration instead of reading past it', () => {
    expect(() => requestedExternals('@forgeweaver/fw-client-fixture', { external: 'react' }))
      .toThrow(/fw\.client\.external must be a string array/)
  })
})
...
describe('client bundle debug artifacts', () => {
  ...
  it('maps first-party sources to their repository package paths', () => {
    const configs = clientConfigs('@forgeweaver/fw-client-ui-goal')
    ...
    const resolved = new URL(source, 'https://fw.test/plugins/@forgeweaver/fw-client-ui-goal/client.js.map')
  ...
  it('maps dual-face host sources to the host package group', () => {
    const configs = clientConfigs('@forgeweaver/fw-host-directory-picker-native')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
  ...
  it('maps inlined workspace sources to packages and leaves dependencies outside it unchanged', () => {
    const configs = clientConfigs('@forgeweaver/fw-client-connection')
    const outputOptions = configs[0]?.outputOptions
    if (typeof outputOptions !== 'object' || outputOptions === null) throw new Error('client output options missing')
    const transform = outputOptions.sourcemapPathTransform
    if (transform === undefined) throw new Error('client sourcemap path transform missing')
    ...
    const resolved = new URL(workspaceSource, 'https://fw.test/plugins/@forgeweaver/fw-client-connection/client.js.map')
