...
const MERGE_HEAD = /declare module ['"](?:@forgeweaver\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@forgeweaver/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@forgeweaver/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@forgeweaver/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@forgeweaver/cordis'` Context merge
...
/**
 * Every event name a `declare module '@forgeweaver/cordis'` Events merge declares in one
