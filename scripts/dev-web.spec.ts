...
it('discovers fw.client packages with sibling roles', async () => {
  const root = await mkdtemp(join(tmpdir(), 'fw-dev-web-discovery-'))
  try {
...
it('discovers client-preset packages the shell links, excluding loader-delivered and test infrastructure', async () => {
  const root = await mkdtemp(join(tmpdir(), 'fw-dev-web-library-'))
  try {
    ...
    await write('packages/client/delivered', { fw: { client: { platform: 'web' } } }, clientPreset)
...
it('rebuilds a client-plugin bundle after its source changes', async () => {
  const root = await mkdtemp(join(tmpdir(), 'fw-dev-web-watch-'))
  let bundles: TsdownBundle[] = []
  try {
    await symlink(join(import.meta.dirname, '..', 'node_modules'), join(root, 'node_modules'), 'dir')
    await writeFile(join(root, 'package.json'), JSON.stringify({ name: '@fw-test/dev-web-watch', private: true, type: 'module' }))
