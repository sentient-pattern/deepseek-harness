 ...
 * itself (`fw web`), so any process that rewrites `lib/client.js` files
 ...
 * `lib/client.js`, and `vite build` rewrites `apps/web/dist`, which `fw web`
...
/** Compile-shell workspace whose dist `fw web` serves. */
const SHELL_PACKAGE = '@forgeweaver/fw-web-frontend'
 ...
 * whose package.json carries `fw.client` with platform "web" is a client
...
export function discoverPluginDirs(root = repoRoot): string[] {
  ...
  for (const manifestPath of globSync('packages/*/*/package.json', { cwd: root }).sort()) {
    const manifest = JSON.parse(readFileSync(join(root, manifestPath), 'utf8')) as {
      fw?: { client?: { platform?: unknown } }
    }
    if (manifest.fw?.client?.platform === 'web') dirs.push(dirname(manifestPath).split(sep).join('/'))
  }
  return dirs
}
 ...
 * client preset without declaring `fw.client` has no loader-delivered browser
...
export function discoverLibraryDirs(root = repoRoot): string[] {
  ...
  for (const configPath of globSync('packages/*/*/tsdown.config.ts', { cwd: root }).sort()) {
    ...
    if (manifest.fw?.client === undefined) dirs.push(dir)
  }
  return dirs
}
...
if (isMain) {
  ...
  if (pluginDirs.length === 0) {
    console.error('dev-web: no fw.client (platform "web") packages found under packages/')
    process.exit(1)
  }
  ...
  console.log(
    `dev-web: watching ${String(pluginDirs.length)} fw.client plugin packages`
