...
const SESSION_PACKAGE = '@forgeweaver/fw-session'

/** The type-only module that plugin declaration merges augment. */
const SESSION_TYPES_MODULE = '@forgeweaver/fw-session/types'
 ...
 * top-level declaration (in `@forgeweaver/fw-session`) and any declaration
 * merge inside a `declare module '@forgeweaver/fw-session/types'` block. Both forms
...
export function renderKnownEventTypes(events: AnnotatedLogEventEntry[]): string {
  ...
  return [
    ...
    ' * @module @forgeweaver/fw-session/known-event-types',
    ' */',
    '',
    '/**',
