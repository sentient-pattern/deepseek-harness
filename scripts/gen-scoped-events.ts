/**
 * Generate fw-scope's invariant resolver map from the repository TypeScript
 * Program.
 *
 ...
 * `Events` parameter tuple. Zero matches require `@fwScopeScan unsupported`;
...
class ScopedEventGenerator {
  ...
  render(): string {
    ...
    return [
      '/**',
      ' * Generated scoped-event routing-subject resolvers for fw-scope invariants.',
      ' * Do not edit by hand; run `pnpm run gen-scoped-events`.',
      ' *',
      ' * @module @forgeweaver/fw-scope/scoped-events.generated',
      ' */',
      '',
        ...
        if (ts.isInterfaceDeclaration(node) && node.name.text === 'Events' && isCordisModuleInterface(node)) {
          for (const member of node.members) {
            ...
            if (!scopedBase) {
              ...
              if (tag.present) {
                this.violations.push(`${where} has @fwScopeScan metadata but is not a Scoped event`)
              }
              continue
            }
            if (!raw.includes(SCOPE_DOC_MARKER)) {
            ...
            if (candidates.length === 0) {
              if (!tag.unsupported) {
                ...
                this.violations.push(
                  ...
                  + 'add @fwScopeScan unsupported only when the key is intentionally absent from the payload',
                )
              }
            ...
            if (tag.unsupported) {
              this.violations.push(
                `${where} has unnecessary @fwScopeScan unsupported; ${candidates[0]?.path} exposes the routing key`,
              )
              continue
            }
...
/** Return whether an Events interface is inside declare module '@forgeweaver/cordis'. */
function isCordisModuleInterface(node: ts.InterfaceDeclaration): boolean {
  ...
  return ts.isModuleBlock(block)
    ...
    && declaration.name.text === '@forgeweaver/cordis'
}
...
/** Parse and validate the optional @fwScopeScan unsupported tag. */
function parseScopeTag(raw: string, where: string, violations: string[]): ScopeTag {
  const tags = raw
    ...
    .filter(line => line.startsWith('@fwScopeScan'))
  if (tags.length > 1) violations.push(`${where} has multiple @fwScopeScan tags`)
  if (tags.length === 0) return { present: false, unsupported: false }
  const unsupported = tags[0] === '@fwScopeScan unsupported'
  if (!unsupported) {
    violations.push(
      `${where} has invalid scoped-event scan metadata '${tags[0]}'; expected '@fwScopeScan unsupported'`,
    )
  }
...
/** Generate or freshness-check the fixed fw-scope source file. */
function main(): void {
