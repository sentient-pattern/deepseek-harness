...
describe('tierExternalDeps', () => {
  it('tiers by declaring area, not by the declaring section name', () => {
    const { manifests, names } = workspace({
      ...
      'packages/test-support/loader-smoke/package.json': { name: '@forgeweaver/fw-loader-smoke', dependencies: { 'smoke-helper': '^1' } },
      'packages/test-support/client-runtime/package.json': { name: '@forgeweaver/fw-client-test-runtime', dependencies: { 'test-lib': '^1' } },
      ...
      'packages/mcp/mcp-client/package.json': { name: '@forgeweaver/fw-mcp-client', dependencies: { 'protocol-sdk': '^1' }, devDependencies: { 'protocol-fixture-server': '^1' } },
      'apps/cli/package.json': { name: '@forgeweaver/fw-cli', dependencies: { 'cli-lib': '^1', '@forgeweaver/fw-mcp-client': 'workspace:^' } },
    })
  ...
  it('keeps a package runtime when any shipping area declares it, and excludes workspace links', () => {
    const { manifests, names } = workspace({
      ...
      'packages/interaction/tui/package.json': { name: '@forgeweaver/fw-tui', dependencies: { shared: '^1', '@forgeweaver/fw-cli': 'workspace:^' } },
      'apps/cli/package.json': { name: '@forgeweaver/fw-cli' },
    })

    expect(tierExternalDeps(manifests, names).get('shared')).toBe(true)
    expect(tierExternalDeps(manifests, names).has('@forgeweaver/fw-cli')).toBe(false)
  })
})

describe('virtualManifest', () => {
  it('resolves a manifest from an ordinary prefix-matching store directory', () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-notices-prefix-'))
    try {
  ...
  it('falls back to a content scan when pnpm 11 truncates the store directory name', () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-notices-truncated-'))
    try {
      const name = '@scope/pkg'
      const version = '2.0.0'
      const store = join(root, 'store')
  ...
  it('returns undefined when neither the prefix nor the content scan finds the package', () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-notices-miss-'))
    try {
      const store = join(root, 'store')
...
describe('parseVendoredRows', () => {
  ...
  it('yields nothing when the table columns change, so the generator fails loud', () => {
    expect(parseVendoredRows('| `cordis/` | `@forgeweaver/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\n')).toEqual([])
  })
...
describe('parsePyprojectRequirements', () => {
  ...
  it('locates requirement arrays by TOML table, so author-named groups are not missed', () => {
    expect(parsePyprojectRequirements([
      ...
      'packages = ["src/forgeweaver_harness"]',
      '',
...
describe('collectPythonDependencies', () => {
  it('excludes normalized local project names without exempting a third-party prefix', () => {
    const pyprojects = [
      '[project]\nname = "forgeweaver-harness-runtime-bin"\ndependencies = ["pydantic"]\n',
      '[project]\nname = "forgeweaver-harness-sdk"\ndependencies = ["ForgeWeaver.Harness_Runtime-Bin", "forgeweaver-unrelated"]\n',
    ]
    expect(() => collectPythonDependencies(pyprojects)).toThrow(
      'python dependency forgeweaver-unrelated is missing from PYTHON_METADATA',
    )
  })
})
