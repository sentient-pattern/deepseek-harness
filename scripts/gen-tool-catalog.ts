...
import GoalService from '@forgeweaver/fw-goal'
import SystemPrompt from '@forgeweaver/fw-system-prompt'
import ToolRuntime, { type Config as ToolsConfig } from '@forgeweaver/fw-tools'
import LocalBashExecutor from '@forgeweaver/fw-bash-local'
import * as BashEnvPlugin from '@forgeweaver/fw-shell-env'
import { PwshLocalExecutor } from '@forgeweaver/fw-pwsh-local'
...
import type { ImageAttachmentLimits, ImageAttachmentRef, SaveImageAttachment, StoredImageAttachment } from '@forgeweaver/fw-attachment'
import UserQuestionService from '@forgeweaver/fw-user-questions'
import PlanModeController from '@forgeweaver/fw-plan-mode'
import WebRuntime from '@forgeweaver/fw-web'
import * as WebSearchExa from '@forgeweaver/fw-web-search-exa'
import * as WebFetchLocal from '@forgeweaver/fw-web-fetch-http'
import SubagentRuntime from '@forgeweaver/fw-subagent'
import type { SubagentProvider, SubagentReportDelivery } from '@forgeweaver/fw-subagent'
import * as ToolSubagentControl from '@forgeweaver/fw-tool-subagent-control'
import * as ToolSubagentListAgents from '@forgeweaver/fw-tool-subagent-control/list-agents'
import * as ToolSubagentReport from '@forgeweaver/fw-tool-subagent-report'
import SkillRegistry from '@forgeweaver/fw-skill'
import * as SkillFileSystem from '@forgeweaver/fw-skill-filesystem'
import LocalJobRegistry from '@forgeweaver/fw-jobs-local'
import * as ToolAskUser from '@forgeweaver/fw-tool-ask-user'
import * as ToolBash from '@forgeweaver/fw-tool-bash'
import * as ToolPwsh from '@forgeweaver/fw-tool-pwsh'
...
import CordisHostRunner from '@forgeweaver/fw-cordis-host-runner'
import * as ToolCordis from '@forgeweaver/fw-tool-cordis'
import * as ToolFs from '@forgeweaver/fw-tool-fs'
import * as ToolFsSearch from '@forgeweaver/fw-tool-fs-search'
import * as ToolStrReplaceEditor from '@forgeweaver/fw-tool-str-replace-editor'
import TerminalSessionService from '@forgeweaver/fw-terminal'
import * as ToolPty from '@forgeweaver/fw-tool-terminal'
import * as ToolGoal from '@forgeweaver/fw-tool-goal'
import * as ToolSchedule from '@forgeweaver/fw-schedule'
import Lsp from '@forgeweaver/fw-lsp'
import * as ToolLsp from '@forgeweaver/fw-tool-lsp'
import * as ToolSkill from '@forgeweaver/fw-tool-skill'
import * as ToolSessionQuery from '@forgeweaver/fw-tool-session-query'
import * as ToolTasks from '@forgeweaver/fw-tool-jobs'
import type TeamService from '@forgeweaver/fw-experimental-agent-team'
import * as ToolTeam from '@forgeweaver/fw-experimental-tool-agent-team'
import * as ToolTodo from '@forgeweaver/fw-tool-todo'
import * as ToolSubagent from '@forgeweaver/fw-tool-subagent'
import * as ToolWeb from '@forgeweaver/fw-tool-web'
import VmWorkflowEngine from '@forgeweaver/fw-workflow-worker-thread'
import * as ToolRalph from '@forgeweaver/fw-tool-ralph'
import * as ToolWorkflow from '@forgeweaver/fw-tool-workflow'
...
const TOOL_PACKAGES: ToolPackage[] = [
  {
    pkg: '@forgeweaver/fw-tool-ask-user',
    dir: 'tool-ask-user',
  ...
  },
  {
    ...
    note:
      'The bash tool is the model-facing consumer of the bash executor seam. A `run_in_background` run registers with the generic `ctx.jobs` runtime and is collected/stopped through the `job_*` tools from `@forgeweaver/fw-tool-jobs`; the `enableRunInBackground` config (default true) removes the parameter entirely when disabled.',
  },
  {
    pkg: '@forgeweaver/fw-tool-pwsh',
    dir: 'tool-pwsh',
    ...
    note:
      'The pwsh tool is the PowerShell-dialect consumer of the bash executor seam for Windows compositions (a PowerShell executor such as `@forgeweaver/fw-pwsh-local` backs `ctx.shell`); it mirrors the bash tool call-for-call minus sandbox controls — `run_in_background` runs register with the generic `ctx.jobs` runtime and are collected/stopped through the `job_*` tools, and the managed `FW_*` environment comes from `@forgeweaver/fw-shell-env`. Each call runs in a fresh process (no persistent PTY session), with native `C:\\...` paths and `$env:NAME` variables.',
  },
  {
    pkg: '@forgeweaver/fw-tool-cordis',
    dir: 'tool-cordis',
    ...
    note:
      'Not in any shipped tree (a deliberate opt-in — dynamic package code reaches the real runtime, see .agents/notes/implemented/feature/2026-07-08-self-referential-cordis-toolset.md). The toolset injects `ctx.dynamicCordisRunner` from `@forgeweaver/fw-cordis-host-runner`, which owns the definition registry and the vm sandbox; a composition missing it never activates the tools. A running package may register ADDITIONAL model-visible tools until it is stopped, undefined, or DSH restarts; a full changed request header logs those tool-set changes.',
  },
  {
    pkg: '@forgeweaver/fw-tool-bash-persistent',
  ...
  {
    pkg: '@forgeweaver/fw-tool-pwsh-persistent',
  ...
  {
    pkg: '@forgeweaver/fw-tool-str-replace-editor',
  ...
  },
  {
    ...
    note:
      'The read-before-write/edit policy is added by `@forgeweaver/fw-fs-observation-policy` (an `fs/*` event-gate plugin, no schema change); a deployment that loads these tools is expected to also load it. The image tool is not registered without `ctx.attachments`; its schema is route-independent, and execution refuses unless the exact routed model declares image input.',
  },
  {
    pkg: '@forgeweaver/fw-tool-fs-search',
    dir: 'tool-fs-search',
  ...
  {
    pkg: '@forgeweaver/fw-tool-terminal',
    dir: 'tool-terminal',
  ...
  },
  {
    ...
    note:
      'The lsp tool keeps provider selection and language-server subprocesses behind ctx.lsp, so its model-visible schema stays stable across providers. Requires a registered provider (e.g. `@forgeweaver/fw-lsp-stdio`) at runtime; without one, a query returns the structured `LSP_UNAVAILABLE` error rather than changing the schema.',
  },
  {
    pkg: '@forgeweaver/fw-tool-ralph',
    dir: 'tool-ralph',
  ...
  },
  {
    ...
    async mount(ctx) {
      ...
      await ctx.plugin(SkillFileSystem, {
        fwHome: resolve(root, '.tmp/tool-catalog/.fw'),
  ...
  {
    pkg: '@forgeweaver/fw-tool-session-query',
  ...
  {
    pkg: '@forgeweaver/fw-tool-subagent',
    dir: 'tool-subagent',
  ...
  {
    pkg: '@forgeweaver/fw-tool-subagent-control',
  ...
  {
    pkg: '@forgeweaver/fw-tool-subagent-report',
  ...
  {
    pkg: '@forgeweaver/fw-experimental-tool-agent-team',
    dir: 'tool-agent-team',
    ...
    scope: ctx => catalogChildScopes.get(ctx) as Agent,
    note:
      'All ten tools are scoped to implicit Team Leads and durable teammates. The shipped fw-base bundle keeps the package disabled; the documented Agent Teams profile patch enables it while disabling the legacy continuable-child control names.',
  },
  {
    pkg: '@forgeweaver/fw-tool-todo',
    dir: 'tool-todo',
  ...
  {
    pkg: '@forgeweaver/fw-tool-workflow',
    dir: 'tool-workflow',
