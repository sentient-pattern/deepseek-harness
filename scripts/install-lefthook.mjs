...
const HOOKS_DIRECTORY = 'fw-hooks'
const OWNERSHIP_MARKER = '.fw-lefthook-owned'
const OWNERSHIP_MARKER_VERSION = 1
const OWNERSHIP_MARKER_OWNER = 'forgeweaver-harness worktree-local lefthook hooks'
const INSTALL_LOCK = 'fw-lefthook-install.lock'
const INSTALL_LOCK_TIMEOUT_MS = 30_000
const INSTALL_LOCK_INITIALIZATION_TIMEOUT_MS = 5_000
const INSTALL_LOCK_POLL_MS = 50
const ALLOW_HOOKS_PATH_OVERRIDE = 'FW_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE'
const REPOSITORY_EXTENSION_PATTERN = '^extensions\\.'
const PAIRING_MERGE_DRIVER_CONFIG = [
  ['merge.fw-translation-pairing.name', 'ForgeWeaver bilingual pairing records'],
  [
    'merge.fw-translation-pairing.driver',
  ...
  while (true) {
    try {
      ...
      try {
        ...
        const writeDelay = Number(process.env.FW_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS ?? 0)
        if (writeDelay > 0) {
