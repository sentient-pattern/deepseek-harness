...
const forbiddenConfigKey = process.env.FW_TEST_FORBIDDEN_GIT_CONFIG_KEY
...
const delay = Number(process.env.FW_TEST_LEFTHOOK_DELAY_MS ?? 0)
if (delay > 0) Atomics.wait(new Int32Array(new SharedArrayBuffer(4)), 0, 0, delay)
const shouldFail = process.env.FW_TEST_LEFTHOOK_FAIL === '1'
if (!shouldFail) {
...
if (process.env.FW_TEST_LEFTHOOK_BREAK_WORKTREE_CONFIG === '1') {
...
function createFixture(names: { main?: string; linked?: string } = {}): Fixture {
  const container = mkdtempSync(join(tmpdir(), 'fw-lefthook-'))
...
function hooksPath(fixture: Fixture, root: string): string {
  return join(gitDirectory(fixture, root), 'fw-hooks')
}

function installLockPath(fixture: Fixture): string {
  return join(commonDirectory(fixture), 'fw-lefthook-install.lock')
}
...
describe('worktree-local Lefthook installer', { timeout: 30_000 }, () => {
  for (const [label, extraEnv] of [
    ...
    it(`skips hook installation when ${label} marks an automated job`, async () => {
      ...
      expect(gitResult(fixture, fixture.main, [
        'config', '--get', 'merge.fw-translation-pairing.driver',
      ]).status).toBe(1)
    })
  }
  ...
  it('isolates main and linked worktrees without changing legacy common hooks', async () => {
    ...
    expect(git(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.fw-translation-pairing.driver',
  ...
  it('serializes concurrent installs and keeps repeated output stable', async () => {
    const fixture = createFixture()
    const delayed = { FW_TEST_LEFTHOOK_DELAY_MS: '150' }
    ...
    expect(existsSync(join(commonDirectory(fixture), 'fw-lefthook-install.lock'))).toBe(false)
  ...
  it('waits for a concurrent installer to finish publishing its lock record', async () => {
    ...
    const publishing = runInstaller(fixture, fixture.main, {
      FW_TEST_LEFTHOOK_LOCK_WRITE_DELAY_MS: '200',
    })
  ...
  it('repairs its owned absolute hook path after the checkout moves', async () => {
    ...
    expect(readFileSync(join(movedHooks, '.fw-lefthook-owned'), 'utf8')).toContain(
  ...
  it.skipIf(process.platform === 'win32')('refuses a multiply linked ownership marker before relocation rewrites it', async () => {
    ...
    expect(first.status, first.stderr).toBe(0)
    const oldHooks = hooksPath(fixture, oldRoot)
    const markerName = '.fw-lefthook-owned'
  ...
  it('restores the marker-backed stale hook path when relocation reinstall fails', async () => {
    ...
    const failed = await runInstaller(fixture, movedRoot, { FW_TEST_LEFTHOOK_FAIL: '1' })
  ...
  it('refuses dormant repository extensions before upgrading the repository format', async () => {
    ...
    git(fixture, fixture.main, ['config', 'extensions.fwUnknown', 'true'])
    expect(gitResult(fixture, fixture.main, ['status', '--porcelain']).status).toBe(0)

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('dormant repository extension extensions.fwunknown')
    expect(git(fixture, fixture.main, [
  ...
  it('does not release an installer lock whose ownership changed', async () => {
    ...
    const install = runInstaller(fixture, fixture.main, { FW_TEST_LEFTHOOK_DELAY_MS: '250' })
    try {
  ...
  it('preserves user-owned hook paths unless an inherited value is explicitly overridden', async () => {
    ...
    expect(refused.stderr).toContain('FW_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE=1')
    ...
    const optedIn = await runInstaller(fixture, fixture.main, {
      FW_LEFTHOOK_ALLOW_HOOKS_PATH_OVERRIDE: '1',
    })
  ...
  it('does not trust an ownership marker outside a registered worktree hook path', async () => {
    ...
    write(
      join(externalHooks, '.fw-lefthook-owned'),
      `${JSON.stringify({
        version: 1,
        owner: 'forgeweaver-harness worktree-local lefthook hooks',
  ...
  for (const includeKey of ['include.path', 'includeIf.onbranch:conditional.path']) {
    for (const key of ['core.worktree', 'core.bare', 'extensions.fwunknown']) {
  ...
  it('never replaces a custom worktree pairing merge driver', async () => {
    ...
    git(fixture, fixture.main, ['config', '--file', commonConfig, 'extensions.worktreeConfig', 'true'])
    git(fixture, fixture.main, [
      'config', '--worktree', 'merge.fw-translation-pairing.driver', 'custom-driver %A',
    ])

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('refusing to replace worktree merge.fw-translation-pairing.driver')
    expect(git(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.fw-translation-pairing.driver',
  ...
  it('never masks an inherited custom pairing merge driver', async () => {
    const fixture = createFixture()
    git(fixture, fixture.main, [
      'config', '--local', 'merge.fw-translation-pairing.driver', 'inherited-driver %A',
    ])

    const result = await runInstaller(fixture, fixture.main)

    expect(result.status).toBe(1)
    expect(result.stderr).toContain('refusing to mask inherited merge.fw-translation-pairing.driver')
    expect(git(fixture, fixture.main, [
      'config', '--local', '--get', 'merge.fw-translation-pairing.driver',
  ...
  it('does not pass unrelated command-scoped Git config to Lefthook', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, {
      FW_TEST_FORBIDDEN_GIT_CONFIG_KEY: 'fw.testSentinel',
      GIT_CONFIG_COUNT: '1',
      GIT_CONFIG_KEY_0: 'fw.testSentinel',
  ...
  it('restores the previous hook lookup when Lefthook installation fails', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, { FW_TEST_LEFTHOOK_FAIL: '1' })
    expect(result.status).toBe(1)
    ...
    expect(gitResult(fixture, fixture.main, ['config', '--get', 'core.hooksPath']).status).toBe(1)
    expect(gitResult(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.fw-translation-pairing.name',
    ]).status).toBe(1)
    expect(gitResult(fixture, fixture.main, [
      'config', '--worktree', '--get', 'merge.fw-translation-pairing.driver',
    ]).status).toBe(1)
  ...
  it('reports installation and hook-path rollback failures together', async () => {
    ...
    const result = await runInstaller(fixture, fixture.main, {
      FW_TEST_LEFTHOOK_BREAK_WORKTREE_CONFIG: '1',
      FW_TEST_LEFTHOOK_FAIL: '1',
    })
    ...
    expect(result.stderr).toContain('git config --worktree --unset-all merge.fw-translation-pairing.driver failed')
  })
