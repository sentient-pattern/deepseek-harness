...
function fixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'fw-package-invariants-'))
  roots.push(root)
  const dir = join(root, 'packages/core/probe')
  mkdirSync(join(dir, 'src'), { recursive: true })
  const packageName = options.packageName ?? '@forgeweaver/fw-probe'
  const manifest = {
    name: packageName,
    ...
    peerDependencies: options.invariantDependency === false ? {} : {
      '@forgeweaver/fw-invariants': 'workspace:^',
    },
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) => {
  ctx.invariants.register('@forgeweaver/fw-foreign', install)
...
`
  ...
  it('rejects generated markers and reporter-free executable installers', () => {
    const generated = fixture({
      source: `/** @generated */\n${handwrittenInvariant('@forgeweaver/fw-probe')}`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: typeof install): () => void } }) =>
  Promise.resolve(ctx.invariants.register('@forgeweaver/fw-probe', install))
`,
    })
...
export const apply = (ctx: { invariants: { register(name: string, install: () => void): () => void } }) =>
  ctx.invariants.register('@forgeweaver/fw-probe', () => {})
`,
    })
  ...
  it.each([
    ...
    const source = `${handwrittenInvariant('@forgeweaver/fw-probe')}\n${defaultExport}\n`
...
export const name = 'probe-invariant'
export const inject = ['invariants']
const PACKAGE_NAME = '@forgeweaver/fw-probe'
