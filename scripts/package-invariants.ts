...
function checkManifest(
  ...
  if (owner.packageName === '@forgeweaver/fw-invariants') return
  if (manifest.peerDependencies?.['@forgeweaver/fw-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@forgeweaver/fw-invariants must be a workspace:^ peerDependency',
    )
  }
  if (manifest.devDependencies?.['@forgeweaver/fw-invariants'] !== 'workspace:^') {
    addViolation(
      violations,
      owner.manifestPath,
      '@forgeweaver/fw-invariants must also be a workspace:^ devDependency',
    )
  }
}
...
function checkBuild(
  ...
  if (owner.packageName !== '@forgeweaver/fw-invariants'
