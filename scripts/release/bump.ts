 ...
 *
 * The fw family shares one version across its publishable members, private
...
/** Release types the fw family accepts besides an explicit version. */
const RELEASE_TYPES = ['major', 'minor', 'patch'] as const

/** The workspace root manifest, which carries the fw family's version. */
...
/** One private fw package whose version follows the publishable family. */
...
function nextSharedVersion(current: string, request: string): string {
  if (!RELEASE_TYPES.includes(request as typeof RELEASE_TYPES[number])) {
    if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/.test(request)) {
      throw new Error(`usage: release:fw <major|minor|patch|x.y.z>, got ${request}`)
    }
    return request
  }
...
/**
 * Discover private package manifests that share the fw version without joining
 * its publish set.
 * @param root - repository root.
...
/**
 * Plan the fw family's rewrite: one version for every publishable member,
 * private package, and the root.
 * @param family - the fw family.
 * @param root - repository root.
...
function main(): void {
  ...
  if (values.family === undefined) throw new Error('usage: bump.ts --family <fw|vendor> [version]')
  ...
  if (family.id === 'fw') {
    ...
    if (request === undefined) throw new Error('usage: release:fw <major|minor|patch|x.y.z>')
    if (values.prerelease !== undefined) {
      throw new Error('release:fw takes the prerelease in its version argument, as in 0.0.1-rc.1')
    }
