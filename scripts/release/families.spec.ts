...
function buildFixture(environment: Record<string, string>): string {
  const root = mkdtempSync(join(tmpdir(), 'fw-release-build-'))
  roots.push(root)
...
describe('release families', () => {
  it('excludes private experimental packages from the fw release', () => {
    const members = releaseFamily('fw').members(resolve(import.meta.dirname, '../..'))

    expect(members.some(member => member.directory.startsWith('packages/experimental/'))).toBe(false)
    expect(members.map(member => member.name)).not.toContain('@forgeweaver/fw-experimental-agent-team')
  })

  it('bumps private fw packages without adding release tags', () => {
    const root = mkdtempSync(join(tmpdir(), 'fw-release-version-'))
    roots.push(root)
    ...
    const fw = releaseFamily('fw')
    const published = member('packages/core/published', '@forgeweaver/fw-published')
    const { planned } = planShared(fw, root, [published], '0.0.2')

    expect(planned.map(entry => ({ path: entry.manifestPath, tag: entry.tag }))).toEqual([
      { path: 'package.json', tag: undefined },
      { path: 'packages/core/published/package.json', tag: 'fw-v0.0.2' },
      { path: 'packages/experimental/prototype/package.json', tag: undefined },
    ])
  })

  it('names one tag for the whole fw family and one per vendored package', () => {
    const fw = releaseFamily('fw')
    const vendor = releaseFamily('vendor')
    const cli = member('apps/cli', '@forgeweaver/fw')
    const cordis = { ...member('vendor/cordis', '@forgeweaver/cordis'), version: '4.0.1' }

    expect(fw.tagFor(cli)).toBe('fw-v0.0.1')
  ...
  it('rejects a family whose members disagree on the shared version', () => {
    const fw = releaseFamily('fw')
    const members = [member('apps/cli', '@forgeweaver/fw'), { ...member('apps/web', '@forgeweaver/fw-web-frontend'), version: '0.0.2' }]

    expect(() => { fw.verifyVersions(members) }).toThrow(/must share one version/)
    expect(() => { fw.verifyVersions([members[0]!]) }).not.toThrow()
  })

  it('accepts independent vendored versions and rejects an unpublishable one', () => {
    const vendor = releaseFamily('vendor')
    const members = [
      { ...member('vendor/cordis', '@forgeweaver/cordis'), version: '4.0.1' },
      { ...member('vendor/cosmokit', '@forgeweaver/cosmokit'), version: '1.8.2' },
    ]
  ...
  it('requires a current official client build only for fw artifacts', () => {
    const fw = releaseFamily('fw')
    const vendor = releaseFamily('vendor')
    const officialEnvironment = officialClientBuildEnvironment(resolve(import.meta.dirname, '../..'))
    vi.stubEnv('FW_CLIENT_COMMIT_HASH', officialEnvironment.FW_CLIENT_COMMIT_HASH)
    const official = buildFixture(officialEnvironment)
    const defaultBuild = buildFixture({})

    expect(() => { fw.verifyBuildArtifacts(official) }).not.toThrow()
    expect(() => { fw.verifyBuildArtifacts(defaultBuild) }).toThrow(/FW_CLIENT_TITLE/)
    expect(() => { fw.verifyBuildArtifacts(join(defaultBuild, 'missing')) }).toThrow(/record.*missing/)
    ...
    expect(() => { fw.verifyBuildArtifacts(official) }).toThrow(/artifacts differ/)
  })

  it('publishes a dependency before its consumer, and orders ties by name', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/consumer', '@forgeweaver/fw-consumer', { dependencies: { '@forgeweaver/fw-library': 'workspace:^' } }),
      member('packages/a/library', '@forgeweaver/fw-library'),
      member('packages/a/zebra', '@forgeweaver/fw-zebra'),
    ]

    expect(fw.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@forgeweaver/fw-library',
      '@forgeweaver/fw-consumer',
      '@forgeweaver/fw-zebra',
    ])
  })
  ...
  it('reports a runtime dependency cycle instead of emitting an arbitrary order', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/left', '@forgeweaver/fw-left', { dependencies: { '@forgeweaver/fw-right': 'workspace:^' } }),
      member('packages/a/right', '@forgeweaver/fw-right', { dependencies: { '@forgeweaver/fw-left': 'workspace:^' } }),
    ]

    expect(() => { fw.publishOrder(members) }).toThrow(/dependency cycle/)
  })

  it('publishes a peer before its consumer', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/consumer', '@forgeweaver/fw-consumer', { peerDependencies: { '@forgeweaver/fw-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@forgeweaver/fw-zebra'),
    ]
  ...
  it('orders around a peer cycle rather than refusing to publish, and reports the edge it dropped', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/left', '@forgeweaver/fw-left', { peerDependencies: { '@forgeweaver/fw-right': 'workspace:^' } }),
      member('packages/a/right', '@forgeweaver/fw-right', { peerDependencies: { '@forgeweaver/fw-left': 'workspace:^' } }),
    ]
  ...
  it('honours an install edge even when a peer cycle surrounds it', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/base', '@forgeweaver/fw-base', { peerDependencies: { '@forgeweaver/fw-consumer': 'workspace:^' } }),
      member('packages/a/consumer', '@forgeweaver/fw-consumer', {
        dependencies: { '@forgeweaver/fw-base': 'workspace:^' },
        peerDependencies: { '@forgeweaver/fw-base': 'workspace:^' },
      }),
    ]
    ...
    expect(plan.droppedPeerEdges).toEqual([
      { consumer: '@forgeweaver/fw-base', peer: '@forgeweaver/fw-consumer' },
    ])
  })
  ...
  it('refuses an order that would publish a consumer before a dependency it installs', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/alpha', '@forgeweaver/fw-alpha', { peerDependencies: { '@forgeweaver/fw-bravo': 'workspace:^' } }),
      member('packages/a/bravo', '@forgeweaver/fw-bravo', { peerDependencies: { '@forgeweaver/fw-charlie': 'workspace:^' } }),
      member('packages/a/charlie', '@forgeweaver/fw-charlie', { dependencies: { '@forgeweaver/fw-alpha': 'workspace:^' } }),
    ]
    ...
    expect(() => { fw.publishOrder(members) }).toThrow(/no publish order honours @forgeweaver\/fw-charlie -> @forgeweaver\/fw-alpha/)
  })

  it('ignores devDependencies when ordering', () => {
    const fw = releaseFamily('fw')
    const members = [
      member('packages/a/alpha', '@forgeweaver/fw-alpha', { devDependencies: { '@forgeweaver/fw-zebra': 'workspace:^' } }),
      member('packages/a/zebra', '@forgeweaver/fw-zebra'),
    ]
    ...
    expect(fw.publishOrder(members).order.map(entry => entry.name)).toEqual([
      '@forgeweaver/fw-alpha',
      '@forgeweaver/fw-zebra',
    ])
  })

  it('applies the harness payload policy to fw and keeps upstream payloads for vendored packages', () => {
    const fw = releaseFamily('fw')
    const vendor = releaseFamily('vendor')
    const harness = member('packages/a/library', '@forgeweaver/fw-library')
    const vendored = member('vendor/cordis', '@forgeweaver/cordis')

    expect(() => { fw.validatePayload(harness, ['package/lib/index.js', 'package/src/index.ts']) })
  ...
  it('drives the installed entry only for the family that publishes one', () => {
    expect(releaseFamily('fw').installedEntry).toEqual({ packageName: '@forgeweaver/fw', binPath: 'lib/bin.js' })
...
describe('payload change judgement', () => {
  const sourceShipping = member('vendor/cosmokit', '@forgeweaver/cosmokit', {
    files: ['lib/index.js', 'lib/types/**/*.d.ts', 'src'],
  })
  const buildOutputOnly = member('vendor/cordis', '@forgeweaver/cordis', {
  ...
  it('ignores paths no tarball carries', () => {
    ...
    expect(reachesPayload(member('packages/a/library', '@forgeweaver/fw-library', { files: ['lib/index.js'] }),
