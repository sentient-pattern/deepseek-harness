 ...
 * owns: `fw` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@forgeweaver/fw-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@forgeweaver/')) throw new Error(`${normalized} must name an @forgeweaver package`)
...
class DshFamily extends ReleaseFamily {
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`fw release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@forgeweaver/fw', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@forgeweaver/', '')}-v`
  }
