/**
 * Rescope the vendored Cordis packages into the `@forgeweaver` scope, and undo
 ...
 * `@forgeweaver/fw-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@forgeweaver/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@forgeweaver/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@forgeweaver/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@forgeweaver/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@forgeweaver/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@forgeweaver/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@forgeweaver/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@forgeweaver/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@forgeweaver/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@forgeweaver/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@forgeweaver/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@forgeweaver\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@forgeweaver/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@forgeweaver/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@forgeweaver/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@forgeweaver/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@forgeweaver/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@forgeweaver/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@forgeweaver` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@forgeweaver\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@forgeweaver/cordis']
    const dev = manifest.devDependencies?.['@forgeweaver/cordis']

    if (!peer) errors.push(\`\${label}: @forgeweaver/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @forgeweaver/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @forgeweaver/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    // The rescoped name is already covered by the `@forgeweaver/.+` pattern beside it.
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@forgeweaver/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@forgeweaver/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@forgeweaver/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@forgeweaver/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@forgeweaver/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @forgeweaver, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@forgeweaver\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@forgeweaver/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@forgeweaver\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@forgeweaver/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @forgeweaver, so the consumer installs this repository's
  ...
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'fw-packed-consumer', private: true, type: 'module' }))
