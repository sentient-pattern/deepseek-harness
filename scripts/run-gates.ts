...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.FW_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('FW_GATE_CONCURRENCY', concurrencyDefault.workers)
    ...
    case 'check-all':
      return [
        ...
        ...docSyncLeafGates({
          ...
          docTypecheckEnv: { FW_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
...
function ciSharedStaticGates(): Gate[] {
  return [
    ...
    pnpmScript('fw-package-licenses', 'verify-fw-package-licenses', { label: 'DSH package licenses' }),
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('FW_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('fw-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'fw source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { FW_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { FW_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  const workerRaw = process.env.FW_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 2 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: FW_WEB_SNAPSHOT_WORKERS must be an integer greater than 1, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `FW_SNAPSHOT=replay FW_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
      env: { FW_SNAPSHOT: 'replay' },
      needs,
      streamOutput: true,
    })
  }
  return pnpmScript('web-snapshot', 'test:web:built', {
    label: 'web browser snapshot',
    displayCommand: 'FW_SNAPSHOT=replay pnpm run test:web:built',
    env: { FW_SNAPSHOT: 'replay' },
    needs,
  })
}
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.FW_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `FW_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// FW_COVERAGE_MAX_WORKERS is the ordinary lane's worker budget, so the two
// parallel gates split it instead of each claiming it whole. When
// FW_COVERAGE_PARTITIONS is set, its single-worker processes replace the
...
// total of one (the serial reference jobs) also set FW_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// FW_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test and expect.poll
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('FW_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.FW_GATE_VERBOSE === '1'
