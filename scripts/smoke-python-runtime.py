...
if TYPE_CHECKING:
    from forgeweaver_harness import RunResult
...
# (forgeweaver-harness#2488), and the file must replay on both. Everything else is compared.
...
- id: sessions
  ...
  config:
    root: !!js process.env.FW_SESSION_ROOT
    compression: 'none'
- id: code-runtime
  name: '@forgeweaver/fw-code-runtime-worker-thread'
- id: subagents
  name: '@forgeweaver/fw-subagent'
- id: subagent-spawn-in-process
  name: '@forgeweaver/fw-subagent-spawn-in-process'
  config:
    providerName: spawn
- id: subagent-tool
  ...
  config:
    provider: spawn
- id: workflow-engine
  name: '@forgeweaver/fw-workflow-worker-thread'
  config:
    provider: spawn
- id: workflow-tool
  name: '@forgeweaver/fw-tool-workflow'
- id: cordis-host-runner
  name: '@forgeweaver/fw-cordis-host-runner'
- id: cordis-tool
  name: '@forgeweaver/fw-tool-cordis'
"""
FS_SEARCH_CORDIS = """\
- id: sdk-jsonrpc-server
  name: '@forgeweaver/fw-sdk-jsonrpc-server'
...
- id: fs-search
  name: '@forgeweaver/fw-tool-fs-search'
  config:
...
def smoke_sdk_default(base_url: str) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-sdk-default-") as temporary:
...
def smoke_sdk_custom(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-sdk-custom-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_minimal(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-sdk-minimal-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_fs_search(base_url: str, executable: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-sdk-fs-search-") as temporary:
        root = Path(temporary).resolve()
...
def smoke_sdk_mcp(base_url: str, executable: Path | None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-sdk-mcp-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
...
def smoke_sdk_snapshot(base_url: str, executable: Path, update_snapshots: bool) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fw-sdk-snapshot-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="fw-direct-") as temporary:
        root = Path(temporary).resolve()
        sessions = root / "sessions"
        cordis = root / "cordis.yml"
        cordis.write_text(CUSTOM_CORDIS)
        environment = {
            **os.environ,
            "FW_CORDIS_CONFIG": str(cordis),
            "FW_SESSION_ROOT": str(sessions),
            "FW_CWD": str(root),
            "FORGEWEAVER_API_KEY": "sk-keyless-smoke",
            "FORGEWEAVER_BASE_URL": base_url,
        }
        peer = RuntimePeer([str(executable)], root, environment)
        try:
            peer.send({"jsonrpc": "2.0", "id": "initialize", "method": "initialize", "params": {"cwd": str(root), "provider": "forgeweaver-official", "model": "smoke-model"}})
