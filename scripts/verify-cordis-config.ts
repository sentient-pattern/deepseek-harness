 ...
 * silently changes composition. Example configs and the fw Web composition
...
// These example files are overlays consumed by the built fw app, so their bare
...
const CHOOSER_PACKAGE = '@forgeweaver/fw-host-directory-picker-auto'
...
const CHOOSER_BACKEND_PACKAGES = [
  '@forgeweaver/fw-host-directory-picker-native',
  '@forgeweaver/fw-host-directory-picker-browse',
  '@forgeweaver/fw-client-ui-directory-picker-browse',
  '@forgeweaver/fw-client-ui-directory-picker-native',
]
const errors: string[] = []
 ...
 * `fw.client` block, and the node half of a surface plugin is an empty
 ...
 * `fw.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    ...
    const declaresClient = manifest.fw?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no fw.client, so its browser half is never served`
      : `${manifestPath}: declares fw.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `FW_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@forgeweaver/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // fw app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.fw?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).fw?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `fw` source
