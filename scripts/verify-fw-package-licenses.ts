 ...
 * @module scripts/verify-fw-package-licenses
 */
...
const FW_PACKAGE_NAME = /^@forgeweaver\/fw(?:-|$)/
...
function readManifest(root: string, file: string): Record<string, unknown> {
  ...
  if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
    throw new Error(`verify-fw-package-licenses: ${file} must contain a JSON object.`)
  }
...
function workspaceManifestPaths(root: string): string[] {
  ...
  if (!isStringArray(workspaces)) {
    throw new Error('verify-fw-package-licenses: package.json workspaces must be a string array.')
  }
...
export function inspectDshPackageLicenses(root: string): DshPackageLicenseReport {
  ...
  for (const file of workspaceManifestPaths(root)) {
    ...
    if (typeof name !== 'string' || !FW_PACKAGE_NAME.test(name)) continue
...
if (process.argv[1] && import.meta.filename === resolve(process.argv[1])) {
  ...
  if (report.failures.length > 0) {
    process.stderr.write('verify-fw-package-licenses: non-MIT DSH package declarations found:\n')
    for (const failure of report.failures) process.stderr.write(`  ${failure}\n`)
    process.exitCode = 1
  } else {
    process.stdout.write(
      `verify-fw-package-licenses: ${String(report.packageCount)} DSH package(s) checked; all declare MIT.\n`,
    )
  }
}
