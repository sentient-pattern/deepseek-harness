...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/sentient-pattern/deepseek-harness' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/sentient-pattern/deepseek-harness/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
/**
 * The ForgeWeaver wordmark, inlined so its `currentColor` fills follow the active
...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="fw-wordmark" ')
 ...
 */
const siteStyle = `
.fw-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.fw-wordmark { display: block; height: 22px; width: auto; color: var(--vp-c-text-1); }
.fw-tag {
  display: inline-flex;
  align-items: center;
...
/**
 * Navigation-bar title: the ForgeWeaver wordmark and the release-stage tag.
...
function siteTitle(previewTag: string): string {
  return `<span class="fw-lockup">${wordmark}<span class="fw-tag">${previewTag}</span></span>`
}
...
export default withMermaid({
  ...
  vite: {
    ...
    plugins: [
      {
        name: 'forgeweaver-harness-doc-projector',
